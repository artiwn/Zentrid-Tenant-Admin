type FleetNavItem = { icon: string; label: string; href: string; key: string };
type FleetNavGroup = { section: string; items: FleetNavItem[] };
type FleetContextState = { tenant: string; time: string; region: string };
type FleetSearchItem = { type: string; label: string; meta: string; action: string; keywords?: string[] };
type FloatingKebabMenu = HTMLElement & { __zentridHome?: { parent: ParentNode; next: ChildNode | null } };
...
const FleetLayout = (() => {
  ...
  function resolvedTenantName(): string {
  ...
  function detailActive(itemKey: string): boolean {
  ...
  function menu(id: string, items: Array<{ label: string; value: string }>): string {
  ...
  function toast(message: string, requestedTone?: FleetUXTone): void {
    ...
    const icons: Record<FleetUXTone, string> = { info: 'i', success: '✓', warning: '△', danger: '!', neutral: '•' };
    element.className = `toast ${tone}`;
    element.replaceChildren();
    const icon = document.createElement('span'); icon.className = 'toast-icon'; icon.textContent = icons[tone];
    const copy = document.createElement('span'); copy.className = 'toast-message'; copy.textContent = message;
    const close = document.createElement('button'); close.type = 'button'; close.className = 'toast-close'; close.textContent = '×'; close.addEventListener('click', () => element?.classList.remove('show'));
  ...
  const searchItems: FleetSearchItem[] = [
  ...
  function renderSearch(query: string): void {
    ...
    if (!normalized) { box.classList.remove('open'); box.innerHTML = ''; return; }
  ...
  function hydrateAuthProfile(): void {
    const label = currentUserLabel();
    const initials = currentUserInitials();
    const name = document.getElementById('profileName'); if (name) name.textContent = label;
    const avatar = document.getElementById('profileAvatar'); if (avatar) avatar.textContent = initials;
    const menuName = document.getElementById('profileMenuName'); if (menuName) menuName.textContent = label;
    const tenantLabel = document.getElementById('tenantLabel'); if (tenantLabel) tenantLabel.textContent = resolvedTenantName();
  }
  ...
  function wireHeader(): void {
    ...
    [[timeButton, timeMenu], [notifyButton, notifyMenu], [profileButton, profileMenu]].forEach(([button, menuElement]) => button?.addEventListener('click', event => { event.stopPropagation(); const open = menuElement?.classList.contains('open'); closeMenus(); if (!open) menuElement?.classList.add('open'); }));

    timeMenu?.addEventListener('click', event => {
      const button = layoutEventTarget(event)?.closest<HTMLButtonElement>('button[data-value]');
      if (!button) return;
      state.time = button.dataset.value || state.time;
      localStorage.setItem('zentrid_time', state.time);
      const label = document.getElementById('timeLabel'); if (label) label.textContent = state.time;
      closeMenus(); window.dispatchEvent(new CustomEvent('zentrid:context-change', { detail: { ...state } }));
    });

    document.querySelectorAll<HTMLButtonElement>('[data-action]').forEach(button => button.addEventListener('click', async event => {
      const action = button.dataset.action || '';
      if (action === 'logout') { window.ZentridAuth?.logout?.(); window.location.href = basePath + 'login.html'; return; }
      if (action === 'refresh-auth') {
        try { await window.ZentridAuth?.me?.(); hydrateAuthProfile(); toast('Authentication profile refreshed.', 'success'); }
        catch { toast('Unable to refresh authentication profile.', 'danger'); }
        closeMenus(); return;
      }
      if (action) { event.preventDefault(); window.location.href = pathFor(action); }
    }));
    ...
    search?.addEventListener('keydown', event => {
      ...
      if (first) window.location.href = pathFor(first.action);
    });
    results?.addEventListener('click', event => {
      event.stopPropagation();
      const button = layoutEventTarget(event)?.closest<HTMLButtonElement>('button[data-action]');
      if (button) window.location.href = pathFor(button.dataset.action || 'overview');
    });

    document.addEventListener('click', event => {
      if (!layoutEventTarget(event)?.closest('.menu-wrap')) closeMenus();
      if (!layoutEventTarget(event)?.closest('.search-wrap')) results?.classList.remove('open');
      const route = layoutEventTarget(event)?.closest<HTMLElement>('[data-tenant-route]');
      if (route) { event.preventDefault(); window.location.href = pathFor(route.dataset.tenantRoute || 'overview'); }
    });
    document.addEventListener('keydown', event => { if (event.key === 'Escape') { closeMenus(); results?.classList.remove('open'); } });
  }
  ...
  function enhanceActionMenus(root: Document | Element = document): void {
    root.querySelectorAll<HTMLElement>('.data-row > .row-actions:not(.kebabified)').forEach(actions => {
      if (actions.closest('.drawer-actions, .modal-actions, .panel-head, .wizard-actions') || actions.dataset.noKebab === 'true') return;
      const buttons = Array.from(actions.children).filter((element): element is HTMLButtonElement => element.tagName === 'BUTTON');
      if (!buttons.length) return;
      if (buttons.length === 1) { actions.classList.add('single-action'); buttons[0]?.classList.add('single-row-action'); return; }
      const wrap = document.createElement('div'); wrap.className = 'kebab-wrap global-action-wrap';
      const trigger = document.createElement('button'); trigger.type = 'button'; trigger.className = 'kebab-btn'; trigger.dataset.action = 'menu'; trigger.setAttribute('aria-label', 'Open actions'); trigger.textContent = '⋮';
      const menuElement = document.createElement('div'); menuElement.className = 'kebab-menu global-action-menu';
      buttons.forEach(button => { button.textContent = actionLabel(button); button.type = 'button'; menuElement.appendChild(button); });
      actions.replaceChildren(wrap); wrap.append(trigger, menuElement); actions.classList.add('kebabified');
    });
  }

  function resetFloatingMenu(menuElement?: FloatingKebabMenu | null): void {
    if (!menuElement) return;
    menuElement.classList.remove('open', 'floating-menu'); menuElement.style.left = ''; menuElement.style.top = ''; menuElement.style.visibility = '';
    if (menuElement.__zentridHome?.parent && menuElement.parentNode === document.body) menuElement.__zentridHome.parent.insertBefore(menuElement, menuElement.__zentridHome.next || null);
  }

  function closeActionMenus(except?: Element | null): void {
    document.querySelectorAll<FloatingKebabMenu>('.kebab-menu.open').forEach(menuElement => { if (menuElement !== except) resetFloatingMenu(menuElement); });
  }

  function positionActionMenu(button: Element, menuElement: FloatingKebabMenu): void {
    if (!menuElement.__zentridHome && menuElement.parentNode) menuElement.__zentridHome = { parent: menuElement.parentNode, next: menuElement.nextSibling };
    document.body.appendChild(menuElement); menuElement.classList.add('open', 'floating-menu'); menuElement.style.visibility = 'hidden';
    const buttonRect = button.getBoundingClientRect(); const menuRect = menuElement.getBoundingClientRect();
    ...
    menuElement.style.left = `${Math.round(left)}px`; menuElement.style.top = `${Math.round(top)}px`; menuElement.style.visibility = '';
  }
  ...
  function wireActionMenus(): void {
    ...
    document.addEventListener('click', event => {
      ...
      if (trigger) {
        ...
        const menuElement = trigger.closest('.kebab-wrap')?.querySelector<FloatingKebabMenu>('.kebab-menu');
        const wasOpen = menuElement?.classList.contains('open'); closeActionMenus(menuElement); if (menuElement && !wasOpen) positionActionMenu(trigger, menuElement); else resetFloatingMenu(menuElement); return;
      }
      const action = target?.closest<HTMLButtonElement>('.kebab-menu.floating-menu button');
      if (action) {
        event.preventDefault(); event.stopPropagation();
        const menuElement = action.closest<FloatingKebabMenu>('.kebab-menu'); resetFloatingMenu(menuElement); action.dispatchEvent(new MouseEvent('click', { bubbles: true, cancelable: true, view: window })); return;
      }
  ...
  function mount(content: string): FleetContextState {
    ...
    wireHeader(); hydrateAuthProfile(); wireActionMenus();
    const main = app.querySelector('.main-content');
    if (main) enhanceActionMenus(main);
    if (main && !['client-detail', 'plant-detail'].includes(document.body?.dataset.tenantPage || '')) new MutationObserver(() => enhanceActionMenus(main)).observe(main, { childList: true, subtree: true });
    return state;
  }
