...
function savePlants(_list) { }
function plantStatusCls(v) {
    const value = String(v).toLowerCase();
    if (value.includes('fault') || value.includes('offline'))
        return 'danger';
    if (value.includes('warning') || value.includes('delayed'))
        return 'warning';
    return 'success';
}
function plantFreshnessEscape(value) { return String(value ?? '').replace(/[&<>"']/g, character => ({ '&': '&amp;', '<': '&lt;', '>': '&gt;', '"': '&quot;', "'": '&#039;' }[character] || character)); }
function plantUiFreshness(p) {
...
function plantFreshnessInline(p) {
...
var ZentridPlantPager = window.ZentridPlantPager || (window.ZentridPlantPager = { page: 1, size: 50 });
function plantPageSlice(list) {
...
function plantPagerHtml(state) {
    if (state.total <= ZentridPlantPager.size)
...
function assetClientRecords() {
    const rows = Array.isArray(window.ZentridLiveClients) ? window.ZentridLiveClients : [];
    return rows.map((row) => ({
...
const assetClientOptions = [];
const defaultAssetClient = { id: '', name: 'No API client selected', tenant: currentTenantScope(), country: '—', region: '—', city: '—', timezone: '—', contact: '—' };
function replaceAssetClientOptions(rows) {
    assetClientOptions.splice(0, assetClientOptions.length, ...rows);
}
function plantCreateResponseRecord(value) {
...
function plantCreateBackendId(value) {
...
function plantCreateText(value) {
    return typeof value === 'string' ? value.trim() : '';
}
function plantCreateNumber(value) {
...
function plantCreateVendorPayload(form, formData) {
    ...
    for (const [key, rawValue] of formData.entries()) {
        ...
        if (rawValue instanceof File) {
            if (rawValue.size > 0)
                vendorPayload[`has${key.charAt(0).toUpperCase()}${key.slice(1)}File`] = true;
            continue;
        }
        ...
        if (existing === undefined)
            vendorPayload[key] = value;
        else if (Array.isArray(existing))
            existing.push(value);
        else
            vendorPayload[key] = [existing, value];
    }
    for (const input of Array.from(form.querySelectorAll('input[type="file"]'))) {
        if (input.name && input.files?.length)
            vendorPayload[`has${input.name.charAt(0).toUpperCase()}${input.name.slice(1)}File`] = true;
    }
    return vendorPayload;
}
function plantCreateApiPayload(form, formData, client, type, isSolar) {
    ...
    if (capacityAcMw !== null)
        optional('installedCapacityAcMw', capacityAcMw);
    if (gridCapacityMw !== null)
        optional('gridConnectionCapacityMw', gridCapacityMw);
    if (modulesCount !== null)
        optional('modulesCount', modulesCount);
    if (batteryCapacityKwh !== null)
        optional('batteryCapacityKwh', batteryCapacityKwh);
    optional('vendorPayload', plantCreateVendorPayload(form, formData));
    return payload;
}
const sourceSchemes = ['Huawei FusionSolar', 'SolisCloud', 'GoodWe SEMS', 'Sungrow iSolarCloud', 'SolaX Cloud', 'Deye / Solarman', 'Sofar', 'Peimar', 'Other / Manual'];
const vendorFlowConfig = {
    'Huawei FusionSolar': {
...
const defaultVendorFlow = vendorFlowConfig['Other / Manual'] ?? { hint: 'Manual vendor flow', steps: [] };
function vendorFieldControl(field) {
    ...
    if (field.type === 'html')
        return `<div class="vendor-html-field"${cond}>${field.html || ''}</div>`;
    ...
    if (field.type === 'select')
        return `<label${cond}>${label}<select name="${name}"${id}${req}>${(field.options || []).map((x) => `<option>${x}</option>`).join('')}</select></label>`;
    if (field.type === 'textarea')
        return `<label${cond}>${label}<textarea name="${name}"${id}${req}${placeholder}></textarea></label>`;
    if (field.type === 'checkbox')
        return `<label class="checkbox-label checkbox-inline"${cond}><input name="${name}"${id} type="checkbox"><span>${label}</span></label>`;
    if (field.type === 'file')
        return `<label class="file-field"${cond}>${label}<input name="${name}"${id} type="file"${req}></label>`;
    if (field.type === 'readonly-note')
        return `<label class="readonly-note-field"${cond}>${label}<span>${field.value || ''}</span></label>`;
    if (field.type === 'action')
        return `<label class="readonly-note-field action-note-field"${cond}>${label}<button type="button" class="secondary-btn">${field.value || label}</button></label>`;
    return `<label${cond}>${label}<input name="${name}"${id} type="${field.type || 'text'}"${req}${ro}${val}${placeholder}></label>`;
}
function renderVendorStep(step) {
...
function plantEventTarget(event) {
    return event.target instanceof HTMLElement ? event.target : null;
}
function wirePlants() {
    const table = document.getElementById('plantTable');
    const search = document.getElementById('plantSearch');
    const status = document.getElementById('plantStatusFilter');
    const vendor = document.getElementById('plantVendorFilter');
    function apply(resetPage = true) {
        if (!table || !search || !status || !vendor)
            return;
        if (resetPage && !window.FleetRegistryQuery?.pagination('plants'))
            ZentridPlantPager.page = 1;
        ...
        if (scope)
            scope.innerHTML = window.FleetRegistryQuery?.filterScopeHtml('plants') || '';
    }
    search?.addEventListener('input', () => FleetRuntimeStability.debounce('registry:plants:search', () => apply(true), 220));
    status?.addEventListener('change', () => apply(true));
    vendor?.addEventListener('change', () => apply(true));
    table?.addEventListener('click', e => {
        const target = plantEventTarget(e);
        const pageBtn = target?.closest('[data-plant-page]');
        if (pageBtn && !window.FleetRegistryQuery?.pagination('plants')) {
            ZentridPlantPager.page += pageBtn.dataset.plantPage === 'next' ? 1 : -1;
            apply(false);
            return;
        }
        ...
        if (id)
            localStorage.setItem('zentrid_selected_plant', id);
        ...
        if (btn.dataset.action === 'edit') {
            localStorage.setItem('zentrid_plant_detail_edit', 'overview');
            location.href = detailUrl;
        }
        if (btn.dataset.action === 'devices') {
            localStorage.setItem('zentrid_device_filter_plant', id);
            FleetLayout.toast('Opening Device Registry for selected plant');
            location.href = FleetLayout.pathFor('devices');
        }
        if (btn.dataset.action === 'telemetry' && p) {
            localStorage.setItem('zentrid_telemetry_context', JSON.stringify({ tenant: p.tenant, plant: p.name, device: 'All Devices', metric: 'Current Power', range: localStorage.getItem('zentrid_time') || 'Last 24h', layer: 'Normalized' }));
            location.href = FleetLayout.pathFor('telemetry');
        }
        if (btn.dataset.action === 'alerts' && p) {
            localStorage.setItem('zentrid_alert_context', JSON.stringify({ plantId: p.id, tenant: p.tenant }));
            location.href = FleetLayout.pathFor('alerts');
        }
    });
    const plantCreateForm = document.getElementById('plantCreateForm');
    ...
    function queryCreateContext() { return new URLSearchParams(window.location.search); }
    function applyCreateContext() {
        ...
        const select = document.getElementById('assetClientSelect');
        if (!select)
            return;
        ...
        if (match)
            select.value = String(match.id || match.name);
    ...
    function refreshAssetClientSelect() {
        const select = document.getElementById('assetClientSelect');
        if (!select)
            return;
    ...
    async function loadAssetClientsFromApi() {
    ...
    function resetPlantCreateForm() {
        ...
        if (submit)
            FleetFormUX.setBusy(submit, false);
    }
    function plantCreateDirty() {
    ...
    function closePlantCreateModal(force = false) {
        const modal = document.getElementById('plantCreateModal');
        if (!modal || plantCreateSaving)
            return;
        if (!force && plantCreateDirty() && !window.confirm('Close Create Plant? Unsaved wizard data will be lost.'))
            return;
    ...
    document.getElementById('plantCreateModal')?.addEventListener('click', e => { if (plantEventTarget(e)?.id === 'plantCreateModal')
        closePlantCreateModal(); });
    const allStepButtons = Array.from(document.querySelectorAll('[data-asset-step]'));
    const baseStepKeys = ['client', 'plant-profile'];
    ...
    function flowStepKeys() {
        if (!isSolarSelected())
            return ['plant-profile', 'client', 'vendor-basic', 'review'];
    ...
    function setConditionVisible(name, visible) {
    ...
    function applyVendorConditionals() {
        ...
        if (scheme === 'Huawei FusionSolar') {
            ...
            if (hideString && activeStep === 'vendor-string')
                showAssetStep('vendor-more');
        }
    ...
    function renderVendorFlow() {
        ...
        flow.steps.forEach(step => {
            ...
            if (host) {
                host.innerHTML = renderVendorStep(step);
                host.hidden = false;
            }
        });
        ...
        if (note)
            note.innerHTML = `<strong>Source scheme: ${selectedSourceScheme()}</strong><small>${flow.hint}</small>`;
        bindVendorFlowControls();
        applyVendorConditionals();
    }
    function plantNameControl() {
        return plantCreateForm?.querySelector('input[name="name"], input[name="genericName"]') || null;
    }
    function plantDuplicateIssues() {
    ...
    function validateAssetStep(step, focus = true) {
        ...
        const button = document.querySelector(`[data-asset-step="${step}"]`);
        button?.classList.toggle('has-error', !result.valid);
        if (!result.valid && focus)
            FleetFormUX.focusFirst(result, plantValidationSummary);
        return result.valid;
    }
    function validateAssetRange(targetStep) {
        ...
        for (let index = currentIndex; index < targetIndex; index += 1) {
            ...
            if (step && !validateAssetStep(step))
                return false;
            if (step)
                document.querySelector(`[data-asset-step="${step}"]`)?.classList.add('completed');
        }
        return true;
    }
    function validateAllAssetSteps() {
        const steps = visibleSteps().filter(step => step !== 'review');
        for (const step of steps) {
            if (!validateAssetStep(step, false)) {
                showAssetStep(step);
                validateAssetStep(step, true);
                return false;
            }
        }
        if (!validateAssetStep('review', true)) {
            showAssetStep('review');
            return false;
        }
        return true;
    }
    function showAssetStep(step) {
        const allowed = flowStepKeys().filter(key => !document.querySelector(`[data-asset-step="${key}"]`)?.hidden);
        if (!allowed.includes(step))
            step = allowed[0] || 'client';
        activeStep = step;
        ...
        if (progress)
            progress.textContent = `Step ${Math.max(ix + 1, 1)} of ${allowed.length}`;
        updateAssetReview();
    }
    ...
    function syncClientDefaults() {
        ...
        Object.entries(map).forEach(([id, val]) => { const el = document.getElementById(id); if (el && !el.value)
            el.value = val; if (el?.readOnly)
            el.value = val; });
        updateAssetReview();
    }
    ...
    function updateAssetReview() {
        ...
        const rType = document.getElementById('reviewAssetType');
        if (rType)
            rType.textContent = type;
        const rClient = document.getElementById('reviewClient');
        if (rClient)
            rClient.textContent = c.name;
        const rTenant = document.getElementById('reviewTenant');
        if (rTenant)
            rTenant.textContent = c.tenant;
        const rScheme = document.getElementById('reviewSourceScheme');
        if (rScheme)
            rScheme.textContent = selectedSourceScheme();
        const rAllowed = document.getElementById('reviewVendorAllowed');
        if (rAllowed)
            rAllowed.textContent = selectedFlow().steps.map(s => s.label).join(' · ');
        const formData = plantCreateForm ? new FormData(plantCreateForm) : null;
        const rName = document.getElementById('reviewPlantName');
        if (rName)
            rName.textContent = String(formData?.get('name') || formData?.get('genericName') || 'Not entered');
        const rLocation = document.getElementById('reviewPlantLocation');
        if (rLocation)
            rLocation.textContent = [formData?.get('countryManual') || c.country, formData?.get('city') || c.city, formData?.get('address')].filter(Boolean).join(' · ') || 'Location pending';
    }
    allStepButtons.forEach(b => b.addEventListener('click', () => {
        const targetStep = b.dataset.assetStep || 'client';
        if (validateAssetRange(targetStep))
            showAssetStep(targetStep);
    }));
    document.getElementById('nextAssetStep')?.addEventListener('click', () => {
        if (!validateAssetStep(activeStep))
            return;
    ...
    document.getElementById('assetTypeSelect')?.addEventListener('change', () => {
        const sourceScheme = document.getElementById('assetSourceScheme');
        if (!isSolarSelected() && sourceScheme)
            sourceScheme.value = 'Other / Manual';
        renderVendorFlow();
        showAssetStep(activeStep);
        updateAssetReview();
    });
    ...
    if (queryCreateContext().get('create') === '1')
        window.setTimeout(openPlantCreateModal, 0);
    document.addEventListener('keydown', event => { if (event.key === 'Escape' && document.getElementById('plantCreateModal')?.classList.contains('open'))
        closePlantCreateModal(); });
    document.getElementById('plantCreateForm')?.addEventListener('submit', async (e) => {
        e.preventDefault();
        if (!FleetActionPermissions.guard({ action: 'create', resource: 'plant' }))
            return;
        if (plantCreateSaving || !validateAllAssetSteps())
            return;
        ...
        if (submitButton)
            FleetFormUX.setBusy(submitButton, true, 'Creating Plant…');
        try {
            if (!window.FleetAPIMutations)
                throw new Error('Plant mutation runtime is unavailable.');
            const result = await FleetAPIMutations.plants.create(payload);
            if (!result.ok) {
                plantCreateSaving = false;
                if (submitButton)
                    FleetFormUX.setBusy(submitButton, false);
...
function plantTabLabel(tab) { return ({ overview: 'Overview', master: 'Master Data', structure: 'Structure & Devices', energy: 'Energy & Alerts', commercial: 'Commercial', documents: 'Documents', parties: 'Parties & Users', audit: 'Audit', source: 'Source & Sync' })[tab] || 'Plant Detail'; }
function wirePlantDetail() {
    ...
    document.querySelectorAll('[data-plant-tab]').forEach(b => b.onclick = () => {
        ...
        if (title)
            title.textContent = plantTabLabel(tab);
        const detailContent = document.getElementById('plantDetailContent');
        if (detailContent)
            detailContent.innerHTML = plantTab(p, tab);
    });
}
