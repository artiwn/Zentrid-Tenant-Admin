...
(function () {
    ...
    let activeResource = 'unknown';
    let timerId = null;
    let tickerId = null;
    function inferConfig(pathname = location.pathname) {
    ...
    function normalizedAutoRefresh(value) {
        const parsed = Number(value);
        return AUTO_OPTIONS.includes(parsed) ? parsed : 0;
    }
    function autoRefreshFor(resource) {
        const raw = storage()?.getItem(`${STORAGE_PREFIX}${resource}`);
        return normalizedAutoRefresh(raw);
    }
    function storeAutoRefresh(resource, intervalMs) {
        const safe = normalizedAutoRefresh(intervalMs);
        try {
            if (safe)
                storage()?.setItem(`${STORAGE_PREFIX}${resource}`, String(safe));
            else
                storage()?.removeItem(`${STORAGE_PREFIX}${resource}`);
        }
        catch (_error) {
            // Storage failure must not block refresh controls.
        }
    }
    function currentSnapshot(resource = activeResource) {
    ...
    function recordValueAtPath(record, path) {
        return path.split('.').reduce((current, key) => recordObject(current)[key], record);
    }
    function firstRecordTimestamp(record, paths) {
    ...
    function firstRecordText(record, paths) {
        for (const path of paths) {
            const raw = recordValueAtPath(record, path);
            if (raw === undefined || raw === null)
                continue;
    ...
    function compactRecordAge(ageMs) {
        if (ageMs === null)
            return 'Timestamp unavailable';
        if (ageMs < 60_000)
            return `${Math.max(1, Math.floor(ageMs / 1_000))} sec ago`;
        if (ageMs < 3_600_000)
            return `${Math.max(1, Math.floor(ageMs / 60_000))} min ago`;
        if (ageMs < 86_400_000)
            return `${Math.max(1, Math.floor(ageMs / 3_600_000))} hr ago`;
        return `${Math.max(1, Math.floor(ageMs / 86_400_000))} d ago`;
    }
    function recordFreshnessPaths(kind) {
        if (kind === 'plant')
            return [
    ...
    function evaluateRecordFreshness(value, kind = 'generic') {
        ...
        const status = ageMs <= hour ? 'fresh' : ageMs <= day ? 'stale' : 'very-stale';
    ...
    function relativeAge(ageMs) {
        ...
        if (ageMs < 60_000)
            return `Updated ${Math.max(1, Math.floor(ageMs / 1_000))} sec ago`;
        if (ageMs < 3_600_000)
            return `Updated ${Math.max(1, Math.floor(ageMs / 60_000))} min ago`;
    ...
    function autoLabel(intervalMs) {
        if (!intervalMs)
            return 'Off';
        if (intervalMs < 60_000)
            return `${Math.round(intervalMs / 1_000)} sec`;
        return `${Math.round(intervalMs / 60_000)} min`;
    }
    function derivedStatus(input, previous) {
        if (input.status)
            return input.status;
        const detailText = `${input.message || ''} ${input.details || ''}`.toLowerCase();
        if (input.liveState === 'loading')
            return 'refreshing';
        if (input.liveState === 'live' || input.liveState === 'empty') {
            if (detailText.includes('cache') || detailText.includes('saved page'))
                return 'cached';
            return 'live';
        }
        ...
        if (input.liveState === 'fallback')
            return previous.updatedAt ? 'stale' : 'partial';
    ...
    function effectiveStatus(snapshot) {
        if (!snapshot.online)
            return snapshot.updatedAt ? 'stale' : 'unavailable';
        ...
        if ((snapshot.status === 'live' || snapshot.status === 'cached') && ageMs !== null && ageMs > config.staleAfterMs)
            return 'stale';
        return snapshot.status;
    }
    function setBannerState(status) {
        const banner = document.querySelector('.live-data-state');
        if (!banner)
            return;
    ...
    function controlsRoot() {
        ...
        if (!banner)
            return null;
        let root = banner.querySelector('.fleet-freshness-controls');
        if (!root) {
    ...
    function requestRefresh(reason) {
        ...
        if (snapshot.refreshing || navigator.onLine === false)
            return;
    ...
    function scheduleAutoRefresh() {
        clearTimer();
        const snapshot = currentSnapshot();
        if (!snapshot.autoRefreshMs || snapshot.refreshing || navigator.onLine === false || document.visibilityState === 'hidden')
            return;
    ...
    function markRefreshComplete(success = true) {
        const snapshot = currentSnapshot();
        snapshot.refreshing = false;
        if (!success && snapshot.status === 'refreshing')
            snapshot.status = snapshot.updatedAt ? 'stale' : 'unavailable';
        stateByResource.set(snapshot.resource, snapshot);
        render();
    }
    ...
    document.addEventListener('click', event => {
        ...
        if (target?.closest('[data-freshness-refresh]'))
            requestRefresh('manual');
        if (target?.closest('[data-freshness-retry]'))
            requestRefresh('retry');
    });
    document.addEventListener('change', event => {
        const target = event.target instanceof HTMLSelectElement ? event.target : null;
        if (target && typeof target.matches === 'function' && target.matches('[data-freshness-auto]'))
            setAutoRefresh(Number(target.value));
    });
    ...
    const api = {
        ...
        inferResource() { return inferConfig().resource; },
