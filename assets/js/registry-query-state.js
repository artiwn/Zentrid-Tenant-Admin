...
(function () {
    ...
    const paginationByEntity = new Map();
    ...
    function update(entity, patch, options = {}) {
    ...
    function setPagination(entity, pagination) {
    ...
    function activeCurrentPageFilters(entity) {
    ...
    function pagerHtml(entity, fallbackTotal = 0) {
    ...
    function filterScopeHtml(entity) {
    ...
    function requestedPage(entity, action) {
        ...
        if (action === 'prev')
            return Math.max(1, current.page - 1);
        if (action === 'next')
            return Math.min(current.totalPages, current.page + 1);
        return current.page;
    }
    document.addEventListener('click', event => {
        const target = event.target instanceof Element ? event.target : null;
        const button = target?.closest('[data-registry-page][data-registry-entity]');
        if (!button || button.hasAttribute('disabled'))
            return;
        const entity = button.dataset.registryEntity;
    ...
    document.addEventListener('change', event => {
        ...
        if (!target)
            return;
        const pageSizeEntity = target.dataset.registryPageSize;
        if (pageSizeEntity) {
            update(pageSizeEntity, { page: 1, pageSize: normalizePageSize(target.value) }, { replace: false, emit: true });
            return;
        }
        const jumpEntity = target.dataset.registryPageJump;
        if (jumpEntity) {
