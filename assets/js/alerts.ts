...
function normalizedAlertTenantToken(value: unknown): string {
  return String(value || '').trim().toLowerCase().replace(/[^a-z0-9]+/g, '');
}
function alertBelongsToTenant(alert: FleetAlertRecord, tenantName = currentAlertTenantScope()): boolean {
  if (!alert.tenant) return true;
  return normalizedAlertTenantToken(alert.tenant) === normalizedAlertTenantToken(tenantName);
}
function normalizeAlertTenant(alert: FleetAlertRecord, tenantName = currentAlertTenantScope()): FleetAlertRecord {
  const sourceLabel = String(alert.integration || alert.source || alert.vendor || 'Vendor source').replace(/^.*?\s—\s/, '');
  const normalized: FleetAlertRecord = {
    ...alert,
    tenant: tenantName,
...
function alertRegistryRecords(): FleetAlertRecord[] {
  ...
  const liveRows = Array.isArray(window.ZentridLiveAlerts)
    ? (window.ZentridLiveAlerts as FleetAlertRecord[]).filter(alert => alert?.dataOrigin === 'live')
    : [];
...
function getAlertContext(): AlertContextState {
  try {
    const context = JSON.parse(localStorage.getItem('zentrid_alert_context') || '{}') as AlertContextState;
...
function setAlertContextFromQuery(): void {
  ...
  (['plantId', 'deviceId', 'status', 'severity'] as const).forEach(k => {
...
function filteredAlerts(): FleetAlertRecord[] {
  ...
  return tenantScopedAlerts().filter(a =>
...
function renderAlertContextBanner(): string {
  const ctx = getAlertContext();
  if (!ctx.plantId && !ctx.deviceId) return '';
  const parts: string[] = [];
...
function selectedAlert(): FleetAlertRecord {
  ...
  if (selectedId) {
    ...
    if (!selected) throw new Error(`No API alert matches the selected ID: ${selectedId}`);
    return selected;
  }
  const firstAlert = scoped[0];
  if (!firstAlert) throw new Error('No API alerts are available in the current tenant scope.');
  return firstAlert;
}
...
function wireAlertsPage(): void {
  document.querySelector('.main-content')?.addEventListener('click', (e: Event) => {
    ...
    if (target.closest('#resetAlertFilters')) {
      ['severityFilter','statusFilter','plantFilter','vendorFilter'].forEach(id => { const el = document.getElementById(id); if (el) el.value = 'All'; });
...
function alertDetailEmptyState(title: string, detail: string): string {
  return `<div class="empty-state"><strong>${title}</strong><small>${detail}</small></div>`;
}

function alertRawRecord(a: FleetAlertRecord): Record<string, unknown> {
  return a.raw && typeof a.raw === 'object' ? a.raw : {};
}

function alertNestedValue(value: unknown, path: string): unknown {
...
function alertApiValue(a: FleetAlertRecord, paths: string[], fallback = '—'): string {
...
function alertApiArray(a: FleetAlertRecord, paths: string[]): unknown[] {
  const raw = alertRawRecord(a);
  for (const path of paths) {
    const value = alertNestedValue(raw, path);
...
function wireAlertDetailPage(): void {
  let alert: FleetAlertRecord;
  try { alert = selectedAlert(); } catch { return; }
  ...
  document.querySelectorAll<HTMLElement>('.alert-detail-nav-v71 button').forEach(button => {
    button.onclick = () => {
      ...
      if (content) content.innerHTML = alertDetailTab(alert, button.dataset.tab || 'summary');
...
function bindAlertDetailActions(a: FleetAlertRecord): void {
  ...
  if (openPlant) {
    (openPlant as HTMLButtonElement).disabled = !a.plantId || a.plantId === '—';
    openPlant.onclick = () => {
      if (!a.plantId || a.plantId === '—') return;
  ...
  [openDevice, heroDevice].forEach(button => {
    if (!button) return;
    (button as HTMLButtonElement).disabled = !a.deviceId || a.deviceId === '—';
    button.onclick = () => {
      if (!a.deviceId || a.deviceId === '—') return;
  ...
  if (telemetry) {
    ...
    (telemetry as HTMLButtonElement).disabled = !metric || metric === '—';
    telemetry.onclick = () => {
...
function alertIncidentCaseBlock(a: FleetAlertRecord): string {
  ...
  if (caseId === '—') return alertDetailEmptyState('No incident case returned', 'The current alert API response does not include a linked incident case, task or work order.');
...
function alertCaseTimeline(a: FleetAlertRecord): string {
  ...
  if (!rows.length) return alertDetailEmptyState('No case timeline returned', 'The current alert API response does not include incident workflow history.');
  return `<div class="incident-timeline">${rows.map((row,index) => {
    const item = row && typeof row === 'object' ? row as Record<string, unknown> : { description: row };
...
function alertSopChecklistBlock(a: FleetAlertRecord): string {
  ...
  if (!steps.length) return `<section class="alert-sop-card glass-card">${alertDetailEmptyState('No SOP checklist returned', 'The current alert API response does not include a procedure or checklist. No browser-local SOP progress is created.')}</section>`;
  return `<section class="alert-sop-card glass-card"><div class="sop-head"><div><span class="eyebrow">SOP Checklist</span><h2>${alertApiValue(a,['sopTitle','procedureName','vendorExtensions.sopTitle'],'Backend procedure')}</h2><p class="muted">Read-only checklist returned by the backend.</p></div></div><div class="sop-checklist">${steps.map((step,index) => {
    const item = step && typeof step === 'object' ? step as Record<string, unknown> : { label: step };
...
function alertCurveBlock(_a: FleetAlertRecord, m: AlertDetailModel): string {
...
function renderAlertDetailUnavailable(message = 'The selected alert could not be loaded from the backend API.'): string {
