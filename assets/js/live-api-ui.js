...
(function () {
    ...
    let liveLoadingWatchdog = null;
    const registryRequestVersions = new Map();
    function isRegistryPage(entity) {
        return location.pathname.endsWith(`/${entity}.html`) || location.pathname.endsWith(`${entity}.html`);
    }
    function registryReadOptions(entity, forceRefresh = false) {
        ...
        if (entity === 'alerts') {
            try {
                const stored = JSON.parse(localStorage.getItem('zentrid_alert_context') || '{}');
    ...
    function detailReadOptions(entity, pageSize = 100, forceRefresh = false) {
        return {
            page: 1,
            pageSize,
            staleWhileRevalidate: true,
    ...
    function cacheAgeLabel(ageMs) {
        if (ageMs < 1_000)
            return 'just now';
        if (ageMs < 60_000)
            return `${Math.max(1, Math.round(ageMs / 1_000))} sec ago`;
        return `${Math.max(1, Math.round(ageMs / 60_000))} min ago`;
    }
    function repositoryCachePresentation(result) {
        const cache = result.cache;
        if (!cache)
            return { state: 'live', prefix: '', details: '', freshnessStatus: 'live' };
    ...
    function cacheFreshnessOptions(cacheInfo) {
        return {
    ...
    function publishRegistryPagination(entity, result) {
    ...
    function requestErrorShape(error) {
    ...
    function liveErrorState(error) {
        const shaped = requestErrorShape(error);
        const status = Number(shaped.status || 0);
        const code = String(shaped.code || '').toUpperCase();
        if (code === 'TIMEOUT')
            return 'timeout';
        if (status === 401 || code === 'SESSION_EXPIRED' || code === 'NO_REFRESH_TOKEN')
            return 'unauthorized';
        if (status === 403)
            return 'forbidden';
        return 'unavailable';
    }
    const LIVE_STATE_TITLES = {
    ...
    function renderedDataOrigin() {
        const chips = Array.from(document.querySelectorAll('.record-origin-chip[data-record-origin]'))
            .filter(chip => !chip.closest('.data-source-summary'));
        const origins = new Set();
        for (const chip of chips) {
            const origin = chip.dataset.recordOrigin;
            if (origin === 'live' || origin === 'mock' || origin === 'local' || origin === 'mixed')
                origins.add(origin);
        }
        if (origins.size > 1)
            return 'mixed';
    ...
    function removeDataSourceSummary() {
        document.querySelector('.data-source-summary')?.remove();
    }
    function setDataSourceSummary(origin, options = {}) {
        const main = document.querySelector('.main-content');
        if (!main || !window.FleetDataSource)
            return;
        let summary = main.querySelector('.data-source-summary');
        if (!summary) {
            summary = document.createElement('section');
            summary.className = 'data-source-summary';
            const stateBanner = main.querySelector('.live-data-state');
            if (stateBanner)
                stateBanner.insertAdjacentElement('afterend', summary);
            else {
        ...
        ['live', 'mock', 'local', 'mixed'].forEach(value => {
    ...
    function removeContractDiagnostics() {
        document.querySelector('.contract-diagnostics')?.remove();
    }
    function contractIssueTitle(issue) {
        return `${issue.entityLabel} #${issue.index + 1} · ${issue.field}`;
    }
    function syncContractDiagnostics(state) {
        if (state === 'loading') {
            contractDiagnosticsApi()?.clear();
        ...
        let panel = main.querySelector('.contract-diagnostics');
        if (!panel) {
            ...
            const stateBanner = main.querySelector('.live-data-state');
            if (sourceSummary)
                sourceSummary.insertAdjacentElement('afterend', panel);
            else if (stateBanner)
                stateBanner.insertAdjacentElement('afterend', panel);
            else
                main.prepend(panel);
        }
    ...
    function syncDataSourceForState(state, options) {
        if (state === 'loading') {
    ...
    function setLiveDataState(state, message, options = {}) {
        const main = document.querySelector('.main-content');
        if (!main)
            return;
        ...
        if (state === 'loading') {
            liveLoadingWatchdog = window.setTimeout(() => {
                liveLoadingWatchdog = null;
                const current = document.querySelector('.live-data-state[data-live-state="loading"]');
                if (!current)
                    return;
        ...
        let banner = main.querySelector('.live-data-state');
        if (!banner) {
    ...
    function setRequestFailure(endpoint, error, fallbackMessage) {
    ...
    function badge(status) {
        ...
        if (value.includes('ok') || value.includes('active') || value.includes('online') || value.includes('normal'))
            return 'success';
        if (value.includes('fail') || value.includes('fault') || value.includes('offline') || value.includes('critical'))
            return 'danger';
        if (value.includes('warn') || value.includes('unknown') || value.includes('stale') || value.includes('delayed'))
            return 'warning';
        return 'neutral';
    }
    function insertIntegrationLiveSummary(items = []) {
      ...
      <article>
        ...
        let section = document.querySelector('.integration-live-summary');
        if (!section) {
            ...
            if (context && context.nextSibling)
                main.insertBefore(section, context.nextSibling);
            else
    ...
    function compactNumber(value, suffix = '') {
        ...
        if (Math.abs(n) >= 1_000_000)
            return `${(n / 1_000_000).toFixed(1)}M${suffix}`;
        if (Math.abs(n) >= 1_000)
            return `${(n / 1_000).toFixed(1)}k${suffix}`;
        return `${n}${suffix}`;
    }
    function knownCollectionTotal(totalCount, rows) {
        if (typeof totalCount !== 'number' || !Number.isFinite(totalCount) || totalCount < 0)
            return null;
        return Math.max(totalCount, rows.length);
    }
    function overviewCoverageLabel(loaded, totalCount, noun) {
        if (totalCount === null)
            return `${loaded} ${noun} row(s) loaded`;
        if (loaded >= totalCount)
            return `All ${compactNumber(totalCount)} ${noun} row(s) loaded`;
        return `Page sample · ${loaded} of ${compactNumber(totalCount)} ${noun}`;
    }
    function uniqueOverviewStrings(values) {
        const seen = new Set();
        const output = [];
    ...
    function overviewProviderNames(providers, integrations) {
        const providerRows = providers.map(value => {
            if (!value || typeof value !== 'object')
                return value;
            return firstOf(value, ['displayName', 'providerName', 'provider', 'name', 'vendor', 'providerType'], '');
        });
    ...
    function overviewStatusBucket(value) {
        const status = safeText(value, 'Unknown').trim().toLowerCase();
        if (/offline|disconnected|inactive|unavailable/.test(status))
            return 'Offline';
        if (/critical|fault|failed|failure|error/.test(status))
            return 'Fault';
        if (/warning|warn|delayed|stale|degraded|attention/.test(status))
            return 'Warning';
        if (/normal|healthy|active|online|ok|operational|available/.test(status))
            return 'Normal';
        return 'Unknown';
    }
    function overviewFleetHealth(plants) {
        if (!plants.length)
            return [];
    ...
    function snapshotFromLive({ plants = [], devices = [], alerts = [], integrations = [], providers = [], templates = [], plantTotalCount = null, deviceTotalCount = null, alertTotalCount = null, integrationTotalCount = null }) {
    ...
    function applyOverviewDataFromLive(payload) {
        const store = window.TenantOverviewData;
        if (!store)
            return;
    ...
    function plantMatchesDevice(plant, device) {
    ...
    function plantMatchesAlert(plant, alert) {
    ...
    function deviceMatchesAlert(device, alert) {
    ...
    function plantMatchesTelemetry(plant, telemetry) {
    ...
    function publishPlantDetailTelemetry(record, rows) {
        ...
        if (!keys.length)
            return;
        const telemetryStore = { ...(window.ZentridLiveTelemetryByPlant || {}) };
        const loadedStore = { ...(window.ZentridLiveTelemetryLoadedPlants || {}) };
    ...
    function mergePlantDetailRows(rows) {
        const output = [];
        const seen = new Set();
    ...
    function mappedPlantDeviceRows(payload) {
        return FleetAPIContracts.devices.mapList(asArray(payload), contractMapperContext);
    }
    async function loadTenantPlantDeviceRelations(plant, forceRefresh) {
        const errors = [];
        const sources = [];
        const rows = [];
    ...
    async function loadTenantPlantAlertRelations(plant, forceRefresh) {
    ...
    function enrichPlantRelations(plants, devices, alerts) {
        return plants.map(plant => {
            ...
            const typeCounts = relatedDevices.reduce((acc, device) => {
                ...
                if (key.includes('invert'))
                    ...
                    acc.meters += 1;
                else if (key.includes('logger') || key.includes('collector') || key.includes('gateway'))
                    acc.loggers += 1;
                else
                    acc.other += 1;
                return acc;
    ...
    function enrichDeviceRelations(devices, plants, alerts) {
    ...
    function liveTable(title, subtitle, columns, rows, emptyText) {
    ...
    function relatedDevicesTable(devices, plant) {
    ...
    function relatedAlertsTable(alerts, contextLabel) {
    ...
    function renderOverviewLiveSnapshot(payload) {
    ...
    async function applyOverview(forceRefresh = false) {
        ...
        const updateState = () => {
            ...
            const state = pending.size || errors.length ? 'partial' : 'live';
        ...
        try {
            ...
            results.forEach(result => { if (result.status === 'rejected')
                errors.push(result.reason); });
            if (plantsResult.status === 'fulfilled')
                errors.push(...plantsResult.value.errors);
            if (devicesResult.status === 'fulfilled')
                errors.push(...devicesResult.value.errors);
            if (registryResult.status === 'fulfilled')
                errors.push(...registryResult.value.errors);
            ...
            if (!hasCoreSignal) {
                renderEmptyOverview();
                if (errors.length)
                    setRequestFailure('Overview core endpoints', errors[0], 'No prototype fallback is displayed.');
                else
                    setLiveDataState('empty', 'Core endpoints returned no records. The dashboard remains empty while background checks continue.', { source: 'Zentrid Platform APIs', dataOrigin: 'live', recordCount: 0 });
            }
            else {
    ...
    async function applyPlants(backgroundRefresh = false, forceRefresh = false) {
        if (!/plants\.html$/.test(location.pathname))
            return;
        const requestVersion = beginRegistryRequest('plants');
        if (!backgroundRefresh)
            setLiveDataState('loading', 'Loading the requested plant page first. Device and alert relations will be attached in the background.', { source: '/api/plants + /api/admin/plants' });
        try {
            ...
            if (!data.length) {
                ...
                if (live.errors.length)
                    setRequestFailure(live.source, live.errors[0], 'No browser-local or prototype plant records are displayed.');
                else
                    setLiveDataState('empty', 'The requested plant page returned no records.', { source: live.source, recordCount: live.pagination.totalCount });
                return;
            }
            let relatedDevices = [];
            let relatedAlerts = [];
            const relationErrors = [...live.errors];
            ...
            const render = () => {
                ...
                const state = pending.size || relationErrors.length || cacheInfo.state === 'partial' ? 'partial' : 'live';
            ...
            void FleetAPIRepositories.devices.list(detailReadOptions('plant-relations:devices', 100, forceRefresh))
                .then(result => { if (isCurrentRegistryRequest('plants', requestVersion)) {
                relatedDevices = result.items;
                relationErrors.push(...result.errors);
            } })
                .catch(error => { if (isCurrentRegistryRequest('plants', requestVersion))
                relationErrors.push(error); })
                .finally(() => { if (isCurrentRegistryRequest('plants', requestVersion)) {
                pending.delete('devices');
                render();
            } });
    ...
    async function applyDevices(backgroundRefresh = false, forceRefresh = false) {
        if (!/devices\.html$/.test(location.pathname))
            return;
        const requestVersion = beginRegistryRequest('devices');
        if (!backgroundRefresh)
            setLiveDataState('loading', 'Loading the requested device page first. Plant and alert relations will be attached in the background.', { source: '/api/devices' });
        try {
            ...
            void FleetAPIRepositories.plants.list(detailReadOptions('device-relations:plants', 100, forceRefresh))
                .then(result => { if (isCurrentRegistryRequest('devices', requestVersion)) {
                relatedPlants = result.items;
                relationErrors.push(...result.errors);
            } })
                .catch(error => { if (isCurrentRegistryRequest('devices', requestVersion))
                relationErrors.push(error); })
                .finally(() => { if (isCurrentRegistryRequest('devices', requestVersion)) {
                pending.delete('plants');
                render();
            } });
    ...
    async function applyAlerts(backgroundRefresh = false, forceRefresh = false) {
        if (!/alerts\.html$/.test(location.pathname))
            return;
        const requestVersion = beginRegistryRequest('alerts');
        if (!backgroundRefresh)
            setLiveDataState('loading', 'Loading the requested alert page.', { source: '/api/alerts' });
        try {
    ...
    function realNameFromRow(row, entityLabel, typeHint) {
    ...
    function cleanLabelToken(value, fallback) {
    ...
    function isGeneratedLiveName(value) {
        const text = safeText(value, '').trim();
    ...
    function looksLikeTechnicalId(value) {
        ...
        if (/^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i.test(text))
            return true;
        if (/^[A-Z]{2,}[-_][A-Z0-9]{3,}[-_]?\d*$/i.test(text))
            return true;
        if (/^\d{5,}$/.test(text))
            return true;
        return false;
    }
    function collectNameCandidates(row, entityLabel, explicitKeys = []) {
        const result = [];
        explicitKeys.forEach(key => result.push(firstOf(row, [key], '')));
        const allowKey = (key) => {
            const k = key.toLowerCase();
            if (/id$|uuid|guid|status|type|provider|vendor|source|serial|code|capacity|power|energy|date|time|count|number|url|uri|ref/.test(k))
                return false;
            if (entityLabel === 'Plant' && /(plantname|plant_name|stationname|station_name|sitename|site_name|displayname|display_name|name|alias|label|title)/.test(k))
                return true;
            if (entityLabel === 'Device' && /(devicename|device_name|equipmentname|equipment_name|displayname|display_name|name|alias|label|title)/.test(k))
                return true;
            if (/Alert|Alarm/i.test(entityLabel) && /(alertname|alert_name|alarmname|alarm_name|eventname|event_name|displayname|display_name|title|message|name|alias|label)/.test(k))
                return true;
            if (entityLabel === 'Tenant' && /(tenantname|tenant_name|organizationname|organization_name|companyname|company_name|legalname|legal_name|displayname|display_name|name|alias|label|title)/.test(k))
                return true;
            if (entityLabel === 'Client' && /(clientname|client_name|customername|customer_name|companyname|company_name|legalname|legal_name|fullname|full_name|displayname|display_name|name|alias|label|title)/.test(k))
                return true;
            return false;
        };
        const walk = (value, depth) => {
            ...
            Object.entries(value).forEach(([key, child]) => {
                if (allowKey(key) && child !== undefined && child !== null && child !== '')
                    result.push(child);
    ...
    function isUsefulDisplayName(value, row, entityLabel, typeHint) {
        const text = safeText(value, '').trim();
        if (!text || text === '—')
            return false;
        if (isGeneratedLiveName(text) || looksLikeTechnicalId(text))
            return false;
        const typeText = safeText(typeHint, '').trim().toLowerCase();
        if (typeText && text.toLowerCase() === typeText)
            return false;
        const provider = safeText(firstOf(row, ['provider', 'vendor', 'sourceSystem'], ''), '').trim().toLowerCase();
        if (provider && text.toLowerCase() === provider)
            return false;
    ...
    function shortenGeneratedLiveName(value, entityLabel, index, typeHint) {
        const text = safeText(value, '').trim();
        ...
        if (/alert|alarm/i.test(entityLabel) && !/alert|alarm/i.test(kind))
            return `${kind} Alert ${suffix}`;
        return `${kind} ${suffix}`;
    }
    function liveDisplayName(row, keys, entityLabel, index, typeHint) {
    ...
    function upsertLiveRecord(target, record) {
        if (!Array.isArray(target) || !record?.id)
            return;
        const index = target.findIndex(item => item?.id === record.id || item?.externalId === record.externalId);
        if (index >= 0)
            target[index] = { ...target[index], ...record };
        else
            target.unshift(record);
    }
    function liveCapacity(value, unit) {
        const text = safeText(value, '').trim();
        if (!text || text === '—')
            return `0 ${unit}`;
        if (/[a-z]/i.test(text))
    ...
    function ensureLiveClientModelPlant(plant, devices = []) {
        const model = window.FleetClientModel;
        if (!model || !Array.isArray(model.plants) || !Array.isArray(model.devices))
            return;
        const liveClients = Array.isArray(model.clients) ? model.clients.filter(client => client?.dataOrigin === 'live') : [];
        ...
        if (typeof model.selectPlant === 'function')
            model.selectPlant(livePlant.id);
    }
    function syncLiveClientRecords(clients) {
        const model = window.FleetClientModel;
        if (!model || !Array.isArray(model.clients))
            return;
    ...
    function clearLivePlantDetailModel() {
        const model = window.FleetClientModel;
        if (!model)
            return;
        if (Array.isArray(model.plants)) {
            const retained = model.plants.filter(item => item?.dataOrigin !== 'live');
            model.plants.splice(0, model.plants.length, ...retained);
        }
        if (Array.isArray(model.devices)) {
            const retained = model.devices.filter(item => item?.dataOrigin !== 'live' && item?.sourceStatus !== 'Live API');
    ...
    function syncLiveClientModel(plants, devices = []) {
        const model = window.FleetClientModel;
        if (!model || !Array.isArray(model.plants) || !Array.isArray(model.devices))
            return;
        ...
        if (previousPlantId && typeof model.selectPlant === 'function')
            model.selectPlant(previousPlantId);
    }
    function mountExistingRenderer(renderName, wireName) {
        ...
        if (typeof rendered === 'string')
            FleetLayout.mount(rendered);
        if (typeof wirer === 'function')
            wirer();
        return true;
    }
    function liveDetailGrid(row, fields) {
    ...
    function renderLivePlantDetail(plant) {
    ...
    function renderLiveDeviceDetail(device) {
    ...
    function normalizedDeviceSourceKey(value) {
        return String(value ?? '').trim().toLowerCase();
    }
    function normalizedDeviceProvider(value) {
        const key = normalizedDeviceSourceKey(value).replace(/[\s_-]+/g, '');
        if (key === 'deye' || key === 'deyecloud')
            return 'deyecloud';
        if (key === 'solarx' || key === 'solax')
            return 'solax';
        return key;
    }
    function tenantLiveDeviceMatchesRegistry(candidate, device) {
        ...
        if (!sourceDeviceId || !registrySourceDeviceId || sourceDeviceId !== registrySourceDeviceId)
            return false;
    ...
    async function loadTenantLiveDeviceDetail(device, forceRefresh) {
        ...
        if (explicitLiveId) {
            try {
                ...
                if (direct && typeof direct === 'object')
                    return { id: explicitLiveId, detail: direct };
            }
            ...
            try {
                ...
                if (match && liveId) {
                    try {
                        ...
                        return { id: liveId, detail: detail && typeof detail === 'object' ? detail : match };
                    }
                    catch (_error) {
        ...
        if (!sourceDeviceId || sourceDeviceId === '—')
            return null;
        try {
    ...
    function clearLiveDeviceDetailModel() {
        window.ZentridLiveDevices = [];
        window.ZentridLivePlants = [];
        window.ZentridLiveAlerts = [];
    }
    async function applyDeviceDetail(forceRefresh = false) {
        if (!/device-detail\.html$/.test(location.pathname))
            return;
        ...
        try {
            ...
            if (!selectedRecord) {
                ...
                if (deviceResult.errors.length)
                    setRequestFailure(deviceResult.source || '/api/admin/devices', deviceResult.errors[0], 'No Tenant device detail is displayed.');
                else
                    setLiveDataState('empty', selectedId ? 'The Device Registry endpoint returned no record matching the selected ID.' : 'The Device Registry endpoint returned no records.', { source: deviceResult.source || '/api/admin/devices', recordCount: 0 });
                return;
            }
            let plantRows = [];
            let alertRows = [];
            const relationErrors = [...deviceResult.errors];
            const sync = () => {
            ...
            const applyDeviceResource = (field, payload) => {
            ...
            try {
                ...
                if (liveMatch) {
                    ...
                    applyDeviceResource('liveLookupStatus', 'not-linked');
            }
            catch (error) {
                relationErrors.push(error);
            }
            window.FleetDetailLazyTabs?.register('device', [
                {
                    ...
                    loader: async () => {
                        ...
                        relationErrors.push(...result.errors);
                        if (!plantRows.length && result.errors.length)
                ...
                },
                {
                    ...
                    loader: async () => {
                        ...
                        relationErrors.push(...result.errors);
                        if (!alertRows.length && result.errors.length)
                            throw result.errors[0];
                ...
                },
                {
                    ...
                    loader: async () => {
                        if (!selectedAdminDeviceId)
                            throw new Error('A Device Registry id is required for latest telemetry.');
                        const [adminTelemetry, liveTelemetry] = await Promise.all([
                            ...
                            selectedLiveDeviceId ? window.ZentridPlatformAPI?.liveDevices?.telemetryLatest(selectedLiveDeviceId, detailReadOptions('device-detail:live-telemetry-latest', 20, forceRefresh)).catch((error) => { relationErrors.push(error); return null; }) : Promise.resolve(null)
                        ]);
                ...
                },
                {
                    ...
                    loader: async () => {
                        if (!selectedAdminDeviceId)
                            throw new Error('A Device Registry id is required for connectivity.');
                        const [connectivity, network, linked, liveConnectivity, liveNetwork] = await Promise.all([
                            ...
                            selectedLiveDeviceId ? window.ZentridPlatformAPI?.liveDevices?.connectivity(selectedLiveDeviceId, detailReadOptions('device-detail:live-connectivity', 20, forceRefresh)).catch((error) => { relationErrors.push(error); return null; }) : Promise.resolve(null),
                            selectedLiveDeviceId ? window.ZentridPlatformAPI?.liveDevices?.network(selectedLiveDeviceId, detailReadOptions('device-detail:live-network', 20, forceRefresh)).catch((error) => { relationErrors.push(error); return null; }) : Promise.resolve(null)
                        ]);
                ...
                },
                {
                    ...
                    loader: async () => {
                        if (!selectedAdminDeviceId)
                            throw new Error('A Device Registry id is required for warranty.');
                        const [adminWarranty, liveWarranty] = await Promise.all([
                            ...
                            selectedLiveDeviceId ? window.ZentridPlatformAPI?.liveDevices?.warranty(selectedLiveDeviceId, detailReadOptions('device-detail:live-warranty', 20, forceRefresh)).catch((error) => { relationErrors.push(error); return null; }) : Promise.resolve(null)
                        ]);
                ...
                },
                {
                    ...
                    loader: async () => {
                        if (!selectedAdminDeviceId)
                            throw new Error('A Device Registry id is required for audit history.');
            ...
            if (!mountExistingRenderer('renderDeviceDetail', 'wireDeviceDetail'))
                console.warn('Zentrid Tenant: Device Detail renderer was not found.');
    ...
    async function applyPlantDetail(forceRefresh = false) {
        if (!/plant-detail\.html$/.test(location.pathname) || window.__zentridDisableLiveDetailCore)
            return;
        ...
        try {
            const [plantResult, clientResult] = await Promise.all([
                ...
                FleetAPIRepositories.clients.list(detailReadOptions('plant-detail:clients', 100, forceRefresh)).catch(error => ({ entity: 'clients', items: [], rawItems: [], source: '/api/admin/clients', errors: [error], pagination: { page: 1, pageSize: 100, totalCount: 0, totalPages: 1, hasPreviousPage: false, hasNextPage: false } }))
            ]);
            ...
            if (!selectedPlant) {
                ...
                if (plantResult.errors.length)
                    setRequestFailure(plantResult.source, plantResult.errors[0], 'No Tenant plant detail is displayed.');
                else
                    setLiveDataState('empty', 'The Plant Registry endpoint returned no matching record.', { source: plantResult.source, recordCount: 0 });
                return;
            }
            let deviceRows = [];
            let alertRows = [];
            ...
            const relationErrors = [...plantResult.errors, ...clientResult.errors];
            const sync = () => {
                const relationAwareData = data.map(item => ({ ...item, devicesLoaded, alertsLoaded, telemetryLoaded }));
                const mapped = enrichPlantRelations(relationAwareData, deviceRows, alertRows).map(item => ({
                    ...item,
                    devicesLoaded,
                    alertsLoaded,
                    telemetryLoaded,
                ...
                if (plant) {
                    ...
                    if (adminId)
                        localStorage.setItem('zentrid_selected_plant_context', JSON.stringify({ selectedId: safeText(plant.id, selectedId), adminId }));
            ...
            window.FleetDetailLazyTabs?.register('plant', [
                {
                    ...
                    loader: async () => {
                        const plant = sync();
                        if (!plant)
                            throw new Error('The selected plant is not available for device matching.');
                        ...
                        if (!deviceRows.length && relation.errors.length)
                            throw relation.errors[0];
                        sync();
                ...
                },
                {
                    ...
                    loader: async () => {
                        const plant = sync();
                        if (!plant)
                            throw new Error('The selected plant is not available for alert matching.');
                        const relation = await loadTenantPlantAlertRelations(plant, forceRefresh);
                        alertRows = relation.rows;
                        alertsLoaded = true;
                        relationErrors.push(...relation.errors);
                        if (!alertRows.length && relation.errors.length)
                            throw relation.errors[0];
                        sync();
                ...
                },
                {
                    ...
                    loader: async () => {
                        const plant = sync();
                        if (!plant)
                            throw new Error('The selected plant is not available for telemetry matching.');
                        ...
                        telemetryRows = result.items.filter((row) => plantMatchesTelemetry(plant, row));
    ...
    async function applyAlertDetail(forceRefresh = false) {
        if (!/alert-detail\.html$/.test(location.pathname))
            return;
        ...
        try {
            ...
            if (!selectedRecord) {
                ...
                if (result.errors.length)
                    setRequestFailure(result.source || '/api/alerts', result.errors[0], 'No prototype or browser-local alert record is displayed.');
                else
                    setLiveDataState('empty', selectedId ? 'The alert endpoint returned no record matching the selected ID.' : 'The alert endpoint returned no records.', { source: result.source || '/api/alerts', recordCount: 0 });
                return;
            }
            localStorage.setItem('zentrid_selected_alert', String(selectedRecord.id));
            FleetLayout.mount(renderAlertDetailContent(selectedRecord));
            wireAlertDetailPage();
            const cacheInfo = repositoryCachePresentation(result);
    ...
    function clientMatchesPlant(client, plant) {
        if (sameId(plant.clientId, client.id) || sameId(plant.raw?.clientId, client.id) || sameId(plant.raw?.adminRecord?.clientId, client.id))
            return true;
        const assigned = Array.isArray(client.plants) ? client.plants : [];
        if (assigned.some((id) => sameId(id, plant.id) || sameId(id, plant.externalId) || sameId(id, plant.code)))
            return true;
    ...
    async function applyClients(backgroundRefresh = false, forceRefresh = false) {
        if (!/clients\.html$/.test(location.pathname) && !/client-detail\.html$/.test(location.pathname))
            return;
        ...
        if (!backgroundRefresh)
            setLiveDataState('loading', registry ? 'Loading the requested Tenant Admin client page.' : selectedId ? 'Loading the selected Tenant Admin client record.' : 'Loading Tenant Admin client records.', { source: registry ? '/api/admin/clients' : detailSource });
        try {
            ...
            if (registry && !isCurrentRegistryRequest('clients', requestVersion))
                return;
            if (registry)
                publishRegistryPagination('clients', result);
            const data = registry
                ? result.items
                : ('item' in result && result.item ? [result.item] : result.items.slice(0, 1));
            if (!data.length) {
                setLiveClients([]);
                if (registry)
                    renderClientsPage();
                    ...
                    const model = window.FleetClientModel;
                    if (model?.plants)
                        model.plants.splice(0, model.plants.length);
                    if (model?.devices)
                        model.devices.splice(0, model.devices.length);
                    renderClientDetailPage();
                }
                if (result.errors.length)
                    setRequestFailure(detailSource, result.errors[0], 'No prototype or browser-local client record is displayed.');
                else
                    setLiveDataState('empty', selectedId ? 'The selected client endpoint returned no matching record.' : 'The client endpoint returned no records.', { source: detailSource, recordCount: registry ? result.pagination.totalCount : 0 });
                return;
            }
            ...
            if (registry) {
                ...
                if (!data.some(item => item.id === currentSelectedId) && data[0])
                    clientModel?.selectClient(data[0].id);
                renderClientsPage();
            }
            else {
                if (selected?.id)
                    clientModel?.selectClient(selected.id);
                ...
                if (clientModel?.plants)
                    clientModel.plants.splice(0, clientModel.plants.length);
                if (clientModel?.devices)
                    clientModel.devices.splice(0, clientModel.devices.length);
                syncLiveClientModel(enrichedPlants, clientDevices);
                const selectedRecord = clientModel?.clients.find(client => client.id === selected.id);
                if (selectedRecord) {
                ...
                renderClientDetailPage();
                const relationErrors = relationResults.filter((item) => item.status === 'rejected').map(item => item.reason);
    ...
    const repositoryRefreshTimers = new Map();
    function handleRepositoryUpdated(event) {
        const detail = event.detail;
        const entity = detail?.entity;
        if (!entity || detail?.reason !== 'revalidated' || !isRegistryPage(entity))
            return;
        const current = window.FleetRegistryQuery?.read(entity);
        const result = detail.result;
        if (result && current && (result.pagination.page !== current.page || result.pagination.pageSize !== current.pageSize))
            return;
        ...
        repositoryRefreshTimers.set(entity, window.setTimeout(() => {
            ...
            if (entity === 'clients')
                void applyClients(true);
            if (entity === 'plants')
                void applyPlants(true);
            if (entity === 'devices')
                void applyDevices(true);
            if (entity === 'alerts')
                void applyAlerts(true);
        }, 40));
    }
    function handleRegistryQueryChange(event) {
        const detail = event.detail;
        const entity = detail?.entity;
        if (!entity || !isRegistryPage(entity))
            return;
        if (entity === 'clients')
            void applyClients();
        if (entity === 'plants')
    ...
    function handleDataRefreshRequest(event) {
        const detail = event.detail;
        const resource = detail?.resource || window.FleetDataFreshness?.inferResource();
        const forceRefresh = detail?.forceRefresh !== false;
        if (resource === 'overview')
            void applyOverview(forceRefresh);
        if (resource === 'clients')
            void applyClients(true, forceRefresh);
        if (resource === 'client-detail')
            void applyClients(false, forceRefresh);
        if (resource === 'plants')
            void applyPlants(true, forceRefresh);
        if (resource === 'plant-detail')
            void applyPlantDetail(forceRefresh);
        if (resource === 'devices')
            void applyDevices(true, forceRefresh);
        if (resource === 'device-detail')
            void applyDeviceDetail(forceRefresh);
        if (resource === 'alerts')
            void applyAlerts(true, forceRefresh);
        if (resource === 'alert-detail')
            void applyAlertDetail(forceRefresh);
    }
    ...
    async function run() {
        if (!window.ZentridPlatformAPI || !window.FleetAPI || !window.FleetAPIRepositories || !FleetAPIRepositories.isConfigured())
            return;
