type TenantOverviewTone = 'success' | 'warning' | 'danger' | 'info' | string;

type TenantOverviewKpi = {
  icon: string;
  label: string;
  value: string;
  delta: string;
  tone: TenantOverviewTone;
  route: string;
};

type TenantOverviewHealthItem = { label: string; count: number; percent: number };
type TenantOverviewAlertItem = { title: string; plant: string; device: string; severity: string; time: string };
type TenantOverviewIntegrationItem = { name: string; sync: string; status: string; errors: number | string };
type TenantOverviewQualityItem = { label: string; value: string };
type TenantOverviewPlantItem = { name: string; capacity: string; energy: string; health: string };
type TenantOverviewState = { tenant: string; time: string; region: string };

type TenantOverviewDataStore = {
  kpis: TenantOverviewKpi[];
  fleetHealth: TenantOverviewHealthItem[];
  alerts: TenantOverviewAlertItem[];
  integrations: TenantOverviewIntegrationItem[];
  quality: TenantOverviewQualityItem[];
  plants: TenantOverviewPlantItem[];
};

declare const TenantOverviewData: TenantOverviewDataStore;

function tenantOverviewEscape(value: unknown): string {
...
function tenantOverviewElement<T extends HTMLElement = HTMLElement>(element: Element | null): T | null {
  return element instanceof HTMLElement ? element as T : null;
}

function tenantOverviewStatusClass(value: unknown): string {
  const normalized = String(value || '').toLowerCase();
  if (normalized.includes('critical') || normalized.includes('failed') || normalized.includes('fault') || normalized.includes('offline') || normalized.includes('error')) return 'danger';
  if (normalized.includes('high') || normalized.includes('delayed') || normalized.includes('warning') || normalized.includes('stale') || normalized.includes('degraded')) return 'warning';
  if (normalized.includes('info')) return 'info';
  if (normalized.includes('unknown') || normalized === '—') return 'neutral';
  return 'success';
}

function tenantOverviewEmpty(title: string, message: string): string {
...
function tenantOverviewKpis(): string {
...
function tenantOverviewHealthBars(): string {
...
function tenantOverviewAlerts(): string {
...
function tenantOverviewIntegrations(): string {
...
function tenantOverviewQuality(): string {
...
function tenantOverviewPlants(): string {
...
function tenantOverviewRender(state: TenantOverviewState = FleetLayout.state): string {
  return `
...
function tenantOverviewDrawer(title: string, body: string, route: string): void {
...
function tenantOverviewDetail(type: string | undefined, title: string, state: TenantOverviewState): string {
...
const tenantOverviewState: TenantOverviewState = {
...
function wireOverview(): void {
  ...
  if (!main || main.dataset.tenantOverviewWired === 'true') return;
  ...
  main.addEventListener('click', event => {
    ...
    if (drill) {
      tenantOverviewDrawer(
        drill.dataset.title || 'API record',
        tenantOverviewDetail(drill.dataset.panel, drill.dataset.title || 'API record', tenantOverviewState),
        drill.dataset.route || 'overview'
      );
      return;
    }
    ...
    if (!context) return;
    if (context.dataset.context === 'tenant') FleetLayout.toast(`Tenant scope is fixed to ${tenantOverviewState.tenant}.`);
    if (context.dataset.context === 'time') document.getElementById('timeBtn')?.click();
    if (context.dataset.context === 'region') FleetLayout.toast('Region is shown from the current tenant context. API requests remain tenant-scoped.');
  });
}
...
window.addEventListener('zentrid:context', (event: Event) => {
  const detail = (event as CustomEvent<Partial<TenantOverviewState>>).detail || {};
  if (detail.tenant) tenantOverviewState.tenant = detail.tenant;
  if (detail.time) tenantOverviewState.time = detail.time;
  if (detail.region) tenantOverviewState.region = detail.region;
  ...
  if (tenant) tenant.textContent = tenantOverviewState.tenant;
  if (time) time.textContent = tenantOverviewState.time;
  if (region) region.textContent = tenantOverviewState.region;
});
