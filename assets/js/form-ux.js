...
const FleetFormUX = (() => {
    ...
    function validationMessage(control) {
        ...
        if (control.validity.rangeOverflow)
            return `${label} is above the allowed value.`;
    ...
    function clearErrors(root, summary) {
    ...
    function renderSummary(summary, issues, title = 'Please review the highlighted fields') {
        if (!summary)
            return;
        if (!issues.length) {
            summary.hidden = true;
    ...
    function validate(root, customIssues = [], summary, title) {
        clearErrors(root, summary);
        const issues = [];
        controls(root).forEach(control => {
            if (!control.checkValidity())
                issues.push({ control, message: validationMessage(control) });
        });
        ...
        unique.forEach(issue => { if (issue.control)
            setControlError(issue.control, issue.message); });
        renderSummary(summary, unique, title);
        return { valid: unique.length === 0, issues: unique };
    }
    function focusFirst(result, summary) {
    ...
    function bindClearOnInput(form, summary) {
        const clear = (event) => {
