"use strict";
const asInput = (el) => el;
const formValue = (value) => (value || '').toString();
const eventElement = (event) => event.target;
const FleetClientModel = (() => {
    ...
    try {
        const savedClients = (window.FleetLocalStore ? FleetLocalStore.read(FleetLocalStore.KEYS.clients, []) : JSON.parse(localStorage.getItem('zentrid_custom_clients') || '[]'));
    ...
    function badge(value) {
        ...
        if (v.includes('fault') || v.includes('review') || v.includes('maintenance'))
            return 'warning';
        if (v.includes('offline') || v.includes('blocked'))
            return 'danger';
        return 'success';
    }
    ...
    function selectClient(id) { localStorage.setItem('zentrid_selected_client', id); }
    function selectPlant(id) { localStorage.setItem('zentrid_selected_plant', id); }
    function selectedClient() { return getClient(localStorage.getItem('zentrid_selected_client') || clients[0].id); }
    function selectedPlant() { return getPlant(localStorage.getItem('zentrid_selected_plant') || plantsForClient(selectedClient().id)[0]?.id || plants[0].id); }
...
(function hydrateCustomPlantBuilderRecords() {
    try {
        const customPlants = (window.FleetLocalStore ? FleetLocalStore.read(FleetLocalStore.KEYS.clientPlants, []) : JSON.parse(localStorage.getItem('zentrid_custom_plants') || '[]'));
        const customDevices = (window.FleetLocalStore ? FleetLocalStore.read(FleetLocalStore.KEYS.clientDevices, []) : JSON.parse(localStorage.getItem('zentrid_custom_devices') || '[]'));
        customPlants.forEach(p => {
            if (!p || !p.id)
                return;
            const existingIndex = FleetClientModel.plants.findIndex(item => item.id === p.id);
            if (existingIndex >= 0)
                FleetClientModel.plants[existingIndex] = { ...FleetClientModel.plants[existingIndex], ...p };
            else
                FleetClientModel.plants.push(p);
        });
        customDevices.forEach(d => {
            ...
            if (existingIndex >= 0)
                FleetClientModel.devices[existingIndex] = { ...FleetClientModel.devices[existingIndex], ...d };
            else
                FleetClientModel.devices.push(d);
        });
    }
    catch (err) {
...
function renderClientsPage() {
  ...
  `);
    const render = () => {
        ...
        if (target)
            FleetRuntimeStability.replaceHtml(target, clientRowsMarkup(filtered));
        ...
        if (scope)
            scope.innerHTML = window.FleetRegistryQuery?.filterScopeHtml('clients') || '';
    };
    ...
    document.getElementById('clientCreateBackdrop')?.addEventListener('click', e => {
        ...
        if (!(target instanceof Element))
            return;
        if (target.id === 'clientCreateBackdrop' || target.closest('[data-close-client-create]'))
    ...
    document.querySelector('.client-table-v17')?.addEventListener('click', e => {
        ...
        if (!(target instanceof Element))
            return;
        const row = target.closest('.data-row[data-client]');
        ...
        if (action === 'edit')
            localStorage.setItem('zentrid_client_detail_edit', 'identity');
        location.href = `${FleetLayout.pathFor('client-detail')}?id=${encodeURIComponent(clientId)}`;
    });
}
const clientCreateSteps = ['tenant', 'identity', 'location', 'portal', 'documentation', 'banking', 'review'];
const clientCreateLocationRules = {
    Armenia: {
        regions: {
            Yerevan: ['Yerevan'],
...
function clientCreateFormElement() {
    return document.getElementById('clientCreateForm');
}
function clientCreateModalElement() {
    return document.getElementById('clientCreateBackdrop');
}
function clientCreateControl(name) {
    return clientCreateFormElement()?.elements.namedItem(name);
}
function normalizeClientDuplicateValue(value) {
    return String(value || '').trim().toLocaleLowerCase().replace(/\s+/g, ' ');
}
function normalizeClientIdentifier(value) {
    return String(value || '').trim().toLocaleLowerCase().replace(/[^a-z0-9]/g, '');
}
function clientDraftName(form) {
    const fd = new FormData(form);
    const type = formValue(fd.get('type')) || 'Individual';
    if (type === 'Legal Entity')
        return formValue(fd.get('companyName')).trim();
    return [fd.get('name'), fd.get('surname'), fd.get('lastName')]
        .map(value => formValue(value).trim())
        .filter(Boolean)
        .join(' ');
}
function renderClientDocumentList() {
    const modal = clientCreateModalElement();
    const host = modal?.querySelector('#clientUploadedDocs');
    if (!modal || !host)
        return;
    const docs = Array.from(modal.querySelectorAll('[data-doc-label]'))
...
function resetClientBankList() {
    const host = clientCreateModalElement()?.querySelector('#clientBankList');
    if (!host)
        return;
    const cards = Array.from(host.querySelectorAll('.client-bank-card'));
    ...
    first.querySelectorAll('select').forEach(select => { select.value = 'AMD'; });
    const deleteButton = first.querySelector('[data-remove-bank]');
    if (deleteButton)
        deleteButton.disabled = true;
    first.dataset.bankIndex = '0';
    const title = first.querySelector('.client-bank-head strong');
    if (title)
        title.textContent = 'Bank 1';
}
function clientCreateDraftSnapshot() {
...
function openClientCreateModal() {
    ...
    if (!modal || !form)
        return;
    form.reset();
    form.querySelectorAll('[data-doc-label]').forEach(input => { input.value = ''; });
    resetClientBankList();
    ...
    const saveButton = document.getElementById('saveClientCreate');
    if (saveButton)
    ...
    window.setTimeout(() => clientCreateControl('type')?.focus(), 0);
}
function closeClientCreateModal(force = false) {
    const modal = clientCreateModalElement();
    if (!modal || !modal.classList.contains('open'))
        return;
    const changed = clientCreateInitialSnapshot && clientCreateDraftSnapshot() !== clientCreateInitialSnapshot;
    if (!force && !clientCreateSaving && changed && !window.confirm('Discard the client information entered in this wizard?'))
        return;
...
function initClientCreateWizard() {
    const modal = clientCreateModalElement();
    const form = clientCreateFormElement();
    if (!modal || !form || modal.dataset.clientWizardReady)
        return;
    modal.dataset.clientWizardReady = 'true';
    initClientDocumentList(modal);
    initClientBankList(modal);
    modal.querySelectorAll('[data-client-create-step]').forEach(button => {
        button.addEventListener('click', () => {
            const target = clientCreateSteps.indexOf(button.dataset.clientCreateStep);
            if (target < 0)
                return;
            if (target > clientCreateCurrentStep && !validateBeforeClientCreateStep(target))
                return;
    ...
    document.getElementById('nextClientCreateStep')?.addEventListener('click', event => {
        ...
        if (!validateClientCreateStep(clientCreateCurrentStep))
            return;
...
function setClientCreateStep(step) {
    ...
    if (!modal)
        return;
    const requested = clientCreateSteps.indexOf((step || 'tenant'));
    ...
    modal.querySelectorAll('[data-client-create-panel]').forEach(panel => panel.classList.toggle('active', panel.dataset.clientCreatePanel === activeStep));
    const previous = document.getElementById('previousClientCreateStep');
    const next = document.getElementById('nextClientCreateStep');
    const save = document.getElementById('saveClientCreate');
    const progress = document.getElementById('clientWizardProgress');
    if (previous)
        previous.disabled = clientCreateCurrentStep === 0;
    if (next)
        next.hidden = clientCreateCurrentStep === clientCreateSteps.length - 1;
    if (save)
        save.hidden = clientCreateCurrentStep !== clientCreateSteps.length - 1;
    if (progress)
        progress.textContent = `Step ${clientCreateCurrentStep + 1} of ${clientCreateSteps.length}`;
...
function setClientSelectOptions(select, options, preferred) {
    if (!select)
        return;
...
function updateClientCreateCities() {
    const country = clientCreateControl('country')?.value || 'Armenia';
    const region = clientCreateControl('region')?.value || '';
    const rule = clientCreateLocationRules[country] || clientCreateLocationRules.Armenia;
    setClientSelectOptions(clientCreateControl('city'), rule.regions[region] || []);
}
function applyClientCreateLocationRules(resetDependent = false) {
    const country = clientCreateControl('country')?.value || 'Armenia';
    const rule = clientCreateLocationRules[country] || clientCreateLocationRules.Armenia;
    const region = clientCreateControl('region');
    ...
    updateClientCreateCities();
    const timezone = clientCreateControl('timezone');
    if (timezone && Array.from(timezone.options).some(option => option.value === rule.timezone))
        timezone.value = rule.timezone;
    const currency = clientCreateControl('currency');
    if (currency && Array.from(currency.options).some(option => option.value === rule.currency))
        currency.value = rule.currency;
    const phone1 = clientCreateControl('phone1');
    const phone2 = clientCreateControl('phone2');
    if (phone1)
        phone1.placeholder = rule.phonePlaceholder;
    if (phone2)
        phone2.placeholder = `Optional · ${rule.phonePlaceholder}`;
}
function clientCreateCustomIssues(index, includeDuplicates = false) {
    const form = clientCreateFormElement();
    if (!form)
        return [];
    const issues = [];
    const type = clientCreateControl('type')?.value || 'Individual';
    if (index === 1) {
        ['name', 'surname', 'lastName', 'companyName', 'registrationNo', 'taxId', 'contactPerson'].forEach(name => {
            const control = clientCreateControl(name);
            if (control && !control.disabled)
                control.value = control.value.trim();
        });
        const dob = clientCreateControl('dob');
        if (dob && !dob.disabled && dob.value) {
            ...
            if (!match || day < 1 || day > 31 || month < 1 || month > 12 || year < 1900 || year > currentYear)
                issues.push({ control: dob, message: 'Enter a valid date of birth in dd/mm/yyyy format.' });
        }
        if (includeDuplicates) {
            ...
            if (duplicateName) {
                const control = type === 'Legal Entity' ? clientCreateControl('companyName') : clientCreateControl('name');
                issues.push({ control, message: `A client named “${duplicateName.name}” already exists.` });
            }
            if (type === 'Legal Entity') {
                const registration = clientCreateControl('registrationNo');
                const tax = clientCreateControl('taxId');
                ...
                if (duplicateRegistration && registration)
                    issues.push({ control: registration, message: `Registration number is already used by ${duplicateRegistration.name}.` });
                if (duplicateTax && tax)
                    issues.push({ control: tax, message: `Tax identifier is already used by ${duplicateTax.name}.` });
            }
        }
    }
    if (index === 3) {
        const email = clientCreateControl('email');
        const username = clientCreateControl('username');
        const password = clientCreateControl('password');
        const phone1 = clientCreateControl('phone1');
        if (email)
        ...
        if (password?.value && (!/[A-Za-z]/.test(password.value) || !/\d/.test(password.value)))
            issues.push({ control: password, message: 'Temporary password must contain at least one letter and one number.' });
        if (includeDuplicates) {
            ...
            if (duplicateEmail && email)
                issues.push({ control: email, message: `E-mail is already used by ${duplicateEmail.name}.` });
            if (duplicateUsername && username)
                issues.push({ control: username, message: `Username is already assigned to ${duplicateUsername.name}.` });
        }
    }
    if (index === 4) {
        const passportNumber = clientCreateControl('passportNumber');
        const registrationDocumentNumber = clientCreateControl('stateRegistrationNumber');
        const passportFile = clientCreateControl('clientPassport');
        const registrationFile = clientCreateControl('stateRegistrationDocument');
        form.querySelectorAll('input[type="file"][data-doc-label]').forEach(input => {
            const file = input.files?.[0];
            if (file && !clientDocumentFileAllowed(file))
                issues.push({ control: input, message: `${file.name} is not supported. Allowed document formats: PDF, DOC, DOCX, JPG, JPEG, PNG.` });
        });
        if (type === 'Individual' && !passportNumber?.value.trim() && !passportFile?.files?.length)
            issues.push({ control: passportNumber, message: 'Enter a passport / personal document number or upload the client passport.' });
        if (type === 'Legal Entity' && !registrationDocumentNumber?.value.trim() && !registrationFile?.files?.length)
            issues.push({ control: registrationDocumentNumber, message: 'Enter a state registration document number or upload the registration document.' });
    }
    if (index === 5) {
        const accounts = new Map();
        form.querySelectorAll('.client-bank-card').forEach(card => {
            const bank = card.querySelector('[name="bankName"]');
            const code = card.querySelector('[name="bankCode"]');
            const account = card.querySelector('[name="accountNumber"]');
            [bank, code, account].forEach(control => { if (control)
                control.value = control.value.trim(); });
            const started = Boolean(bank?.value || code?.value || account?.value);
            if (!started)
                return;
            if (!bank?.value)
                issues.push({ control: bank, message: 'Enter the bank name for each configured account.' });
            if (!account?.value)
                issues.push({ control: account, message: 'Enter the account number or IBAN for each configured bank.' });
            ...
            if (normalizedAccount && account) {
                ...
                if (previous)
                    issues.push({ control: account, message: 'The same bank account number cannot be added twice.' });
                else
                    accounts.set(normalizedAccount, account);
            }
        });
    }
    return issues;
}
function validateClientCreateStep(index, includeDuplicates = false) {
    const stepName = clientCreateSteps[index];
    const modal = clientCreateModalElement();
    const summary = document.getElementById('clientValidationSummary');
    if (!stepName || !modal)
        return true;
    const panel = modal.querySelector(`[data-client-create-panel="${stepName}"]`);
    const rail = modal.querySelector(`[data-client-create-step="${stepName}"]`);
    if (!panel)
        return true;
...
function validateBeforeClientCreateStep(targetIndex) {
    if (targetIndex <= clientCreateCurrentStep)
        return true;
    for (let index = 0; index < targetIndex; index += 1) {
        if (!validateClientCreateStep(index))
            return false;
    }
    return true;
}
function updateClientCreateReview() {
    ...
    const set = (key, value, emptyLabel = 'Not entered') => {
        const element = form.querySelector(`[data-review-field="${key}"]`);
        if (element)
            element.textContent = formValue(value).trim() || emptyLabel;
    };
    ...
    const clientPassport = clientCreateControl('clientPassport')?.files?.[0]?.name || '';
    const stateDocument = clientCreateControl('stateRegistrationDocument')?.files?.[0]?.name || '';
    const projectDocument = clientCreateControl('projectDoc')?.files?.[0]?.name || '';
    ...
    const banks = Array.from(form.querySelectorAll('.client-bank-card')).map(card => {
        const name = card.querySelector('[name="bankName"]')?.value.trim() || '';
        const code = card.querySelector('[name="bankCode"]')?.value.trim() || '';
        const account = card.querySelector('[name="accountNumber"]')?.value.trim() || '';
...
function initClientDocumentList(modal) {
    if (modal.dataset.clientDocumentsReady)
        return;
    modal.dataset.clientDocumentsReady = 'true';
    modal.querySelectorAll('[data-doc-label]').forEach(input => input.addEventListener('change', () => {
        renderClientDocumentList();
        updateClientCreateReview();
    }));
    modal.addEventListener('click', event => {
        ...
        if (!(target instanceof Element))
            return;
        const button = target.closest('[data-remove-doc]');
        if (!button)
            return;
        const label = button.dataset.removeDoc || 'document';
        if (!window.confirm(`Remove ${label} from the client draft?`))
            return;
        const input = Array.from(modal.querySelectorAll('[data-doc-label]')).find(item => item.dataset.docLabel === label);
        if (input)
            input.value = '';
        renderClientDocumentList();
        updateClientCreateReview();
...
function initClientBankList(modal) {
    ...
    if (!host || !add || host.dataset.ready)
        return;
    ...
    const refresh = () => {
        ...
        cards.forEach((card, index) => {
            ...
            const title = card.querySelector('.client-bank-head strong');
            if (title)
                title.textContent = `Bank ${index + 1}`;
        ...
        if (!host.querySelector('[name="primaryBank"]:checked'))
            host.querySelector('[name="primaryBank"]')?.click();
        updateClientCreateReview();
    };
    const bindCard = (card) => card.querySelectorAll('input, select').forEach(element => {
        element.addEventListener('input', updateClientCreateReview);
        element.addEventListener('change', updateClientCreateReview);
    });
    host.querySelectorAll('.client-bank-card').forEach(bindCard);
    add.onclick = () => {
        const source = host.querySelector('.client-bank-card');
        if (!source)
            return;
        ...
        clone.querySelectorAll('select').forEach(select => { select.value = clientCreateControl('currency')?.value || 'AMD'; });
    ...
    host.addEventListener('click', event => {
        ...
        if (!(target instanceof Element))
            return;
        const remove = target.closest('[data-remove-bank]');
        if (!remove || host.querySelectorAll('.client-bank-card').length <= 1)
            return;
        if (!window.confirm('Remove this bank account from the client draft?'))
            return;
...
function createLocalClientId() {
    ...
    let suffix = 1;
    while (FleetClientModel.clients.some(client => client.id === id))
        id = `CL-${Date.now().toString(36).toUpperCase()}-${suffix++}`;
    return id;
}
function clientCreateResponseRecord(value) {
...
function clientCreateBackendId(value) {
...
function clientCreateTenantId() {
    const runtime = window.FleetAPI;
    const session = runtime?.auth?.getSession?.();
    const sources = [session?.user, session?.claims].filter((value) => Boolean(value && typeof value === 'object'));
...
function clientCreateApiPayload(fd, type, fullName, contactPerson, bankAccounts, form) {
    ...
    const passportNumber = String(form.querySelector('[name="passportNumber"]')?.value || '').trim();
    const legalRegistrationNumber = String(form.querySelector('[name="registrationNo"]')?.value || '').trim();
    const legalTaxId = String(form.querySelector('[name="taxId"]')?.value || '').trim();
    return {
        clientName: fullName,
        tenantLink: {
...
function clientDocumentFileAllowed(file) {
    if (!file)
        return false;
    const extension = file.name.split('.').pop()?.toLocaleLowerCase() || '';
    return ['pdf', 'doc', 'docx', 'jpg', 'jpeg', 'png'].includes(extension);
}
function clientCreateUploadCandidates(form) {
    return Array.from(form.querySelectorAll('input[type="file"][data-doc-label]')).map(input => {
        const file = input.files?.[0];
        if (!clientDocumentFileAllowed(file))
            return null;
        const safeFile = file;
        ...
        const expiryControl = expiryName ? form.elements.namedItem(expiryName) : null;
        return { file: safeFile, name, type, expiry: String(expiryControl?.value || '').trim() };
    }).filter((item) => Boolean(item));
}
async function uploadClientCreateDocuments(clientId, form) { const candidates = clientCreateUploadCandidates(form); if (!candidates.length)
    return { uploaded: 0, failures: [] }; let uploaded = 0; const failures = []; for (const candidate of candidates) {
    const result = await window.FleetAPIMutations.clients.uploadDocument(clientId, clientDocumentMultipart(candidate));
    if (result.ok)
        uploaded += 1;
    else
        failures.push(`${candidate.name}: ${result.error?.message || result.message || 'upload failed'}`);
} return { uploaded, failures }; }
async function submitClientCreateForm(e) {
    e.preventDefault();
    if (!FleetActionPermissions.guard({ action: 'create', resource: 'client' }))
        return;
    ...
    for (let index = 0; index < clientCreateSteps.length; index += 1) {
        ...
        if (!validateClientCreateStep(index, includeDuplicates))
            return;
    }
    const fd = new FormData(form);
    const type = formValue(fd.get('type')) || 'Individual';
    ...
    const bankAccounts = Array.from(form.querySelectorAll('.client-bank-card')).map(card => ({
        bankName: card.querySelector('[name="bankName"]')?.value.trim() || '',
        bankCode: card.querySelector('[name="bankCode"]')?.value.trim() || '',
        accountNumber: card.querySelector('[name="accountNumber"]')?.value.trim() || '',
        accountCurrency: card.querySelector('[name="accountCurrency"]')?.value || '',
        primary: Boolean(card.querySelector('[name="primaryBank"]')?.checked)
    ...
    if (saveButton)
        FleetFormUX.setBusy(saveButton, true, 'Creating Client…');
    try {
        if (!window.FleetAPIMutations)
            throw new Error('Client mutation runtime is unavailable.');
        ...
        if (backendId) {
            ...
            if (uploads.failures.length)
                FleetLayout.toast(`Client created, but ${uploads.failures.length} document upload(s) failed.`);
            else if (uploads.uploaded)
                FleetLayout.toast(`Client created with ${uploads.uploaded} document${uploads.uploaded === 1 ? '' : 's'}.`);
        }
        clientCreateInitialSnapshot = clientCreateDraftSnapshot();
...
function clientRowsMarkup(rows) {
    if (!rows.length)
        return `<div class="empty-state-v28"><strong>No clients found</strong><small>Try changing search, type or status filters.</small></div>`;
...
let clientDetailActiveTab = 'overview';
let clientDetailEditMode = false;
let clientDetailDraft = null;
let clientDetailEditSnapshot = '';
let clientDetailBusy = false;
let clientDetailBeforeUnloadBound = false;
function clientDetailEscape(value) {
    return String(value ?? '').replace(/&/g, '&amp;').replace(/</g, '&lt;').replace(/>/g, '&gt;').replace(/"/g, '&quot;').replace(/'/g, '&#039;');
}
function clientDetailAttr(value) { return clientDetailEscape(value).replace(/`/g, '&#096;'); }
function clientDetailClone(record) { return JSON.parse(JSON.stringify(record)); }
function clientDetailOrigin(record) { return FleetEntityDetailUX.origin(record, 'client'); }
function clientDetailBackendManaged(record) { return FleetEntityDetailUX.backendManaged(record, 'client'); }
function clientDetailIsArchived(record) { return FleetEntityDetailUX.archived(record.status); }
function clientDetailEditableTab(tab = clientDetailActiveTab) { return ['overview', 'identity', 'location', 'portal', 'users', 'commercial'].includes(tab); }
function clientDetailCanEdit(record, tab = clientDetailActiveTab) {
...
function clientDetailFreshness(record) {
...
function clientDetailModeCopy(record) {
...
function renderClientDetailControl(record) {
...
function setClientDetailFeedback(tone, title, message) {
...
function clearClientDetailFeedback() {
    FleetEntityDetailUX.clearFeedback('clientDetailFeedback', 'client-detail-feedback-v118');
}
function clientDetailDocumentsData(client) {
    if (Array.isArray(client.documentRecords))
        return client.documentRecords.map(item => ({ ...item }));
    const raw = client.raw && typeof client.raw === 'object' ? client.raw : {};
    const candidates = [raw.documentRecords, raw.clientDocuments, raw.documents].find(Array.isArray);
    if (!candidates)
        return [];
...
function clientDetailPortalUsersData(client, _plants) {
    return clientPortalUsers(client, []);
}
function clientDetailPrepareDraft(record, plants) {
...
function clientDetailSnapshot() { return clientDetailDraft ? JSON.stringify(clientDetailDraft) : ''; }
function clientDetailHasUnsavedEdits() { return clientDetailEditMode && clientDetailEditSnapshot !== clientDetailSnapshot(); }
function clientDetailConfirmDiscard(message = 'Discard unsaved client changes?') {
    return FleetEntityDetailUX.confirmDiscard(clientDetailHasUnsavedEdits(), message);
}
function clientDetailSectionTitle(tab) {
    const titles = {
...
function clientDetailSectionContext(record, tab, editable = clientDetailEditMode) {
...
function clientDetailEditableValue(value) {
    if (value === null || value === undefined)
        return '';
    const text = String(value).trim();
    if (!text || text === '—' || text.toLowerCase() === 'not provided' || text.toLowerCase() === 'not configured')
        return '';
    return text;
}
function clientDetailInput(key, label, value, options, type = 'text', required = false) {
    const req = required ? ' required' : '';
    const editableValue = clientDetailEditableValue(value);
    const safeValue = clientDetailAttr(editableValue);
    if (options) {
        const normalizedOptions = [...options];
        if (editableValue && !normalizedOptions.includes(editableValue))
            normalizedOptions.push(editableValue);
        if ((!required || !editableValue) && !normalizedOptions.includes(''))
    ...
    if (textarea)
        return `<label>${clientDetailEscape(label)}${required ? ' *' : ''}<textarea data-client-edit-key="${String(key)}" name="client-edit-${String(key)}"${req}>${clientDetailEscape(editableValue)}</textarea></label>`;
...
function clientDetailDocumentsEditor(client) {
...
function clientDetailUsersEditor(client) {
...
function clientDetailBankEditor(client) {
...
function clientDetailValidationIssues(record, tab, root) {
    const issues = [];
    const byKey = (key) => root.querySelector(`[data-client-edit-key="${key}"]`);
    const duplicate = (predicate) => FleetClientModel.clients.some(client => client.id !== record.id && predicate(client));
    if (tab === 'identity') {
        if (duplicate(client => client.name.trim().toLowerCase() === record.name.trim().toLowerCase()))
            issues.push({ control: byKey('name'), message: 'Another client already uses this name.' });
        if (record.registrationNo && duplicate(client => String(client.registrationNo || '').trim().toLowerCase() === record.registrationNo.trim().toLowerCase()))
            issues.push({ control: byKey('registrationNo'), message: 'Another client already uses this registration or identity number.' });
        if (record.taxId && record.taxId !== 'Not provided' && duplicate(client => String(client.taxId || '').trim().toLowerCase() === record.taxId.trim().toLowerCase()))
            issues.push({ control: byKey('taxId'), message: 'Another client already uses this Tax ID.' });
        const docs = record.documentRecords || [];
        const names = docs.map(doc => doc.name.trim().toLowerCase()).filter(Boolean);
        if (new Set(names).size !== names.length)
            issues.push({ message: 'Document names must be unique within the client record.' });
        docs.forEach((doc, index) => {
            const row = root.querySelector(`[data-client-document-row="${index}"]`);
            if (!doc.name.trim())
                issues.push({ control: row?.querySelector('[data-client-document-field="name"]') || null, message: `Document ${index + 1} requires a name.` });
            if (!doc.type.trim() || !doc.status.trim())
                issues.push({ message: `Document ${index + 1} requires a type and status.` });
        });
    }
    if (tab === 'portal') {
        const email = record.contactEmail.trim().toLowerCase();
        if (email && duplicate(client => String(client.contactEmail || '').trim().toLowerCase() === email))
            issues.push({ control: byKey('contactEmail'), message: 'Another client already uses this contact email.' });
        const username = String(record.username || '').trim().toLowerCase();
        if (username && duplicate(client => String(client.username || '').trim().toLowerCase() === username))
            issues.push({ control: byKey('username'), message: 'Another client already uses this portal username.' });
    }
    if (tab === 'users') {
        const users = record.portalUsers || [];
        const emails = users.map(user => user.email.trim().toLowerCase()).filter(Boolean);
        if (new Set(emails).size !== emails.length)
            issues.push({ message: 'Portal user emails must be unique.' });
        if ((record.username || record.status === 'Active') && !users.length)
            issues.push({ message: 'An active or configured portal client requires at least one portal user.' });
        if (users.length && !users.some(user => ['Owner User', 'Client Admin'].includes(user.role) && user.status !== 'Suspended'))
            issues.push({ message: 'At least one non-suspended Owner User or Client Admin is required.' });
        users.forEach((user, index) => {
            const row = root.querySelector(`[data-client-user-row="${index}"]`);
            if (!user.name.trim())
                issues.push({ control: row?.querySelector('[data-client-user-field="name"]') || null, message: `Portal user ${index + 1} requires a name.` });
            if (!user.email.trim())
                issues.push({ control: row?.querySelector('[data-client-user-field="email"]') || null, message: `Portal user ${index + 1} requires an email.` });
        });
    }
    if (tab === 'commercial') {
        ...
        if (new Set(accountNumbers).size !== accountNumbers.length)
            issues.push({ message: 'Bank account numbers must be unique.' });
        accounts.forEach((account, index) => {
            const row = root.querySelector(`[data-client-bank-row="${index}"]`);
            if (!account.bankName.trim())
                issues.push({ control: row?.querySelector('[data-client-bank-field="bankName"]') || null, message: `Bank account ${index + 1} requires a bank name.` });
            if (!account.accountNumber.trim())
                issues.push({ control: row?.querySelector('[data-client-bank-field="accountNumber"]') || null, message: `Bank account ${index + 1} requires an account number.` });
        });
        if (accounts.length && !accounts.some(account => account.primary))
            issues.push({ message: 'Select one primary bank account.' });
    }
    return issues;
}
function updateClientDetailActions(record) {
    const edit = document.getElementById('editClientTab');
    const cancel = document.getElementById('cancelClientEdit');
    const save = document.getElementById('saveClientEdit');
    ...
    if (cancel)
        cancel.hidden = !clientDetailEditMode;
    if (save)
        save.hidden = !clientDetailEditMode;
}
function renderClientDetailCurrentTab(baseRecord, plants) {
    ...
    if (content)
        content.innerHTML = clientTab(record, plants, clientDetailActiveTab, clientDetailEditMode);
    const title = document.getElementById('clientDetailActiveTitle');
    if (title)
        title.textContent = clientDetailSectionTitle(clientDetailActiveTab);
    updateClientDetailActions(baseRecord);
}
function setClientDetailEditMode(enabled, baseRecord, plants) {
    if (clientDetailBusy)
        return;
    if (enabled && clientDetailActiveTab === 'overview') {
        clientDetailActiveTab = 'identity';
        document.querySelectorAll('[data-client-tab]').forEach(item => {
    ...
    if (!enabled && clientDetailEditMode && !clientDetailConfirmDiscard())
        return;
...
function clientApiNullableScalar(value) {
    if (value === null || value === undefined)
        return null;
...
function clientApiDateOnly(value) {
...
function clientApiDocumentReference(value) {
    return value && typeof value === 'object' && !Array.isArray(value) ? value : null;
}
function clientDetailApiPayload(record) {
    const raw = record.raw && typeof record.raw === 'object' ? record.raw : {};
    const rawTenantLink = raw.tenantLink && typeof raw.tenantLink === 'object' && !Array.isArray(raw.tenantLink) ? raw.tenantLink : {};
    const rawIdentity = raw.identity && typeof raw.identity === 'object' && !Array.isArray(raw.identity) ? raw.identity : {};
    const rawPortal = raw.portalAccount && typeof raw.portalAccount === 'object' && !Array.isArray(raw.portalAccount) ? raw.portalAccount : {};
    const rawAddress = raw.address && typeof raw.address === 'object' && !Array.isArray(raw.address) ? raw.address : {};
    const rawPreferences = raw.preferences && typeof raw.preferences === 'object' && !Array.isArray(raw.preferences) ? raw.preferences : {};
    const rawPrimary = raw.primaryContact && typeof raw.primaryContact === 'object' && !Array.isArray(raw.primaryContact) ? raw.primaryContact : {};
    const rawDocumentation = raw.documentation && typeof raw.documentation === 'object' && !Array.isArray(raw.documentation) ? raw.documentation : {};
    ...
    const portalAccountPayload = { role: clientApiNullableScalar(record.portalRole) || clientApiNullableScalar(record.assignmentRole) || clientApiNullableScalar(rawPortal.role) };
    ...
    return {
        ...
        bankAccounts: Array.isArray(record.bankAccounts) ? record.bankAccounts.map(account => { const payload = { bankName: String(account.bankName || account.bank || '').trim(), bankCode: String(account.bankCode || '').trim(), accountNumber: String(account.accountNumber || account.account || '').trim(), accountCurrency: String(account.accountCurrency || account.currency || '').trim(), primary: Boolean(account.primary) }; if (account.id)
            payload.id = account.id; return payload; }) : []
    };
}
function normalizeClientDocumentType(value) {
    const raw = String(value || '').trim();
    const aliases = { identity: 'Identity', passport: 'Identity', registration: 'Legal', legal: 'Legal', project: 'Commercial', commercial: 'Commercial', finance: 'Finance', compliance: 'Compliance', access: 'Access', assignment: 'Assignment' };
    return aliases[raw.toLowerCase()] || raw || 'Legal';
}
function clientDocumentMultipart(candidate) {
...
async function uploadClientDetailDocuments(clientId, docs) {
    const candidates = docs.filter(doc => doc.localFile instanceof File && !doc.id && !doc.filePath);
    if (!candidates.length)
        return { uploaded: 0, failures: [] };
    if (!window.FleetAPIMutations?.clients?.uploadDocument)
        return { uploaded: 0, failures: candidates.map(doc => `${doc.name || doc.localFile?.name || 'Document'}: upload API unavailable`) };
    let uploaded = 0;
    const failures = [];
...
async function saveClientDetailEdits(baseRecord, plants) {
    if (!clientDetailEditMode || !clientDetailDraft || clientDetailBusy)
        return;
    const backendManaged = clientDetailBackendManaged(baseRecord);
    if (!FleetActionPermissions.guard({ action: 'edit', resource: 'client', record: baseRecord, status: baseRecord.status, origin: clientDetailOrigin(baseRecord), updateAvailable: backendManaged, localOverride: !backendManaged }))
        return;
    ...
    if (!content)
        return;
    const validation = FleetFormUX.validate(content, clientDetailValidationIssues(clientDetailDraft, clientDetailActiveTab, content), summary, 'Client changes were not saved');
    if (!validation.valid) {
        FleetFormUX.focusFirst(validation, summary);
        return;
    }
    const button = document.getElementById('saveClientEdit');
    clientDetailBusy = true;
    document.getElementById('clientDetailControl')?.setAttribute('aria-busy', 'true');
    if (button)
        FleetFormUX.setBusy(button, true, backendManaged ? 'Saving to API…' : 'Saving…');
    try {
        if (backendManaged) {
            if (!window.FleetAPIMutations?.clients?.update)
                throw new Error('Client update API runtime is unavailable.');
            const result = await window.FleetAPIMutations.clients.update(baseRecord.id, clientDetailApiPayload(clientDetailDraft));
            if (!result.ok)
                throw new Error(result.error?.message || result.message || 'Backend rejected the client update.');
            ...
            const apiRecord = refreshed?.item;
            if (apiRecord?.id)
                Object.assign(baseRecord, apiRecord);
        ...
        const changed = FleetDataSource.markChanged({ ...clientDetailDraft, updated: new Date().toLocaleString() }, 'client');
        ...
        if (button)
            FleetFormUX.setBusy(button, false);
        FleetFormUX.renderSummary(summary, [{ message: error instanceof Error ? error.message : 'Unable to save the client.' }], 'Client changes were not saved');
        summary?.focus();
    }
}
function addClientDetailDocument(baseRecord, plants) {
    if (!clientDetailDraft)
        return;
...
function selectClientDetailDocumentFile(input) {
    if (!clientDetailDraft)
        return;
    const index = Number(input.dataset.clientDocumentFile);
    const file = input.files?.[0];
    if (!Number.isInteger(index) || !file || !clientDetailDraft.documentRecords?.[index])
        return;
    ...
    const row = input.closest('[data-client-document-row]');
    const nameInput = row?.querySelector('[data-client-document-field="name"]');
    if (nameInput && !nameInput.value.trim())
        nameInput.value = doc.name;
    const fileName = row?.querySelector(`[data-client-document-file-name="${index}"]`);
    if (fileName)
        fileName.textContent = file.name;
    setClientDetailFeedback('info', 'Document ready to upload', `${file.name} will be uploaded when you save Client changes.`);
}
function removeClientDetailDocument(index, baseRecord, plants) {
    if (!clientDetailDraft?.documentRecords?.[index])
        return;
    if (!window.confirm(`Remove ${clientDetailDraft.documentRecords[index].name || 'this document'} from the client draft?`))
        return;
    clientDetailDraft.documentRecords.splice(index, 1);
    renderClientDetailCurrentTab(baseRecord, plants);
}
function addClientDetailUser(baseRecord, plants) {
    if (!clientDetailDraft)
        return;
...
function removeClientDetailUser(index, baseRecord, plants) {
    if (!clientDetailDraft?.portalUsers?.[index])
        return;
    const user = clientDetailDraft.portalUsers[index];
    if (!window.confirm(`Remove ${user.name || user.email || 'this portal user'} from the client draft?`))
        return;
    clientDetailDraft.portalUsers.splice(index, 1);
    renderClientDetailCurrentTab(baseRecord, plants);
}
function addClientDetailBank(baseRecord, plants) {
    if (!clientDetailDraft)
        return;
...
function removeClientDetailBank(index, baseRecord, plants) {
    if (!clientDetailDraft?.bankAccounts?.[index])
        return;
    const account = clientDetailDraft.bankAccounts[index];
    if (!window.confirm(`Remove ${account.bankName || account.accountNumber || 'this bank account'} from the client draft?`))
        return;
    const wasPrimary = account.primary;
    clientDetailDraft.bankAccounts.splice(index, 1);
    if (wasPrimary && clientDetailDraft.bankAccounts[0])
        clientDetailDraft.bankAccounts[0].primary = true;
    renderClientDetailCurrentTab(baseRecord, plants);
}
function clientDetailString(value, fallback = '—') {
    const text = String(value ?? '').trim();
    return text || fallback;
}
function normalizeClientDetailRecord(source) {
    if (!source || source.dataOrigin !== 'live')
        return null;
    const row = source;
...
function selectedTenantClientForDetail() {
    ...
    if (index >= 0)
        FleetClientModel.clients[index] = record;
    else
        FleetClientModel.clients.unshift(record);
    FleetClientModel.selectClient(record.id);
    return record;
}
function backendDocumentMime(fileName) {
...
function triggerBackendDocumentDownload(blob, fileName) {
...
function resolveBackendDocumentPayload(payload, safeFileName) {
    const visited = new Set();
    const resolve = (value, depth = 0) => {
        if (depth > 4 || value == null || visited.has(value))
            return false;
        if (value instanceof Blob) {
            triggerBackendDocumentDownload(value, safeFileName);
            return true;
        }
        if (typeof value === 'string') {
            if (value.length && (value.startsWith('PK\x03\x04') || value.startsWith('%PDF-') || /[\x00-\x08\x0e-\x1f]/.test(value.slice(0, 64)))) {
                ...
                triggerBackendDocumentDownload(new Blob([bytes], { type: backendDocumentMime(safeFileName) }), safeFileName);
                return true;
            }
        ...
        if (typeof base64 === 'string') {
            try {
                ...
                triggerBackendDocumentDownload(new Blob([bytes], { type: String(record.contentType || record.mimeType || backendDocumentMime(safeFileName)) }), String(record.fileName || record.filename || safeFileName));
                return true;
            }
            catch (_error) {
                return false;
        ...
        for (const key of ['downloadUrl', 'fileUrl', 'url', 'uri', 'data', 'document', 'file', 'result', 'content'])
            if (resolve(record[key], depth + 1))
                return true;
        return false;
    };
...
function renderClientDetailPage() {
    const requestedEditTab = localStorage.getItem('zentrid_client_detail_edit');
    if (requestedEditTab && ['identity', 'location', 'portal', 'users', 'commercial'].includes(requestedEditTab))
        clientDetailActiveTab = requestedEditTab;
    if (requestedEditTab)
        localStorage.removeItem('zentrid_client_detail_edit');
  ...
  `);
    ...
    document.getElementById('backToClients')?.addEventListener('click', () => {
        if (!clientDetailConfirmDiscard('Discard unsaved client changes and return to Client Registry?'))
            return;
    ...
    document.querySelectorAll('[data-client-tab]').forEach(btn => btn.addEventListener('click', () => {
        const nextTab = (btn.dataset.clientTab || 'overview');
        if (clientDetailEditMode && !clientDetailConfirmDiscard('Discard unsaved changes and open another client section?'))
            return;
        clientDetailEditMode = false;
        clientDetailDraft = null;
        clientDetailEditSnapshot = '';
    ...
    clientTabContent?.addEventListener('click', event => {
        ...
        if (!(target instanceof Element))
            return;
        const lifecycle = target.closest('[data-client-lifecycle]');
        if (lifecycle) {
            void runClientDetailLifecycle(client, lifecycle.dataset.clientLifecycle);
            return;
        }
        const downloadClientDocument = target.closest('[data-client-document-download]');
        if (downloadClientDocument?.dataset.clientDocumentDownload) {
            const documentId = downloadClientDocument.dataset.clientDocumentDownload;
            const doc = clientDetailDocumentsData(client).find(item => String(item.id || item.filePath || '') === documentId);
            const fileName = String(doc?.fileName || doc?.name || `client-document-${documentId}`).trim() || `client-document-${documentId}`;
            void window.ZentridPlatformAPI.clients.getDocument(client.id, documentId).then((payload) => { if (resolveBackendDocumentPayload(payload, fileName))
                FleetLayout.toast('Client document download started.');
            else
                FleetLayout.toast('Unable to convert the backend document response into a downloadable file.'); }).catch(() => FleetLayout.toast('Unable to download client document.'));
            return;
        }
        const deleteClientDocument = target.closest('[data-delete-client-document]');
        if (deleteClientDocument?.dataset.deleteClientDocument) {
            const documentId = deleteClientDocument.dataset.deleteClientDocument;
            if (!FleetActionPermissions.guard({ action: 'edit', resource: 'client', record: client, status: client.status, origin: clientDetailOrigin(client), updateAvailable: true }))
                return;
            if (!window.confirm('Delete this client document from the backend?'))
                return;
            void window.FleetAPIMutations.clients.deleteDocument(client.id, documentId).then(result => { FleetLayout.toast(result.message); if (result.ok)
                window.setTimeout(() => window.location.reload(), 250); });
            return;
        }
        if (target.closest('[data-add-client-document]')) {
            addClientDetailDocument(client, plants);
            return;
        }
        const removeDocument = target.closest('[data-remove-client-document]');
        if (removeDocument) {
            removeClientDetailDocument(Number(removeDocument.dataset.removeClientDocument), client, plants);
            return;
        }
        if (target.closest('[data-add-client-user]')) {
            addClientDetailUser(client, plants);
            return;
        }
        const removeUser = target.closest('[data-remove-client-user]');
        if (removeUser) {
            removeClientDetailUser(Number(removeUser.dataset.removeClientUser), client, plants);
            return;
        }
        if (target.closest('[data-add-client-bank]')) {
            addClientDetailBank(client, plants);
            return;
        }
        const removeBank = target.closest('[data-remove-client-bank]');
        if (removeBank) {
            removeClientDetailBank(Number(removeBank.dataset.removeClientBank), client, plants);
            return;
        }
        const builderBtn = target.closest('[data-open-plant-builder]');
        const closeBuilder = target.closest('[data-close-plant-builder]');
        const stepBtn = target.closest('[data-builder-step]');
        const addDevice = target.closest('[data-builder-add-device]');
        const removeDevice = target.closest('[data-builder-remove-device]');
        const row = target.closest('[data-plant]');
        if (builderBtn)
            return openPlantBuilder();
        if (closeBuilder)
            return closePlantBuilder();
        if (stepBtn)
            return setPlantBuilderStep(stepBtn.dataset.builderStep || '1');
        if (addDevice)
            return addBuilderDevice(addDevice.dataset.builderAddDevice || 'Inverter');
        if (removeDevice)
            return removeBuilderDevice(removeDevice.dataset.builderRemoveDevice || '');
        const plantId = row?.dataset.plant;
        if (!plantId)
            return;
        if (!clientDetailConfirmDiscard('Discard unsaved client changes and open Plant Detail?'))
            return;
    ...
    const syncDynamicField = (target) => {
        if (!clientDetailDraft)
            return;
        clientDetailSyncControlToDraft(target);
        const documentRow = target.closest('[data-client-document-row]');
        const documentIndex = Number(documentRow?.dataset.clientDocumentRow);
        const documentField = target.dataset.clientDocumentField;
        if (documentField && clientDetailDraft.documentRecords?.[documentIndex] && (documentField === 'name' || documentField === 'type' || documentField === 'status' || documentField === 'expiry'))
            clientDetailDraft.documentRecords[documentIndex][documentField] = target.value;
        const userRow = target.closest('[data-client-user-row]');
        const userIndex = Number(userRow?.dataset.clientUserRow);
        const userField = target.dataset.clientUserField;
        if (userField && clientDetailDraft.portalUsers?.[userIndex])
            clientDetailDraft.portalUsers[userIndex][userField] = target.value;
        const bankRow = target.closest('[data-client-bank-row]');
        const bankIndex = Number(bankRow?.dataset.clientBankRow);
        const bankField = target.dataset.clientBankField;
        if (bankField && clientDetailDraft.bankAccounts?.[bankIndex]) {
            const account = clientDetailDraft.bankAccounts[bankIndex];
            if (bankField === 'bankName' || bankField === 'bankCode' || bankField === 'accountNumber' || bankField === 'accountCurrency' || bankField === 'bank' || bankField === 'account' || bankField === 'currency')
                account[bankField] = target.value;
        }
        ...
        const summary = document.getElementById('clientDetailEditSummary');
        if (clientDetailEditMode)
            FleetFormUX.clearErrors(clientTabContent || document, summary);
    };
    ...
    if (requestedEditTab && clientDetailCanEdit(client, requestedEditTab))
        setClientDetailEditMode(true, client, plants);
...
function plantBuilderStep(step) {
    if (step === 'device')
        return `<div class="builder-two-col-v27">
    ...
    if (step === 'compatibility')
        return `<div class="section-title-v17"><div><h2>Device Compatibility Rules</h2><p class="muted">Vendor is an attribute, not the hierarchy. Compatibility is based on protocol, firmware, electrical limits and source capability.</p></div></div>
    ...
    if (step === 'topology')
        return `<div class="section-title-v17"><div><h2>Topology Builder</h2><p class="muted">A plant can contain mixed-vendor device. Relationships are physical/electrical/logical, not vendor ownership.</p></div></div>
    ...
    if (step === 'review')
        return `<div class="section-title-v17"><div><h2>Review</h2><p class="muted">Before creating the plant, check required individual data and compatibility warnings.</p></div></div>
...
function builderDevices() {
    try {
        return JSON.parse(sessionStorage.getItem('zentrid_builder_devices_v27') || '[]');
    }
    catch {
        return [];
    }
}
function saveBuilderDevices(list) { sessionStorage.setItem('zentrid_builder_devices_v27', JSON.stringify(list)); }
function builderDeviceList() {
    const list = builderDevices();
    if (!list.length)
        return `<div class="empty-state"><strong>No device selected</strong><small>Add inverter, meter, logger, BESS or infrastructure devices from the catalog.</small></div>`;
...
function setPlantBuilderStep(step) {
    ...
    if (body)
        body.innerHTML = plantBuilderStep(step);
}
function addBuilderDevice(index) {
    ...
    if (target)
        target.innerHTML = builderDeviceList();
...
function plantSummaryCards(plants) {
    if (!plants.length)
        return `<div class="empty-state"><strong>No plants assigned yet</strong><small>Plants will appear here after assignment is completed.</small></div>`;
...
function assignmentRows(client, plants) {
    if (!plants.length)
        return `<div class="empty-state"><strong>No plant assigned yet</strong><small>This client is registered, but no plant assignment is visible in this tenant workspace.</small></div>`;
...
function clientDetailLifecycleHtml(client) {
    if (!clientDetailBackendManaged(client))
        return '';
    const status = String(client.status || '').trim().toLowerCase();
    if (status === 'archived')
        return `<div class="section-title-v17 mini"><div><h3>Lifecycle Actions</h3><p class="muted">Client lifecycle is managed by the backend.</p></div></div><div class="empty-state"><strong>Client archived</strong><small>This client is read-only.</small></div>`;
...
async function runClientDetailLifecycle(client, action) {
    if (!clientDetailBackendManaged(client) || !window.FleetAPIMutations?.clients?.[action])
        return;
    if (!FleetActionPermissions.guard({ action, resource: 'client', record: client, status: client.status, origin: clientDetailOrigin(client), updateAvailable: true }))
        return;
    const verb = action.charAt(0).toUpperCase() + action.slice(1);
    if (!window.confirm(`${verb} “${client.name}”?`))
        return;
    setClientDetailFeedback('info', `${verb} client`, 'Sending lifecycle action to backend…');
    const result = await window.FleetAPIMutations.clients[action](client.id);
    FleetLayout.toast(result.message);
    if (result.ok)
        window.setTimeout(() => window.location.reload(), 250);
    else
        setClientDetailFeedback('danger', `${verb} failed`, result.message);
}
...
function clientPortalUsers(client, _plants) {
    const raw = client.raw && typeof client.raw === 'object' ? client.raw : {};
    const candidates = [client.portalUsers, raw.portalUsers, raw.clientUsers, raw.users].find(Array.isArray);
    if (!candidates)
        return [];
    return candidates.map(item => {
        const row = item && typeof item === 'object' ? item : {};
...
function clientPlantsTab(client, plants) {
    ...
    if (!plants.length)
        return `<div class="section-title-v17"><div><h2>Assigned Plants</h2><p class="muted">Plants linked to this client will appear here with role, access and commercial scope.</p></div><button class="small-btn primary" type="button" onclick="location.href='${createUrl}'">Create Plant</button></div><div class="empty-state"><strong>No plants assigned</strong><small>No End User portal plant scope will be available until at least one plant is assigned.</small></div>`;
...
function clientBankAccounts(client) {
    const raw = client.raw && typeof client.raw === 'object' ? client.raw : {};
    const candidates = [client.bankAccounts, raw.bankAccounts, raw.banks, raw.paymentAccounts].find(Array.isArray);
    if (!candidates)
        return [];
...
function clientCommercialProfile(client, _plants) {
    const raw = client.raw && typeof client.raw === 'object' ? client.raw : {};
    const commercial = raw.commercial && typeof raw.commercial === 'object' ? raw.commercial : raw;
...
function clientCommercialRows(client, plants, profile) {
    return clientPlantAssignments(client, plants).map(assignment => {
        const raw = assignment.plant.raw && typeof assignment.plant.raw === 'object' ? assignment.plant.raw : {};
        const commercial = raw.commercial && typeof raw.commercial === 'object' ? raw.commercial : raw;
        return {
            ...assignment,
...
function clientAlertsTab(client, plants) {
    const linked = Array.isArray(client.linkedAlerts) ? client.linkedAlerts : [];
    const alerts = linked.map(item => item && typeof item === 'object' ? item : {}).map(row => {
        const plantId = String(row.plantId || (row.raw && typeof row.raw === 'object' ? row.raw.sourcePlantId || '' : ''));
...
function clientActivityTab(client, _plants) {
    const raw = client.raw && typeof client.raw === 'object' ? client.raw : {};
    const activity = [raw.activity, raw.activities, raw.auditTrail, raw.timeline].find(Array.isArray);
    const rows = (activity || []).map(item => item && typeof item === 'object' ? item : {}).map(row => ({
    ...
    if (!rows.length) {
        ...
        if (created)
            rows.push({ time: created, text: 'Client record created' });
        if (updated && updated !== created)
            rows.push({ time: updated, text: 'Client record updated' });
    }
...
function clientTab(client, plants, tab, editable = false, skipContext = false) {
    const activeTab = (tab || 'overview');
    if (editable)
        return clientDetailEditTab(client, plants, activeTab);
    let body = '';
    if (activeTab === 'identity')
        body = `${clientIdentityTab(client)}${clientDocuments(client)}`;
    else if (activeTab === 'location')
        body = clientLocationPreferencesTab(client);
    else if (activeTab === 'portal')
        body = clientContactsPortalTab(client, plants);
    else if (activeTab === 'users')
        body = clientUsersAccessTab(client, plants);
    else if (activeTab === 'plants')
        body = clientPlantsTab(client, plants);
    else if (activeTab === 'commercial')
        body = clientCommercialPaymentsTab(client, plants);
    else if (activeTab === 'alerts')
        body = clientAlertsTab(client, plants);
    else if (activeTab === 'activity')
        body = clientActivityTab(client, plants);
    else
        body = clientOverviewTab(client, plants);
    return `${skipContext ? '' : clientDetailSectionContext(client, activeTab, false)}${body}`;
}
let plantDetailActiveTab = 'overview';
let plantDetailEditMode = false;
let plantDetailDraft = null;
let plantDetailEditSnapshot = '';
let plantDetailBusy = false;
let plantDetailBeforeUnloadBound = false;
function plantDetailEscape(value) {
    return String(value ?? '').replace(/&/g, '&amp;').replace(/</g, '&lt;').replace(/>/g, '&gt;').replace(/"/g, '&quot;').replace(/'/g, '&#039;');
}
function plantDetailAttr(value) { return plantDetailEscape(value).replace(/`/g, '&#096;'); }
function plantDetailClone(record) { return JSON.parse(JSON.stringify(record)); }
function plantDetailOrigin(record) { return FleetEntityDetailUX.origin(record, 'plant'); }
function plantDetailAdminId(record) {
    const raw = record.raw || {};
    const adminRecord = raw.adminRecord && typeof raw.adminRecord === 'object' && !Array.isArray(raw.adminRecord) ? raw.adminRecord : {};
    return String(record.adminId || record.registryPlantId || adminRecord.id || adminRecord.plantId || '').trim();
}
function plantDetailBackendManaged(record) { return Boolean(plantDetailAdminId(record)); }
function plantDetailArchived(record) { return FleetEntityDetailUX.archived(record.status); }
function plantDetailEditableTab(tab = plantDetailActiveTab) { return tab === 'overview'; }
function plantDetailCanEdit(record, tab = plantDetailActiveTab) {
    return plantDetailBackendManaged(record) && !plantDetailArchived(record) && plantDetailEditableTab(tab);
}
function plantDetailSourceSystem(record) {
    if (record.sourceSystem)
        return String(record.sourceSystem);
    if (record.integration)
        return String(record.integration);
    const external = String(record.externalId || '');
    if (external.startsWith('HUA'))
        return 'Huawei FusionSolar';
    if (external.startsWith('SUN'))
        return 'Sungrow iSolarCloud';
    if (external.startsWith('SE-'))
        return 'SolarEdge';
    if (external === 'LOCAL-STORAGE')
        return 'Manual / Local storage';
    return 'Vendor connector';
}
function plantDetailFreshness(record) {
...
function plantDetailModeCopy(record) {
    if (plantDetailOrigin(record) !== 'local' && !plantDetailBackendManaged(record))
        return { tone: 'info', title: 'Operational plant · read-only', message: 'Master-data editing, lifecycle actions and documents require a linked /api/admin/plants record.' };
...
function setPlantDetailFeedback(tone, title, message) {
...
function plantDetailClientOptions(selectedId) {
...
function plantDetailTenantOptions(record) {
...
function plantDetailClientLabel(clientId) {
...
function plantDetailClientSelect(record) {
...
function plantDetailEditTab(record, tab) {
    if (tab === 'overview') {
...
function plantDetailSnapshot() { return JSON.stringify(plantDetailDraft || {}); }
function plantDetailHasUnsavedEdits() { return plantDetailEditMode && plantDetailEditSnapshot !== plantDetailSnapshot(); }
function plantDetailConfirmDiscard(message = 'Discard unsaved plant changes?') {
    return FleetEntityDetailUX.confirmDiscard(plantDetailHasUnsavedEdits(), message);
}
function plantDetailPrepareDraft(record) {
...
function plantDetailValidationIssues(record, tab, root) {
    const issues = [];
    const control = (name) => root.querySelector(`[data-plant-edit="${name}"]`);
    ...
    if (tab === 'overview') {
        ...
        if (duplicateName)
            issues.push({ control: control('name'), message: `Another plant already uses the name ${record.name}.` });
        if (duplicateCode)
            issues.push({ control: control('code'), message: `Another plant already uses the code ${record.code}.` });
        if (!FleetClientModel.clients.some(client => client.id === record.clientId))
            issues.push({ control: control('clientId'), message: 'Select a valid client assignment.' });
        ...
        if (!Number.isFinite(dc) || dc <= 0)
            issues.push({ control: control('capacityDc'), message: 'Installed DC capacity must be greater than 0 MWp.' });
        if (!Number.isFinite(ac) || ac <= 0)
            issues.push({ control: control('capacityAc'), message: 'Installed AC capacity must be greater than 0 MW.' });
        if (!Number.isFinite(grid) || grid <= 0)
            issues.push({ control: control('gridCapacity'), message: 'Grid connection capacity must be greater than 0 MW.' });
        if (Number.isFinite(dc) && Number.isFinite(ac) && ac > dc)
            issues.push({ control: control('capacityAc'), message: 'Installed AC capacity cannot exceed installed DC capacity.' });
        if (Number.isFinite(ac) && Number.isFinite(grid) && grid > ac)
            issues.push({ control: control('gridCapacity'), message: 'Grid connection capacity cannot exceed installed AC capacity.' });
        ...
        if (latitude && (!Number.isFinite(Number(latitude)) || Number(latitude) < -90 || Number(latitude) > 90))
            issues.push({ control: control('latitude'), message: 'Latitude must be between -90 and 90.' });
        if (longitude && (!Number.isFinite(Number(longitude)) || Number(longitude) < -180 || Number(longitude) > 180))
            issues.push({ control: control('longitude'), message: 'Longitude must be between -180 and 180.' });
    }
    if (tab === 'adminsync') {
        ...
        if (duplicateExternal)
            issues.push({ control: control('externalId'), message: `External Plant ID ${record.externalId} is already used by another plant.` });
    }
    return issues;
}
function plantDetailNormalizeForSave(record) {
...
function plantDetailUpdatePayload(record) {
    ...
    const clean = (value) => { const text = String(value ?? '').trim(); return text === '—' ? '' : text; };
    const payload = { plantName: clean(record.name), plantCode: clean(record.code), clientId: clean(record.clientId), plantType: clean(record.type), countryRegion: clean(record.country), plantTimeZone: clean(record.timezone) };
    const optional = (key, value) => { if (value === undefined || value === null || value === '')
    ...
    const raw = record.raw || {};
    const rawLocation = raw.location && typeof raw.location === 'object' && !Array.isArray(raw.location) ? raw.location : {};
    const rawTechnical = raw.technical && typeof raw.technical === 'object' && !Array.isArray(raw.technical) ? raw.technical : {};
    const rawCommercial = raw.commercial && typeof raw.commercial === 'object' && !Array.isArray(raw.commercial) ? raw.commercial : {};
    const location = { ...rawLocation };
    const setLocation = (key, value) => { const text = clean(value); if (text)
        location[key] = text; };
    ...
    const latText = clean(record.latitude), lngText = clean(record.longitude);
    if (latText)
        location.lat = latText;
    if (lngText)
        location.lng = lngText;
    if (!clean(location.coordinates) && latText && lngText)
        location.coordinates = `${latText}, ${lngText}`;
    if (!clean(location.mapRef) && clean(record.address))
        location.mapRef = clean(record.address);
    optional('location', location);
    optional('technical', { ...rawTechnical });
    optional('commercial', { ...rawCommercial });
    ...
    const latitude = Number(clean(record.latitude)), longitude = Number(clean(record.longitude));
    if (clean(record.latitude) && Number.isFinite(latitude))
        optional('latitude', latitude);
    if (clean(record.longitude) && Number.isFinite(longitude))
        optional('longitude', longitude);
    optional('batteryInstalled', clean(record.battery));
    if (Number.isFinite(dcMw)) {
        payload.installedCapacityDcMw = dcMw;
        payload.installedPowerKw = dcMw * 1000;
    }
    if (Number.isFinite(acMw))
        payload.installedCapacityAcMw = acMw;
    if (Number.isFinite(gridMw))
        payload.gridConnectionCapacityMw = gridMw;
    return payload;
}
function plantDetailLifecycleHtml(record) {
    if (!plantDetailBackendManaged(record))
        return '<div class="empty-state"><strong>Lifecycle requires a Plant Registry record</strong><small>Backend lifecycle actions are available only when the plant is linked to /api/admin/plants.</small></div>';
    const status = String(record.status || '').toLowerCase();
    if (status === 'archived')
        return '<div class="empty-state"><strong>Plant archived</strong><small>This plant is read-only.</small></div>';
...
function plantDetailSourceChanged(base, draft) {
    return ['sourceSystem', 'integration', 'externalId'].some(key => String(base[key] || '') !== String(draft[key] || ''));
}
function plantDetailAssignmentChanged(base, draft) {
    return base.clientId !== draft.clientId || String(base.operator || '') !== String(draft.operator || '');
}
function syncPlantClientAssignments(base, changed) {
    FleetClientModel.clients.forEach(client => {
        const values = Array.isArray(client.plants) ? client.plants.filter(id => id !== base.id) : [];
        if (client.id === changed.clientId && !values.includes(changed.id))
            values.push(changed.id);
        client.plants = values;
    });
}
function updatePlantDetailActions(record) {
    const edit = document.getElementById('editPlantTab');
    const cancel = document.getElementById('cancelPlantEdit');
    const save = document.getElementById('savePlantEdit');
    ...
    if (cancel)
        cancel.hidden = !plantDetailEditMode;
    if (save)
        save.hidden = !plantDetailEditMode;
}
function renderPlantDetailCurrentTab(baseRecord, devices) {
    ...
    if (content)
        content.innerHTML = plantDetailEditMode ? plantDetailEditTab(record, plantDetailActiveTab) : plantTab(record, devices, plantDetailActiveTab);
    const title = document.getElementById('plantDetailActiveTitle');
    if (title)
        title.textContent = plantDetailSectionTitle(plantDetailActiveTab);
    updatePlantDetailActions(baseRecord);
    FleetLayout.enhanceActionMenus?.(content);
}
function setPlantDetailEditMode(enabled, baseRecord, devices) {
    if (plantDetailBusy)
        return;
    ...
    if (!enabled && plantDetailEditMode && !plantDetailConfirmDiscard())
        return;
...
async function savePlantDetailEdits(baseRecord, devices) {
    if (!plantDetailEditMode || !plantDetailDraft || plantDetailBusy)
        return;
    const backendManaged = plantDetailBackendManaged(baseRecord);
    if (!FleetActionPermissions.guard({ action: 'edit', resource: 'plant', record: baseRecord, status: baseRecord.status, origin: plantDetailOrigin(baseRecord), updateAvailable: backendManaged, localOverride: !backendManaged }))
        return;
    ...
    const content = document.getElementById('plantTabContent'), summary = document.getElementById('plantDetailEditSummary');
    if (!content)
        return;
    const validation = FleetFormUX.validate(content, plantDetailValidationIssues(plantDetailDraft, plantDetailActiveTab, content), summary, 'Plant changes were not saved');
    if (!validation.valid) {
        FleetFormUX.focusFirst(validation, summary);
        return;
    }
    const sourceChanged = plantDetailSourceChanged(baseRecord, plantDetailDraft), assignmentChanged = plantDetailAssignmentChanged(baseRecord, plantDetailDraft);
    if (sourceChanged && !window.confirm('Source System, Integration or External Plant ID changed. Save these mapping changes?'))
        return;
    if (assignmentChanged && !window.confirm('Client or Managing Tenant assignment changed. Save this assignment update?'))
        return;
    const button = document.getElementById('savePlantEdit');
    ...
    try {
        ...
        if (backendManaged) {
            const result = await window.FleetAPIMutations.plants.update(plantDetailAdminId(baseRecord), plantDetailUpdatePayload(normalized));
            if (!result.ok)
        ...
        if (button)
            FleetFormUX.setBusy(button, false);
        FleetFormUX.renderSummary(summary, [{ message: error instanceof Error ? error.message : 'Unable to save the plant.' }], 'Plant changes were not saved');
        summary?.focus();
    }
}
function plantTelemetryRecords(record) {
    const store = window.ZentridLiveTelemetryByPlant;
    if (!store)
        return Array.isArray(record.relatedTelemetry) ? record.relatedTelemetry : [];
...
function plantTelemetryLoaded(record) {
    if (Boolean(record.telemetryLoaded))
        return true;
    const loaded = window.ZentridLiveTelemetryLoadedPlants;
    if (!loaded)
        return false;
...
function plantTelemetryMetricToken(value) {
    return String(value || '').trim().toLowerCase().replace(/[^a-z0-9]+/g, '');
}
function plantTelemetryTimestamp(record) {
    const value = record.timestampRaw || record.timestamp;
    const timestamp = Date.parse(String(value || ''));
    return Number.isFinite(timestamp) ? timestamp : 0;
}
function plantTelemetryMetricValue(record, aliases) {
...
function plantTelemetrySummary(record) {
...
function plantTelemetryTable(record) {
    if (!plantTelemetryLoaded(record))
        return '<div class="plant-data-state-v119 empty"><strong>Telemetry not loaded yet</strong><small>Open this section to query /api/telemetry for the selected plant.</small></div>';
    const rows = plantTelemetryRecords(record)
        .slice()
        .sort((a, b) => plantTelemetryTimestamp(b) - plantTelemetryTimestamp(a))
        .slice(0, 20);
    if (!rows.length)
        return '<div class="empty-state plant-empty-state-v119"><strong>No matching telemetry records</strong><small>The telemetry request completed, but the current page contained no records linked to this plant.</small></div>';
...
function plantTelemetryState(record) {
    if (plantTelemetryLoaded(record)) {
        const summary = plantTelemetrySummary(record);
        if (!summary.count)
            return { kind: 'empty', title: 'No telemetry available', message: 'The loaded telemetry page returned no records matching this plant.' };
        if (/stale|delay|partial|invalid|error|fault|missing|unavailable/i.test(summary.quality))
            return { kind: 'partial', title: 'Telemetry may be incomplete', message: `${summary.count} matching API record(s) were loaded, but their quality state requires attention.` };
    ...
    if (!hasPower && !hasEnergy)
        return { kind: 'empty', title: 'No telemetry available', message: plantDetailOrigin(record) === 'local' ? 'This locally created plant has no telemetry source yet.' : 'Current power and energy values are unavailable for this record.' };
    if (String(record.health || '').toLowerCase().includes('fault') || plantDetailFreshness(record).toLowerCase().includes('unavailable'))
        return { kind: 'partial', title: 'Telemetry may be incomplete', message: 'Some operational values may be delayed or unavailable. Review Source & Sync before relying on the trend.' };
    return { kind: 'ready', title: 'Telemetry available', message: 'Current and period energy values are available for this plant.' };
}
function plantDevicesLoaded(record) { return Boolean(record.devicesLoaded); }
function plantAlertsLoaded(record) { return Boolean(record.alertsLoaded); }
function plantDeviceCountLabel(record, devices) {
    return plantDevicesLoaded(record) ? String(devices.length) : 'On demand';
}
function plantAlertCountLabel(record) {
    if (!plantAlertsLoaded(record))
        return 'On demand';
    return String(Array.isArray(record.relatedAlerts) ? record.relatedAlerts.length : 0);
}
function deviceRows(items, record) {
    if (!items.length)
        return `<div class="empty-state plant-empty-state-v119"><strong>No device records</strong><small>${record && plantDetailOrigin(record) === 'local' ? 'This local plant has no device onboarding records yet.' : 'No devices were returned for this plant in the current hierarchy model.'}</small></div>`;
...
function plantLazyTab(tab, content) {
    return window.FleetDetailLazyTabs?.panel('plant', tab, content) || content;
}
function plantDetailStructureTree(plant, devices) {
    if (!devices.length)
        return `<div class="empty-state plant-empty-state-v119"><strong>Topology not available</strong><small>No devices were returned for this plant.</small></div>`;
...
function plantDetailAlertRecords(plant) {
    return Array.isArray(plant.relatedAlerts) ? plant.relatedAlerts.filter(row => row && row.dataOrigin === 'live') : [];
}
function plantDetailAlertsTable(plant) {
    const alerts = plantDetailAlertRecords(plant);
    if (!alerts.length)
        return `<div class="empty-state plant-empty-state-v119"><strong>No alert records returned</strong><small>The current API response contains no alerts linked to this plant.</small></div>`;
...
function plantDetailActivityRows(plant) {
    ...
    if (!rows.length) {
        const adminRecord = raw.adminRecord && typeof raw.adminRecord === 'object' ? raw.adminRecord : {};
        ...
        if (created)
            rows.push({ time: created, text: 'Plant record created' });
        if (updated && updated !== created)
            rows.push({ time: updated, text: 'Plant record updated' });
    }
    return rows;
}
function plantTab(plant, devices, tab) {
    ...
    if (activeTab === 'structure')
        return plantLazyTab(activeTab, `${context}<div class="section-title-v17"><div><h2>Plant Structure</h2><p class="muted">Hierarchical plant tree. This is the bridge between the plant and physical devices.</p></div></div>${plantDetailStructureTree(plant, devices)}`);
    if (activeTab === 'energy') {
    ...
    if (activeTab === 'alerts')
        return plantLazyTab(activeTab, `${context}<div class="section-title-v17"><div><h2>Alerts & Events</h2><p class="muted">Plant-level incident entry point with severity and affected device context.</p></div></div><div class="info-grid"><div><span>Open Alerts</span><strong>${plantAlertCountLabel(plant)}</strong></div><div><span>Health</span><strong>${plantDetailEscape(plant.health)}</strong></div><div><span>Primary Scope</span><strong>Plant / Device</strong></div><div><span>Workflow</span><strong>Alert → SOP → Task</strong></div></div>${plantDetailAlertsTable(plant)}`);
    if (activeTab === 'device')
        return plantLazyTab(activeTab, `${context}<div class="section-title-v17"><div><h2>Devices & Device</h2><p class="muted">Full device registry for this plant. Use specific tabs for focused views.</p></div><span class="badge neutral">${plantDeviceCountLabel(plant, devices)} records</span></div>${deviceRows(devices, plant)}`);
    if (activeTab === 'arrays')
        return `${context}<div class="section-title-v17"><div><h2>Arrays & Strings</h2><p class="muted">PV module and string values returned for the selected plant.</p></div></div><div class="info-grid"><div><span>Panels</span><strong>${Number(plant.panels || 0).toLocaleString()}</strong></div><div><span>Strings</span><strong>${plant.strings}</strong></div><div><span>Associated Inverters</span><strong>${plant.inverters}</strong></div><div><span>Traceability</span><strong>${devices.length ? 'Derived from linked device records' : 'No linked device records'}</strong></div></div>`;
    if (activeTab === 'metering')
        return plantLazyTab(activeTab, `${context}<div class="section-title-v17"><div><h2>Metering & Grid</h2><p class="muted">Metering points, transformers, switchgear, grid interface and weather context.</p></div></div>${deviceRows(devices.filter(d => d.type === 'Meter' || d.type === 'Grid Device' || d.type === 'Switchgear' || d.type === 'Weather Station'), plant)}`);
...
function plantDetailText(value, fallback = '—') {
    const text = String(value ?? '').trim();
    return text || fallback;
}
function normalizePlantDetailRecord(source) {
    if (!source || source.dataOrigin !== 'live')
        return null;
...
function selectedTenantPlantForDetail() {
    const queryId = new URLSearchParams(location.search).get('id');
    ...
    if (index >= 0)
        FleetClientModel.plants[index] = record;
    else
        FleetClientModel.plants.unshift(record);
...
function renderPlantDetailPage() {
    const requestedEditTab = localStorage.getItem('zentrid_plant_detail_edit');
    if (requestedEditTab && ['overview', 'adminsync'].includes(requestedEditTab))
        plantDetailActiveTab = requestedEditTab;
    if (requestedEditTab)
        localStorage.removeItem('zentrid_plant_detail_edit');
  ...
  `);
    ...
    document.getElementById('backToClient')?.addEventListener('click', () => {
        if (!plantDetailConfirmDiscard('Discard unsaved plant changes and return to Client Detail?'))
            return;
        if (client) {
            FleetClientModel.selectClient(client.id);
    ...
    document.getElementById('backToPlantRegistry')?.addEventListener('click', () => {
        if (!plantDetailConfirmDiscard('Discard unsaved plant changes and return to Plant Registry?'))
            return;
    ...
    document.querySelectorAll('[data-plant-tab]').forEach(btn => btn.addEventListener('click', () => {
        const nextTab = (btn.dataset.plantTab || 'overview');
        if (plantDetailEditMode && !plantDetailConfirmDiscard('Discard unsaved changes and open another plant section?'))
            return;
        plantDetailEditMode = false;
        plantDetailDraft = null;
        plantDetailEditSnapshot = '';
        ...
        document.querySelectorAll('[data-plant-tab]').forEach(item => {
    ...
    plantTabContent?.addEventListener('click', event => {
        ...
        if (!(target instanceof Element))
            return;
        const lifecycle = target.closest('[data-plant-lifecycle]');
        const downloadDocument = target.closest('[data-plant-document-download]');
        const deleteDocument = target.closest('[data-plant-document-delete]');
        if (lifecycle?.dataset.plantLifecycle) {
            const action = lifecycle.dataset.plantLifecycle;
            const run = window.FleetAPIMutations?.plants?.[action];
            if (!run)
                return;
            if (!FleetActionPermissions.guard({ action, resource: 'plant', record: plant, status: plant.status, origin: plantDetailOrigin(plant), updateAvailable: true }))
                return;
            const prompt = action === 'archive' ? `Archive ${plant.name}?` : action === 'deactivate' ? `Deactivate ${plant.name}?` : '';
            if (prompt && !window.confirm(prompt))
                return;
            void run(plantDetailAdminId(plant)).then(result => { FleetLayout.toast(result.message); if (result.ok)
                window.setTimeout(() => window.location.reload(), 250); });
            return;
        }
        if (downloadDocument?.dataset.plantDocumentDownload) {
            const documentId = downloadDocument.dataset.plantDocumentDownload;
            const doc = plantDetailDocuments(plant).find(item => String(item.id || item.documentId || '') === documentId);
            const fileName = String(doc?.fileName || doc?.filename || doc?.name || `plant-document-${documentId}`).trim() || `plant-document-${documentId}`;
            void window.ZentridPlatformAPI.plantRegistry.getDocument(plantDetailAdminId(plant), documentId).then((payload) => { if (resolveBackendDocumentPayload(payload, fileName))
                FleetLayout.toast('Plant document download started.');
            else
                FleetLayout.toast('Unable to convert the backend document response into a downloadable file.'); }).catch(() => FleetLayout.toast('Unable to download plant document.'));
            return;
        }
        if (deleteDocument?.dataset.plantDocumentDelete) {
            if (!FleetActionPermissions.guard({ action: 'edit', resource: 'plant', record: plant, status: plant.status, origin: plantDetailOrigin(plant), updateAvailable: true }))
                return;
            if (!window.confirm('Delete this plant document from the backend?'))
                return;
            void window.FleetAPIMutations.plants.deleteDocument(plantDetailAdminId(plant), deleteDocument.dataset.plantDocumentDelete).then(result => { FleetLayout.toast(result.message); if (result.ok)
                window.setTimeout(() => window.location.reload(), 250); });
            return;
        }
        const open = target.closest('[data-open-device]');
        const history = target.closest('[data-device-history]');
        const openDeviceId = open?.dataset.openDevice;
        const historyDeviceId = history?.dataset.deviceHistory;
        if ((openDeviceId || historyDeviceId) && !plantDetailConfirmDiscard('Discard unsaved plant changes and open Device Detail?'))
            return;
        if (openDeviceId) {
        ...
        if (historyDeviceId) {
            ...
            location.href = FleetLayout.pathFor('device-detail') + '#activity';
        }
    });
    const syncPlantField = (target) => {
        if (!plantDetailDraft)
            return;
        const field = target.dataset.plantEdit;
        if (field) {
            if (field === 'panels' || field === 'inverters' || field === 'strings' || field === 'transformers' || field === 'meters' || field === 'alerts')
                plantDetailDraft[field] = Number(target.value);
            else
                plantDetailDraft[field] = target.value;
            if (field === 'clientId') {
                const selectedClient = FleetClientModel.clients.find(item => item.id === target.value);
                if (selectedClient)
                    plantDetailDraft.owner = selectedClient.name;
            }
            if (field === 'sourceSystem' || field === 'integration' || field === 'externalId')
                setPlantDetailFeedback('warning', 'Local mapping change', 'Changing source identity may affect mapping traceability. The change will require confirmation before saving.');
        }
        const summary = document.getElementById('plantDetailEditSummary');
        if (plantDetailEditMode)
            FleetFormUX.clearErrors(plantTabContent || document, summary);
    };
    plantTabContent?.addEventListener('input', event => {
        const target = event.target;
        if (target instanceof HTMLInputElement || target instanceof HTMLSelectElement || target instanceof HTMLTextAreaElement)
            syncPlantField(target);
    });
    ...
    plantTabContent?.addEventListener('submit', event => {
        ...
        if (!(form instanceof HTMLFormElement) || form.id !== 'plantDocumentUploadForm')
            return;
        event.preventDefault();
        if (!plantDetailBackendManaged(plant))
            return;
        if (!FleetActionPermissions.guard({ action: 'edit', resource: 'plant', record: plant, status: plant.status, origin: plantDetailOrigin(plant), updateAvailable: true }))
            return;
        const fd = new FormData(form);
        ...
        if (!(file instanceof File) || !file.name) {
            FleetLayout.toast('Choose a document file first.');
            return;
        }
        void window.FleetAPIMutations.plants.uploadDocument(plantDetailAdminId(plant), fd).then(result => { FleetLayout.toast(result.message); if (result.ok)
            window.setTimeout(() => window.location.reload(), 250); });
    });
    if (requestedEditTab && plantDetailCanEdit(plant, requestedEditTab))
        setPlantDetailEditMode(true, plant, devices);
...
function renderDeviceDetailPage() {
  ...
  `);
    ...
    document.querySelectorAll('[data-device-tab]').forEach(btn => btn.addEventListener('click', () => {
        ...
        if (deviceTabContent)
            deviceTabContent.innerHTML = deviceDetailTab(device, plant, siblings, btn.dataset.deviceTab || 'overview');
    }));
    if (location.hash === '#activity')
        document.querySelector('[data-device-tab="activity"]')?.click();
    deviceTabContent?.addEventListener('click', e => {
        const target = e.target;
        if (!(target instanceof Element))
            return;
        const stringBtn = target.closest('[data-open-string]');
        const stringName = stringBtn?.dataset.openString;
        if (stringName) {
            openStringDrawer(device, stringName, stringBtn.dataset.parentMppt || 'MPPT 1');
            return;
        }
        const mpptBtn = target.closest('[data-open-mppt]');
...
function meterDetailTab(device, plant, siblings, tab) {
    ...
    if (tab === 'accounting')
        return `<div class="section-title-v17"><div><h2>Energy Accounting</h2><p class="muted">Confirmed import/export records used for reporting, billing and reconciliation.</p></div></div>${meterAccountingRows(device)}
    ...
    if (tab === 'importExport')
        return `<div class="section-title-v17"><div><h2>Import / Export</h2><p class="muted">Grid exchange view for this metering point.</p></div></div>
    ...
    if (tab === 'tariffs')
        return `<div class="section-title-v17"><div><h2>Tariffs</h2><p class="muted">Commercial profile linked to this meter for revenue, settlements and billing.</p></div></div>
    ...
    if (tab === 'alerts')
        return `<div class="section-title-v17"><div><h2>Alerts</h2><p class="muted">Metering issues usually affect accounting, reporting and billing confidence.</p></div></div>
    ...
    if (tab === 'topology')
        return `<div class="section-title-v17"><div><h2>Topology</h2><p class="muted">Meter belongs to the plant metering/grid layer, not under an inverter.</p></div></div>
...
function bessDetailTab(device, plant, siblings, tab) {
    ...
    if (tab === 'soc')
        return `<div class="section-title-v17"><div><h2>SOC / SOH</h2><p class="muted">Battery health and availability overview. SOC is operational; SOH is lifecycle condition.</p></div></div>
    ...
    if (tab === 'charge')
        return `<div class="section-title-v17"><div><h2>Charge / Discharge</h2><p class="muted">Current storage operating mode and power direction. Commands are capability-gated and audited.</p></div></div>
    ...
    if (tab === 'racks')
        return `<div class="section-title-v17"><div><h2>Racks / Modules</h2><p class="muted">BESS internal topology. Racks contain modules, modules contain monitored cell groups.</p></div></div>
    ...
    if (tab === 'alerts')
        return `<div class="section-title-v17"><div><h2>Alerts</h2><p class="muted">Battery and PCS events connected to SOP, tasks and work orders.</p></div></div>
    ...
    if (tab === 'topology')
        return `<div class="section-title-v17"><div><h2>Topology</h2><p class="muted">Storage hierarchy and plant-level sibling devices.</p></div></div>
...
function pcsDetailTab(device, plant, siblings, tab) {
    ...
    const hasIssue = device.status === 'Warning' || device.status === 'Fault';
    if (tab === 'conversion')
        return `<div class="section-title-v17"><div><h2>Power Conversion</h2><p class="muted">PCS converts battery DC energy to AC grid/load energy and AC energy back to DC during charge.</p></div></div>${pcsMetricCards(device)}
...
function weatherDetailTab(device, plant, siblings, tab) {
    ...
    if (tab === 'irradiance')
        return `<div class="section-title-v17"><div><h2>Irradiance</h2><p class="muted">Solar resource context used to explain production and performance ratio.</p></div></div>
    ...
    if (tab === 'temperature')
        return `<div class="section-title-v17"><div><h2>Temperature</h2><p class="muted">Ambient and module temperature readings used for diagnostics and performance interpretation.</p></div></div>
    ...
    if (tab === 'wind')
        return `<div class="section-title-v17"><div><h2>Wind</h2><p class="muted">Wind conditions can support safety, soiling context and weather normalization.</p></div></div>
    ...
    if (tab === 'sensors')
        return `<div class="section-title-v17"><div><h2>Sensors</h2><p class="muted">Weather plant child sensors. These are subcomponents of the plant, not independent plant devices.</p></div></div>${weatherSensorRows(device)}`;
    if (tab === 'alerts')
        return `<div class="section-title-v17"><div><h2>Alerts</h2><p class="muted">Weather plant alerts affect performance interpretation and data quality confidence.</p></div></div>
    ...
    if (tab === 'topology')
        return `<div class="section-title-v17"><div><h2>Topology</h2><p class="muted">Weather plant belongs to the plant sensor layer and provides context to energy analytics.</p></div></div>
    ...
    if (tab === 'source')
        return `<div class="section-title-v17"><div><h2>Source & Sync</h2><p class="muted">Connector lineage and freshness for weather telemetry.</p></div></div><div class="info-grid"><div><span>Source System</span><strong>${device.vendor} weather connector</strong></div><div><span>Source Entity ID</span><strong>${device.serial}</strong></div><div><span>Zentrid Device ID</span><strong>${device.id}</strong></div><div><span>Last Seen</span><strong>${device.lastSeen}</strong></div><div><span>Mapping Status</span><strong>Mapped to canonical weather station</strong></div><div><span>Data Freshness</span><strong>${m.freshness}</strong></div><div><span>Capability Flags</span><strong>Irradiance · Temperature · Wind · Humidity</strong></div><div><span>Raw Payload</span><strong>Available in Data Governance</strong></div></div>`;
    if (tab === 'activity')
        return `<div class="section-title-v17"><div><h2>Activity</h2><p class="muted">Weather data imports, sensor checks and data-quality events.</p></div></div><div class="timeline-v17"><div><b>Today</b><span>Weather telemetry normalized for ${plant.name}</span></div><div><b>Today</b><span>${hasIssue ? 'Weather feed delay flagged for data quality review' : 'All weather sensors reporting normally'}</span></div><div><b>Yesterday</b><span>Irradiance sensor calibration status checked</span></div><div><b>03 Jun</b><span>Weather plant linked to plant performance analytics</span></div></div>`;
...
function loggerLinkedRows(device, plant, siblings) {
    ...
    if (!linked.length)
        return `<div class="empty-state"><strong>No linked devices</strong><small>This gateway has no sample children in the current plant model.</small></div>`;
...
function loggerDetailTab(device, plant, siblings, tab) {
    ...
    const hasIssue = device.status === 'Warning' || device.status === 'Offline' || device.status === 'Fault';
    if (tab === 'connectivity')
        return `<div class="section-title-v17"><div><h2>Connectivity</h2><p class="muted">Gateway health, network quality and communication stability.</p></div></div>${loggerMetricCards(device)}
    ...
    if (tab === 'linked')
        return `<div class="section-title-v17"><div><h2>Linked Devices</h2><p class="muted">Devices whose telemetry is collected through this logger or gateway.</p></div></div>${loggerLinkedRows(device, plant, siblings)}`;
    if (tab === 'sync')
        return `<div class="section-title-v17"><div><h2>Sync & Freshness</h2><p class="muted">Data freshness across child devices and connector stages.</p></div></div>
    ...
    if (tab === 'network')
        return `<div class="section-title-v17"><div><h2>Network</h2><p class="muted">Communication channel, addressing and network diagnostics.</p></div></div><div class="info-grid"><div><span>Network Type</span><strong>Ethernet + RS485</strong></div><div><span>IP Address</span><strong>10.24.${m.linked % 20}.14</strong></div><div><span>Subnet</span><strong>Plant OT Network</strong></div><div><span>Signal</span><strong>${m.signal}</strong></div><div><span>Protocol</span><strong>${m.protocol}</strong></div><div><span>VPN / Tunnel</span><strong>${hasIssue ? 'Reconnecting' : 'Connected'}</strong></div></div>`;
    if (tab === 'alerts')
        return `<div class="section-title-v17"><div><h2>Alerts</h2><p class="muted">Gateway issues can impact many downstream devices, so they should be visible and prioritized.</p></div></div>
    ...
    if (tab === 'topology')
        return `<div class="section-title-v17"><div><h2>Topology</h2><p class="muted">Logger / Gateway sits between plant device and the vendor connector. It collects child-device data but does not own the electrical hierarchy.</p></div></div>
    ...
    if (tab === 'source')
        return `<div class="section-title-v17"><div><h2>Source & Sync</h2><p class="muted">Connector lineage for gateway and child-device payloads.</p></div></div><div class="info-grid"><div><span>Source System</span><strong>${device.vendor} gateway connector</strong></div><div><span>Source Entity ID</span><strong>${device.serial}</strong></div><div><span>Zentrid Device ID</span><strong>${device.id}</strong></div><div><span>Last Seen</span><strong>${device.lastSeen}</strong></div><div><span>Mapping Status</span><strong>Mapped to canonical logger / gateway</strong></div><div><span>Data Freshness</span><strong>${m.freshness}</strong></div><div><span>Raw Payload</span><strong>Available in Data Governance</strong></div><div><span>Capability Flags</span><strong>Polling · Child devices · Sync trace · Alerts</strong></div></div>`;
    if (tab === 'activity')
        return `<div class="section-title-v17"><div><h2>Activity</h2><p class="muted">Gateway sync, child-device polling, network and governance timeline.</p></div></div><div class="timeline-v17"><div><b>Today</b><span>Gateway payload normalized for ${plant.name}</span></div><div><b>Today</b><span>${hasIssue ? 'Sync delay detected across linked devices' : 'All linked devices reporting within freshness threshold'}</span></div><div><b>Yesterday</b><span>Communication map refreshed from source connector</span></div><div><b>03 Jun</b><span>Gateway linked to plant OT network and Data Governance lineage</span></div></div>`;
...
function transformerDetailTab(device, plant, siblings, tab) {
    ...
    const issue = device.status === 'Warning' || device.status === 'Fault' || device.status === 'Offline';
    if (tab === 'measurements')
        return `<div class="section-title-v17"><div><h2>Electrical Measurements</h2><p class="muted">Transformer electrical state between plant generation/storage and the grid interface.</p></div></div>${transformerMetricCards(device)}${transformerMeasurementRows(device)}`;
    if (tab === 'protection')
        return `<div class="section-title-v17"><div><h2>Protection Events</h2><p class="muted">Relay, trip, thermal and insulation events. These should feed Alerts, SOP and Work Orders.</p></div></div>${transformerProtectionRows(device)}`;
    if (tab === 'grid')
        return `<div class="section-title-v17"><div><h2>Grid / Feeders</h2><p class="muted">Transformer relation to switchgear, feeders, meters and the plant AC bus.</p></div></div>
    ...
    if (tab === 'alerts')
        return `<div class="section-title-v17"><div><h2>Alerts</h2><p class="muted">Transformer events connected to incident, SOP and work-order flows.</p></div></div><div class="data-table compact-table device-alert-table-v19"><div class="data-head"><span>Alert</span><span>Severity</span><span>Time</span><span>Status</span><span>Action</span></div><div class="data-row"><div><strong>${issue ? 'Transformer temperature / oil status check' : 'No active transformer issues'}</strong><small>${device.name}</small></div><div><span class="badge ${issue ? 'warning' : 'success'}">${issue ? 'Warning' : 'Normal'}</span></div><div><span>${issue ? device.lastSeen : 'Now'}</span></div><div><span>${issue ? 'Open' : 'Clear'}</span></div><div><button class="small-btn" type="button">${issue ? 'Open Alert' : 'View History'}</button></div></div></div>`;
    if (tab === 'topology')
        return `<div class="section-title-v17"><div><h2>Topology</h2><p class="muted">Transformer position in the plant electrical infrastructure.</p></div></div><div class="topology-path-v20"><span>Plant</span><b>→</b><span>AC Collection</span><b>→</b><span>${device.name}</span><b>→</b><span>Switchgear / Grid</span></div><div class="asset-tree-v17"><div>Plant · ${plant.name}</div><ul><li>Generation Side<ul><li>Inverters</li><li>PCS / BESS if available</li></ul></li><li>Electrical Infrastructure<ul><li>${device.type} · ${device.name}<ul><li>HV side</li><li>LV side</li><li>Protection relay</li><li>Feeders</li></ul></li><li>Switchgear / Breaker</li><li>Metering point</li></ul></li></ul></div>${deviceSiblingList(device, siblings)}`;
    if (tab === 'passport')
    ...
    if (tab === 'source')
        return `<div class="section-title-v17"><div><h2>Source & Sync</h2><p class="muted">Connector lineage for transformer operational state and protection events.</p></div></div><div class="info-grid"><div><span>Source System</span><strong>${device.vendor} / plant SCADA</strong></div><div><span>Source Entity ID</span><strong>${device.serial}</strong></div><div><span>Zentrid Device ID</span><strong>${device.id}</strong></div><div><span>Last Seen</span><strong>${device.lastSeen}</strong></div><div><span>Mapping Status</span><strong>Mapped to canonical transformer</strong></div><div><span>Data Freshness</span><strong>${m.freshness}</strong></div><div><span>Raw Payload</span><strong>Available in Data Governance</strong></div><div><span>Capability Flags</span><strong>Measurements · Protection · Events</strong></div></div>`;
    if (tab === 'activity')
        return `<div class="section-title-v17"><div><h2>Activity</h2><p class="muted">Transformer operational, inspection and protection timeline.</p></div></div><div class="timeline-v17"><div><b>Today</b><span>Transformer measurements normalized for ${plant.name}</span></div><div><b>Today</b><span>${issue ? 'Temperature / oil status check routed to operations' : 'Protection state verified as normal'}</span></div><div><b>Yesterday</b><span>Feeder load snapshot refreshed</span></div><div><b>03 Jun</b><span>Transformer linked to plant infrastructure topology</span></div></div>`;
...
function switchgearDetailTab(device, plant, siblings, tab) {
    ...
    if (tab === 'breakers')
        return `<div class="section-title-v17"><div><h2>Breakers</h2><p class="muted">Breaker and interlock state for plant grid infrastructure.</p></div></div>${switchgearMetricCards(device)}${switchgearBreakerRows(device)}`;
    if (tab === 'feeders')
        return `<div class="section-title-v17"><div><h2>Feeders</h2><p class="muted">Outgoing circuits connected to PV, BESS/PCS, transformer and auxiliary loads.</p></div></div>${switchgearFeederRows(device)}`;
    if (tab === 'events')
        return `<div class="section-title-v17"><div><h2>Protection Events</h2><p class="muted">Trip, relay, interlock and feeder protection events routed to Alerts, SOP and Work Orders.</p></div></div>${switchgearEventRows(device)}`;
    if (tab === 'commands')
        return `<div class="section-title-v17"><div><h2>Commands</h2><p class="muted">Switchgear commands are risky control actions. They must stay capability-gated and audited.</p></div></div>${switchgearCommandRows(device)}`;
    if (tab === 'alerts')
        return `<div class="section-title-v17"><div><h2>Alerts</h2><p class="muted">Switchgear events connected to incident, SOP and work-order flows.</p></div></div><div class="data-table compact-table device-alert-table-v19"><div class="data-head"><span>Alert</span><span>Severity</span><span>Time</span><span>Status</span><span>Action</span></div><div class="data-row"><div><strong>${issue ? 'Switchgear interlock / feeder check' : 'No active switchgear issues'}</strong><small>${device.name}</small></div><div><span class="badge ${issue ? 'warning' : 'success'}">${issue ? 'Warning' : 'Normal'}</span></div><div><span>${issue ? device.lastSeen : 'Now'}</span></div><div><span>${issue ? 'Open' : 'Clear'}</span></div><div><button class="small-btn" type="button">${issue ? 'Open Alert' : 'View History'}</button></div></div></div>`;
    if (tab === 'topology')
        return `<div class="section-title-v17"><div><h2>Topology</h2><p class="muted">Switchgear sits between transformer, feeders, metering point and grid connection.</p></div></div><div class="topology-path-v20"><span>Plant</span><b>→</b><span>Transformer</span><b>→</b><span>${device.name}</span><b>→</b><span>Feeders / Grid</span></div><div class="asset-tree-v17"><div>Plant · ${plant.name}</div><ul><li>Electrical Infrastructure<ul><li>${transformer ? transformer.name : 'Transformer'}<ul><li>${device.type} · ${device.name}<ul><li>Main breaker</li><li>PV feeder</li><li>BESS / PCS feeder</li><li>Aux feeder</li><li>${meter ? meter.name : 'Metering point'}</li></ul></li></ul></li></ul></li></ul></div>${deviceSiblingList(device, siblings)}`;
    if (tab === 'passport')
    ...
    if (tab === 'source')
        return `<div class="section-title-v17"><div><h2>Source & Sync</h2><p class="muted">Connector lineage for switchgear state, breaker events and command capability.</p></div></div><div class="info-grid"><div><span>Source System</span><strong>${device.vendor} / plant SCADA</strong></div><div><span>Source Entity ID</span><strong>${device.serial}</strong></div><div><span>Zentrid Device ID</span><strong>${device.id}</strong></div><div><span>Last Seen</span><strong>${device.lastSeen}</strong></div><div><span>Mapping Status</span><strong>Mapped to canonical switchgear</strong></div><div><span>Data Freshness</span><strong>${m.freshness}</strong></div><div><span>Capability Flags</span><strong>Breakers · Feeders · Protection · Commands</strong></div><div><span>Raw Payload</span><strong>Available in Data Governance</strong></div></div>`;
    if (tab === 'activity')
        return `<div class="section-title-v17"><div><h2>Activity</h2><p class="muted">Switchgear operational, protection and command audit timeline.</p></div></div><div class="timeline-v17"><div><b>Today</b><span>Switchgear state normalized for ${plant.name}</span></div><div><b>Today</b><span>${issue ? 'Feeder / interlock check routed to operations' : 'Breaker state verified as normal'}</span></div><div><b>Yesterday</b><span>Feeder load snapshot refreshed</span></div><div><b>03 Jun</b><span>Switchgear linked to transformer and grid topology</span></div></div>`;
...
function deviceDetailTab(device, plant, siblings, tab) {
    ...
    if (device.type === 'Meter')
        return meterDetailTab(device, plant, siblings, tab);
    if (device.type === 'PCS')
        return pcsDetailTab(device, plant, siblings, tab);
    if (device.type === 'Battery')
        return bessDetailTab(device, plant, siblings, tab);
    if (device.type === 'Weather Station')
        return weatherDetailTab(device, plant, siblings, tab);
    if (device.type === 'Logger' || device.type === 'Gateway')
        return loggerDetailTab(device, plant, siblings, tab);
    if (isSwitchgearDevice(device))
        return switchgearDetailTab(device, plant, siblings, tab);
    if (isTransformerDevice(device))
        return transformerDetailTab(device, plant, siblings, tab);
    if (tab === 'telemetry')
        return `<div class="section-title-v17"><div><h2>Telemetry</h2><p class="muted">Inverter telemetry split by AC/DC power, voltage, current, frequency and temperature.</p></div></div>
    <div class="telemetry-layout-v20">
    ...
    if (tab === 'alerts')
        return `<div class="section-title-v17"><div><h2>Alerts</h2><p class="muted">Device-level events connected to Alerts, SOP, Tasks and Work Orders.</p></div></div>
    <div class="data-table compact-table device-alert-table-v19"><div class="data-head"><span>Alert</span><span>Severity</span><span>Time</span><span>Status</span><span>Action</span></div>
    ...
    if (tab === 'topology')
        return `<div class="section-title-v17"><div><h2>Topology</h2><p class="muted">Device position inside the plant hierarchy and nearby related device.</p></div></div>
    ...
    if (tab === 'mppt')
        return renderMpptStringsTab(device, warningRow);
    if (tab === 'passport')
...
function openMpptDrawer(device, mpptName) {
    ...
    drawer.querySelectorAll('[data-open-string]').forEach(btn => btn.addEventListener('click', () => {
        const stringName = btn.dataset.openString;
        if (stringName)
            openStringDrawer(device, stringName, btn.dataset.parentMppt || mpptName);
    }));
}
...
function deviceSiblingList(device, siblings) {
    ...
    if (!list.length)
        return `<div class="empty-state"><strong>No sibling device</strong><small>This plant has only one sample device record.</small></div>`;
...
function createClientBuilderPlant() {
    try {
        ...
        const devices = selected.map((x, index) => ({
        ...
        if (window.FleetLocalStore) {
            ...
            const ps = JSON.parse(localStorage.getItem('zentrid_custom_plants') || '[]');
            ps.unshift(plant);
            localStorage.setItem('zentrid_custom_plants', JSON.stringify(ps));
            const ds = JSON.parse(localStorage.getItem('zentrid_custom_devices') || '[]');
            localStorage.setItem('zentrid_custom_devices', JSON.stringify([...devices, ...ds]));
        }
