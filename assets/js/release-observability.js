...
(function () {
    ...
    function createPanel() {
      ...
      <section class="fleet-release-dialog" role="dialog" aria-modal="true" aria-labelledby="fleetReleaseTitle">
        ...
        overlay.querySelectorAll('[data-release-close]').forEach(element => element.addEventListener('click', closePanel));
        overlay.querySelector('[data-release-copy]')?.addEventListener('click', () => { void copySafeReport(); });
        overlay.querySelector('[data-release-download]')?.addEventListener('click', downloadReport);
        overlay.querySelector('[data-release-check]')?.addEventListener('click', () => { void checkForUpdate(true); });
        overlay.querySelector('[data-release-clear]')?.addEventListener('click', () => {
            issues = [];
    ...
    function updatePanel() {
        if (!panel)
            return;
        const summary = panel.querySelector('#fleetReleaseSummary');
        if (summary) {
            const state = health();
            summary.innerHTML = [
        ...
        const report = panel.querySelector('#fleetReleaseReport');
        if (report)
            report.textContent = reportJson();
    }
    function openPanel() {
        ...
        activePanel.querySelector('[data-release-close]')?.focus();
    }
    function closePanel() {
        if (!panel)
            return;
    ...
    async function checkForUpdate(manual = false) {
        ...
        if (!manual && startedAt - lastUpdateCheckAt < UPDATE_CHECK_INTERVAL_MS)
            return false;
        ...
        try {
            ...
            if (manual)
                showNotice('Deployment status could not be checked.');
            return false;
        }
        finally {
            updatePanel();
    ...
    function listenToRuntimeEvents() {
        const names = [
