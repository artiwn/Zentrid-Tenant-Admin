"use strict";
function currentDeviceTenantScope() {
    return String(FleetLayout?.state?.tenant || localStorage.getItem('zentrid_tenant') || 'Tenant workspace').trim() || 'Tenant workspace';
}
function normalizedDeviceTenantToken(value) {
    return String(value || '').trim().toLowerCase().replace(/[^a-z0-9]+/g, '');
}
function deviceBelongsToTenant(device, tenantName = currentDeviceTenantScope()) {
    if (!device.tenant)
        return true;
    return normalizedDeviceTenantToken(device.tenant) === normalizedDeviceTenantToken(tenantName);
}
function normalizeDeviceTenant(device, tenantName = currentDeviceTenantScope()) {
...
function deviceStatusCls(v) { const text = String(v).toLowerCase(); if (text.includes('offline') || text.includes('fault'))
    return 'danger'; if (text.includes('warning') || text.includes('delayed'))
    return 'warning'; return 'success'; }
function deviceStatusPill(d) { return `<span class="badge ${deviceStatusCls(d.status)}">${d.status || 'Unknown'}</span>`; }
function deviceUiFreshness(d) {
...
function deviceFreshnessInline(d) {
...
function deviceRegistryRecords() {
    const tenantName = currentDeviceTenantScope();
    const liveRows = Array.isArray(window.ZentridLiveDevices) ? window.ZentridLiveDevices : [];
    if (!liveRows.length)
        return [];
...
function wireDevices() {
    ...
    function apply(resetPage = true) {
        if (resetPage && !window.FleetRegistryQuery?.pagination('devices'))
            ZentridDevicePager.page = 1;
        ...
        if (type.value !== 'All Types')
            list = list.filter(d => d.type === type.value);
        if (status.value !== 'All Statuses')
            list = list.filter(d => d.status === status.value);
        ...
        if (scope)
            scope.innerHTML = window.FleetRegistryQuery?.filterScopeHtml('devices') || '';
        bindRows();
    }
    function bindRows() { table.querySelectorAll('.data-row').forEach(row => row.querySelectorAll('button').forEach(btn => btn.onclick = () => { const id = row.dataset.id; const d = deviceRegistryRecords().find(x => x.id === id); if (btn.dataset.action === 'open' && id) {
        localStorage.setItem('zentrid_selected_device', id);
        location.href = FleetLayout.pathFor('device-detail');
    } if (btn.dataset.action === 'plant' && d?.plantId) {
        localStorage.setItem('zentrid_selected_plant', d.plantId);
        location.href = FleetLayout.pathFor('plant-detail');
    } if (btn.dataset.action === 'telemetry' && d) {
        localStorage.setItem('zentrid_telemetry_context', JSON.stringify({ tenant: d.tenant, plant: d.plant, device: d.name, metric: 'Current Power', range: localStorage.getItem('zentrid_time') || 'Last 24h', layer: 'Normalized' }));
        location.href = FleetLayout.pathFor('telemetry');
    } if (btn.dataset.action === 'alerts' && d) {
        localStorage.setItem('zentrid_alert_context', JSON.stringify({ deviceId: d.id, plantId: d.plantId, tenant: d.tenant }));
        location.href = FleetLayout.pathFor('alerts');
    } })); table.querySelectorAll('[data-device-page]').forEach(btn => btn.onclick = () => { if (window.FleetRegistryQuery?.pagination('devices'))
        return; ZentridDevicePager.page += btn.dataset.devicePage === 'next' ? 1 : -1; apply(false); }); }
...
function deviceDetailEmptyState(title, note) {
    return `<div class="empty-state"><strong>${title}</strong><small>${note}</small></div>`;
}
function deviceRelatedPlant(d) {
    return d.relatedPlant && typeof d.relatedPlant === 'object' ? d.relatedPlant : null;
}
function deviceRelatedAlerts(d) {
    return Array.isArray(d.relatedAlerts) ? d.relatedAlerts.filter(item => item && typeof item === 'object') : [];
}
function deviceChildRecords(d) {
...
function deviceValue(value, fallback = '—') {
    if (value === null || value === undefined || value === '')
        return fallback;
    const text = String(value).trim();
    return text && text.toLowerCase() !== 'undefined' && text.toLowerCase() !== 'null' ? text : fallback;
}
function deviceEscape(value) {
    return String(value ?? '').replace(/[&<>"']/g, character => ({ '&': '&amp;', '<': '&lt;', '>': '&gt;', '"': '&quot;', "'": '&#039;' }[character] || character));
}
function deviceApiHumanValue(label, value) {
    if (value === null || value === undefined || value === '')
        return '—';
    if (typeof value === 'boolean')
        return value ? 'Yes' : 'No';
    const token = String(label || '').toLowerCase().replace(/[^a-z0-9]+/g, '');
    if ((token.endsWith('connectstatus') || token.endsWith('connectionstatus')) && ['0', '1'].includes(String(value)))
        return String(value) === '1' ? 'Connected' : 'Disconnected';
    if (token.endsWith('onlinestatus') && ['0', '1'].includes(String(value)))
    ...
    if ((token.endsWith('collectiontime') || token.endsWith('collectiontimestamp')) && /^\d{10,13}$/.test(String(value))) {
        ...
        const millis = String(value).length === 13 ? raw : raw * 1000;
...
function deviceApiValueRows(payload, prefix = '', depth = 0) {
    if (payload === null || payload === undefined || depth > 3)
        return [];
    if (Array.isArray(payload))
        return payload.slice(0, 20).flatMap((item, index) => deviceApiValueRows(item, `${prefix}${prefix ? ' · ' : ''}${index + 1}`, depth + 1));
    if (typeof payload !== 'object')
        return [[prefix || 'Value', deviceApiHumanValue(prefix || 'Value', payload)]];
    return Object.entries(payload).flatMap(([key, value]) => {
        const label = `${prefix}${prefix ? ' · ' : ''}${key.replace(/([a-z])([A-Z])/g, '$1 $2')}`;
        if (value !== null && typeof value === 'object')
            return deviceApiValueRows(value, label, depth + 1);
        return [[label, deviceApiHumanValue(label, value)]];
    });
}
function deviceApiPanel(title, payload, emptyText, sourceNote) {
    const rows = deviceApiValueRows(payload).slice(0, 60);
    if (!rows.length)
        return `<div class="empty-state"><strong>${deviceEscape(emptyText)}</strong><small>The endpoint returned no displayable fields.</small></div>`;
...
function deviceLiveId(d) {
    const detail = d.liveDetail && typeof d.liveDetail === 'object' ? d.liveDetail : {};
    return String(d.liveId || detail.deviceId || detail.id || '').trim();
}
function deviceLiveSubresourcePanel(d, title, payload, emptyText, suffix) {
    if (!deviceLiveId(d))
        return `<div class="empty-state"><strong>Platform Live device not linked</strong><small>${deviceEscape(title)} was not requested because the Device Registry record could not be safely matched to a Platform Live device.</small></div>`;
    return deviceApiPanel(title, payload, emptyText, `Platform Live API · GET /api/devices/{liveDeviceId}/${suffix}`);
}
function deviceTelemetryPanel(payload, sourceNote) {
    const rows = deviceApiValueRows(payload).slice(0, 60);
    if (!rows.length)
        return `<div class="empty-state"><strong>No latest telemetry returned</strong><small>The latest telemetry request completed without displayable measurement fields.</small></div>`;
    return deviceApiPanel('Latest Telemetry', payload, 'No latest telemetry returned', sourceNote);
}
function linkedDevicesPanel(payload) {
    ...
    if (!items.length)
        return `<div class="empty-state"><strong>No linked devices returned</strong><small>GET /api/admin/devices/{id}/linked-devices returned an empty collection.</small></div>`;
    return `<div class="data-table compact-table subordinate-device-table-v59"><div class="data-head"><span>Status</span><span>Device Type</span><span>Model</span><span>Software Version</span><span>SN</span></div>${items.map(item => { const row = (item || {}); return `<div class="data-row"><div><span class="badge ${deviceStatusCls(row.status)}">${deviceEscape(row.status || row.operationalStatus || 'Unknown')}</span></div><div><strong>${deviceEscape(row.deviceType || row.type || '—')}</strong></div><div><span>${deviceEscape(row.model || row.name || '—')}</span></div><div><span>${deviceEscape(row.firmwareVersion || row.softwareVersion || '—')}</span></div><div><span>${deviceEscape(row.serialNumber || row.serial || row.id || '—')}</span></div></div>`; }).join('')}</div>`;
}
function telemetryApiSummaryV1(d) {
...
function renderDeviceDetailUnavailable(message = 'The selected device was not returned by the backend API.') {
...
function devicePortalStatusTextV92(d) {
    ...
    if (s.includes('offline') || s.includes('fault'))
        return 'Not visible as healthy in client portal';
    if (s.includes('warning') || s.includes('delayed'))
        return 'Visible with warning note in client portal';
    if (s.includes('online') || s.includes('active') || s.includes('normal'))
...
function deviceDocumentCacheKey(deviceId) { return `zentrid_device_documents_${deviceId}`; }
function readDeviceDocumentCache(deviceId) {
    try {
...
function writeDeviceDocumentCache(deviceId, documents) {
    try {
        sessionStorage.setItem(deviceDocumentCacheKey(deviceId), JSON.stringify(documents));
    }
    catch (_error) { /* best effort */ }
}
function deviceDocuments(d) {
    ...
    return [...apiDocuments, ...cached].reduce((items, raw) => {
        const item = raw;
        const key = String(item.id || item.filePath || item.fileName || '').trim();
        if (!key || items.some(existing => String(existing.id || existing.filePath || existing.fileName) === key))
            return items;
        items.push(item);
        return items;
    }, []);
...
function deviceAuditEntries(payload) {
    if (Array.isArray(payload))
...
function deviceTypeKey(d) {
    ...
    if (t.includes('battery'))
        return 'battery';
    if (t.includes('logger') || t.includes('gateway') || t.includes('communication'))
        return 'logger';
    if (t.includes('meter'))
        return 'meter';
    if (t.includes('weather'))
        return 'weather';
    if (t.includes('pv module') || t.includes('module'))
        return 'module';
    return 'generic';
}
function deviceTypeLabel(d) {
    const map = { inverter: 'Inverter', microinverter: 'Microinverter', battery: 'Battery', logger: 'Logger / Communication', meter: 'Meter', weather: 'Weather Station', module: 'PV Module', generic: String(d.type || 'Device') };
    return map[deviceTypeKey(d)] || String(d.type || 'Device');
}
var ZentridDevicePager = window.ZentridDevicePager || (window.ZentridDevicePager = { page: 1, size: 50 });
let deviceDetailActiveTab = 'overview';
function pageSlice(list, pager) {
...
function pagerHtml(kind, state) {
    if (state.total <= ZentridDevicePager.size)
...
function devicePrimaryMetric(d) {
    ...
    if (k === 'battery')
        return { label: 'SOC / SOH', value: `${deviceMetricValue(d, 'soc')} · ${deviceMetricValue(d, 'soh')}`, hint: 'Battery health' };
    if (k === 'logger')
        return { label: 'Signal / Data Lag', value: `${deviceMetricValue(d, 'signal')} · ${deviceMetricValue(d, 'dataLag')}`, hint: 'Communication health' };
    if (k === 'meter')
        return { label: 'Grid Power', value: deviceMetricValue(d, 'activePower'), hint: 'Accounting point' };
    if (k === 'weather')
        return { label: 'Irradiance', value: deviceMetricValue(d, 'irradiance'), hint: 'Weather telemetry' };
    if (k === 'module')
        return { label: 'Module Power', value: deviceMetricValue(d, 'activePower'), hint: 'Module-level output' };
...
function universalDeviceSidebar(d, activeTab = deviceDetailActiveTab) {
    const key = deviceTypeKey(d);
    ...
    const button = (tab, label) => `<button class="${activeTab === tab ? 'active' : ''}" data-device-tab="${tab}" type="button" ${activeTab === tab ? 'aria-current="page"' : ''}><span>${label}</span></button>`;
...
function operatingDataGrid(d) {
    const key = deviceTypeKey(d);
    if (key === 'logger')
        return cardGrid([['Signal Strength', deviceMetricValue(d, 'signal')], ['WLAN', deviceMetricValue(d, 'wlan')], ['Data Lag', deviceMetricValue(d, 'dataLag')], ['Linked Devices', deviceMetricValue(d, 'linked')], ['LAN IP', deviceMetricValue(d, 'lanIp')], ['Cyber Security Version', deviceMetricValue(d, 'cybersecurity')], ['Status', d.status], ['Last Update', d.lastSeen]]);
    if (key === 'battery')
        return cardGrid([['SOC', deviceMetricValue(d, 'soc')], ['SOH', deviceMetricValue(d, 'soh')], ['Voltage', deviceMetricValue(d, 'voltage')], ['Current', deviceMetricValue(d, 'current')], ['Temperature', deviceMetricValue(d, 'temperature')], ['Rated Capacity', deviceMetricValue(d, 'rated')], ['Charged Today', deviceMetricValue(d, 'charged')], ['Discharged Today', deviceMetricValue(d, 'discharged')]]);
    if (key === 'weather')
        return cardGrid([['Irradiance', deviceMetricValue(d, 'irradiance')], ['Ambient Temp', deviceMetricValue(d, 'ambient')], ['Module Temp', deviceMetricValue(d, 'moduleTemp')], ['Wind Speed', deviceMetricValue(d, 'wind')], ['Humidity', deviceMetricValue(d, 'humidity')], ['Rainfall', deviceMetricValue(d, 'rainfall')]]);
    if (key === 'meter')
        return cardGrid([['Active Power', deviceMetricValue(d, 'activePower')], ['Import Today', deviceMetricValue(d, 'todayImport')], ['Export Today', deviceMetricValue(d, 'todayExport')], ['Voltage', deviceMetricValue(d, 'voltage')], ['Current', deviceMetricValue(d, 'current')], ['Frequency', deviceMetricValue(d, 'frequency')]]);
    if (key === 'module')
        return cardGrid([['Power', deviceMetricValue(d, 'activePower')], ['Voltage', deviceMetricValue(d, 'voltage')], ['Current', deviceMetricValue(d, 'current')], ['Temperature', deviceMetricValue(d, 'temperature')], ['String', deviceMetricValue(d, 'string')], ['MPPT', deviceMetricValue(d, 'mppt')], ['Position', deviceMetricValue(d, 'position')]]);
...
function architectureFlow(d) {
    const nodes = [];
    if (deviceValue(d.plant) !== '—')
        nodes.push(['Plant', deviceValue(d.plant)]);
    if (deviceValue(d.parent) !== '—')
        nodes.push(['Parent', deviceValue(d.parent)]);
    nodes.push([deviceTypeLabel(d), deviceValue(d.name)]);
    if (deviceValue(d.children) !== '—')
        nodes.push(['Children', deviceValue(d.children)]);
...
function stringRows(d) {
    ...
    const vendorExtensions = raw?.vendorExtensions && typeof raw.vendorExtensions === 'object' ? raw.vendorExtensions : null;
    const source = vendorExtensions?.strings || vendorExtensions?.inputs || raw?.strings || raw?.inputs;
    const rows = Array.isArray(source) ? source.filter(item => item && typeof item === 'object') : [];
...
function deviceDetailPanel(d, tab) {
    ...
    if (tab === 'configuration')
        return configurationPanel(d);
    if (tab === 'activity')
        return deviceLazyPanel(tab, `<div class="section-title-v17"><div><h2>Activity Log</h2><p class="muted">Server-recorded device activity from DeviceRegistry.</p></div></div>${deviceAuditTimeline(d.auditDetail)}`);
    if (tab === 'source')
        return `<div class="section-title-v17"><div><h2>Source & Sync</h2><p class="muted">Vendor traceability and canonical mapping state.</p></div></div><div class="info-grid"><div><span>Integration</span><strong>${deviceValue(d.integration)}</strong></div><div><span>Vendor</span><strong>${deviceValue(d.vendor)}</strong></div><div><span>External ID</span><strong>${deviceValue(d.externalId)}</strong></div><div><span>Zentrid ID</span><strong>${deviceValue(d.id)}</strong></div><div><span>Mapping Status</span><strong>${deviceValue(d.sourceStatus)}</strong></div><div><span>Last Seen</span><strong>${deviceValue(d.lastSeen)}</strong></div><div><span>Source System</span><strong>${deviceValue(d.sourceSystem || d.vendor)}</strong></div><div><span>Alarm Status</span><strong>${deviceValue(d.alarmStatus)}</strong></div></div>`;
    if (tab === 'passport')
        return deviceLazyPanel(tab, devicePassportPanelV92(d) + deviceApiPanel('Device Registry Warranty', d.warrantyDetail, 'No Device Registry warranty returned', 'GET /api/admin/devices/{id}/warranty') + deviceLiveSubresourcePanel(d, 'Platform Live Warranty', d.liveWarrantyDetail, 'No Platform Live warranty returned', 'warranty'));
    if (tab === 'connectivity-full')
        return deviceLazyPanel(tab, deviceConnectivityFullPanelV92(d) + deviceApiPanel('Device Registry Connectivity', d.connectivityDetail, 'No Device Registry connectivity returned', 'GET /api/admin/devices/{id}/connectivity') + deviceLiveSubresourcePanel(d, 'Platform Live Connectivity', d.liveConnectivityDetail, 'No Platform Live connectivity returned', 'connectivity') + deviceApiPanel('Device Registry Network', d.networkDetail, 'No Device Registry network returned', 'GET /api/admin/devices/{id}/network') + deviceLiveSubresourcePanel(d, 'Platform Live Network', d.liveNetworkDetail, 'No Platform Live network returned', 'network') + `<div class="section-title-v17 mini"><div><h3>Linked Devices</h3><p class="muted">GET /api/admin/devices/{id}/linked-devices</p></div></div>${linkedDevicesPanel(d.linkedDevices)}`);
    if (tab === 'lifecycle')
        return deviceLazyPanel(tab, lifecyclePanelV92(d) + deviceApiPanel('Device Registry Warranty', d.warrantyDetail, 'No Device Registry warranty returned', 'GET /api/admin/devices/{id}/warranty') + deviceLiveSubresourcePanel(d, 'Platform Live Warranty', d.liveWarrantyDetail, 'No Platform Live warranty returned', 'warranty'));
    if (tab === 'related')
        return deviceLazyPanel(tab, relatedObjectsPanelV92(d));
    if (tab === 'documents')
        return deviceDocumentsPanelV92(d);
    if (tab === 'audit')
        return deviceAuditPanelV92(d);
    return '';
}
function renderDeviceDetail() {
    const d = selectedDevice();
    if (!d)
        return renderDeviceDetailUnavailable();
...
function deviceDocumentFeedback(tone, title, message) {
...
async function downloadDeviceDocument(deviceId, deviceDocument) {
...
function wireDeviceDocuments(d) {
    window.FleetActionPermissions?.refresh(document);
    const form = document.getElementById('deviceDocumentUploadForm');
    form?.addEventListener('submit', async (event) => {
        event.preventDefault();
        if (!window.FleetActionPermissions?.guard({ action: 'document', resource: 'device', record: d, status: d.lifecycle }))
            return;
        const fileInput = document.getElementById('deviceDocumentFile');
        const nameInput = document.getElementById('deviceDocumentName');
        const typeInput = document.getElementById('deviceDocumentType');
        const expiryInput = document.getElementById('deviceDocumentExpiry');
        const file = fileInput?.files?.[0];
        const name = String(nameInput?.value || '').trim();
        if (!file || !name) {
            deviceDocumentFeedback('danger', 'Missing document data', 'Select a file and enter a document name.');
            return;
        }
        ...
        if (expiryInput?.value)
            payload.append('expiry', new Date(expiryInput.value).toISOString());
    ...
    document.querySelectorAll('[data-download-device-document]').forEach(button => button.addEventListener('click', async () => {
        ...
        try {
            await downloadDeviceDocument(d.id, document);
        }
        catch (error) {
            deviceDocumentFeedback('danger', 'Download failed', error instanceof Error ? error.message : String(error));
        }
        finally {
            button.removeAttribute('disabled');
        }
    }));
    document.querySelectorAll('[data-delete-device-document]').forEach(button => button.addEventListener('click', async () => {
        const id = button.dataset.deleteDeviceDocument || '';
        if (!id || !window.FleetActionPermissions?.guard({ action: 'document', resource: 'device', record: d, status: d.lifecycle }))
            return;
        if (!window.confirm('Delete this device document from the backend?'))
            return;
        button.setAttribute('disabled', 'true');
        const result = await window.FleetAPIMutations.devices.deleteDocument(d.id, id);
        if (!result.ok) {
            button.removeAttribute('disabled');
            deviceDocumentFeedback('danger', 'Delete failed', result.message);
            return;
        }
...
async function runDeviceLifecycleAction(d, action) {
    if (!d.id || !window.FleetActionPermissions?.guard({ action, resource: 'device', record: d, status: d.lifecycle }))
        return;
    const label = action === 'activate' ? 'Activate' : action === 'deactivate' ? 'Deactivate' : 'Archive';
    if (action === 'archive' && !window.confirm(`Archive ${deviceValue(d.name)}? The record remains available as Archived.`))
        return;
    const button = document.querySelector(`[data-device-lifecycle-action="${action}"]`);
    button?.setAttribute('disabled', 'true');
...
function wireDeviceDetail() {
    ...
    document.querySelectorAll('[data-device-lifecycle-action]').forEach(button => button.addEventListener('click', () => {
        const action = button.dataset.deviceLifecycleAction;
        if (action === 'activate' || action === 'deactivate' || action === 'archive')
            void runDeviceLifecycleAction(d, action);
    }));
    window.FleetActionPermissions?.refresh(document);
    if (deviceDetailActiveTab === 'documents')
        wireDeviceDocuments(d);
    ...
    document.querySelectorAll('[data-device-tab]').forEach(btn => btn.addEventListener('click', () => {
        ...
        document.querySelectorAll('[data-device-tab]').forEach(item => {
