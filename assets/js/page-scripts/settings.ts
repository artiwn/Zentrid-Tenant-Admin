type TenantSettingsTab = 'overview' | 'profile' | 'branding' | 'localization' | 'notifications' | 'data' | 'security' | 'activity';
type SettingBadgeTone = 'success' | 'warning' | 'danger' | 'info';
type ThemePreset = 'ocean' | 'emerald' | 'violet' | 'graphite';
type TenantSettingsLoadState = 'loading' | 'ready' | 'empty' | 'error';
...
interface TenantBrandSettings {
  displayName: string;
  shortName: string;
  portalSubtitle: string;
  theme: ThemePreset | '';
  status: string;
  showTenantName: boolean | null;
  includeBrandingInReports: boolean | null;
}

interface TenantLocalizationSettings {
  timezone: string;
  language: string;
  currency: string;
  dateFormat: string;
...
interface TenantNotificationSettings {
  ...
  warningAlerts: boolean | null;
  offlineDevices: boolean | null;
  integrationHealth: boolean | null;
  dailySummary: boolean | null;
  invoiceUpdates: boolean | null;
  reportReady: boolean | null;
  accessChanges: boolean | null;
  emailChannel: boolean | null;
  inAppChannel: boolean | null;
}

interface TenantDataSettings {
  ...
  includeQualityFlags: boolean | null;
  useNormalizedData: boolean | null;
...
interface TenantSecuritySettings {
  adminMfa: boolean | null;
  stepUpAuth: boolean | null;
  sessionTimeout: string;
  invitationExpiry: string;
  accessReviewCadence: string;
  externalUserExpiry: string;
  exportReauthentication: boolean | null;
  restrictExternalSharing: boolean | null;
}

interface TenantSettingsEvent {
  id: string;
  event: string;
  area: string;
  description: string;
...
(() => {
  ...
  let activeTab: TenantSettingsTab = 'overview';
  let loadState: TenantSettingsLoadState = 'loading';
  let loadError = '';
  let loadedAt: Date | null = null;

  let profile: TenantProfileSettings = {
  ...
  let branding: TenantBrandSettings = { displayName:'—', shortName:'—', portalSubtitle:'—', theme:'', status:'—', showTenantName:null, includeBrandingInReports:null };
  let localization: TenantLocalizationSettings = { timezone:'—', language:'—', currency:'—', dateFormat:'—', numberFormat:'—', powerUnit:'—', energyUnit:'—', temperatureUnit:'—', irradianceUnit:'—' };
  let notifications: TenantNotificationSettings = { operationalEmail:'—', financeEmail:'—', reportRecipients:'—', criticalAlerts:null, warningAlerts:null, offlineDevices:null, integrationHealth:null, dailySummary:null, invoiceUpdates:null, reportReady:null, accessChanges:null, emailChannel:null, inAppChannel:null };
  let dataSettings: TenantDataSettings = { defaultTimeRange:'—', telemetryGranularity:'—', defaultReportFormat:'—', reportFrequency:'—', reportDelivery:'—', includeRawData:null, includeQualityFlags:null, useNormalizedData:null, completenessThreshold:'—', freshnessThreshold:'—', retentionPolicy:'—', mappingPolicy:'—', storageRegion:'—', sourceSync:'—' };
  let security: TenantSecuritySettings = { adminMfa:null, stepUpAuth:null, sessionTimeout:'—', invitationExpiry:'—', accessReviewCadence:'—', externalUserExpiry:'—', exportReauthentication:null, restrictExternalSharing:null };
  let events: TenantSettingsEvent[] = [];
  ...
  const isRecord = (value: unknown): value is TenantSettingsRecord => Boolean(value && typeof value === 'object' && !Array.isArray(value));
  ...
  const firstOf = (record: TenantSettingsRecord | null, keys: string[], fallback: unknown = undefined): unknown => {
  ...
  const recordOf = (record: TenantSettingsRecord | null, keys: string[]): TenantSettingsRecord | null => {
    if (!record) return null;
    for (const key of keys) if (isRecord(record[key])) return record[key] as TenantSettingsRecord;
    return null;
  };
  const mergeRecords = (...records: Array<TenantSettingsRecord | null>): TenantSettingsRecord => Object.assign({}, ...records.filter(Boolean));
  const bool = (value: unknown): boolean | null => {
    if (typeof value === 'boolean') return value;
    if (typeof value === 'number') return value !== 0;
    if (typeof value !== 'string') return null;
    const normalized = value.trim().toLowerCase();
    if (['true','enabled','active','yes','required','enforced','on'].includes(normalized)) return true;
    if (['false','disabled','inactive','no','optional','off'].includes(normalized)) return false;
    return null;
  };
  const arrayRecords = (record: TenantSettingsRecord | null, keys: string[]): TenantSettingsRecord[] => {
    if (!record) return [];
    for (const key of keys) {
      const value = record[key];
  ...
  const themeValue = (value: unknown): ThemePreset | '' => {
    const candidate = text(value, '').toLowerCase();
    return ['ocean','emerald','violet','graphite'].includes(candidate) ? candidate as ThemePreset : '';
  };
  const tone = (value: string): SettingBadgeTone => {
    const normalized = value.toLowerCase();
    if (normalized === '—' || normalized.includes('unavailable') || normalized.includes('disabled') || normalized.includes('failed') || normalized.includes('blocked')) return 'danger';
    if (normalized.includes('pending') || normalized.includes('review') || normalized.includes('attention')) return 'warning';
    if (normalized.includes('managed') || normalized.includes('read-only') || normalized.includes('info')) return 'info';
    return 'success';
  };
  const badge = (value: string): string => `<span class="badge ${tone(value)}">${esc(value)}</span>`;
  const boolStatus = (value: boolean | null): string => value === true ? 'Enabled' : value === false ? 'Disabled' : '—';
  const known = (value: string): boolean => Boolean(value && value !== '—');

  const mapEvent = (row: TenantSettingsRecord, index: number): TenantSettingsEvent => ({
  ...
  const applyPayload = (payload: unknown): void => {
    if (!isRecord(payload)) throw new Error('Current user response is not an object.');
  ...
  const loadSettings = async (): Promise<void> => {
    loadState = 'loading'; loadError = ''; render();
    try {
      const api = window.ZentridPlatformAPI?.auth?.me || window.ZentridAuth?.me;
      if (typeof api !== 'function') throw new Error(`${SETTINGS_SOURCE} runtime is unavailable.`);
      ...
      <div class="tenant-settings-page-v1437 theme-${esc(branding.theme || 'ocean')}" id="tenantSettingsRoot">
        ...
        <section class="plant-workspace-v17 tenant-settings-workspace-v1437">
          <aside class="glass-card plant-side-card-v17 tenant-settings-side-v1437">
            ...
            ] as Array<[TenantSettingsTab,string]>).map(([key,label]) => `<button type="button" class="${activeTab === key ? 'active' : ''}" data-settings-tab="${key}">${label}</button>`).join('')}
          </aside>
  ...
  const drawTab = (): void => {
    ...
    if (!root) return;
    const renderers: Record<TenantSettingsTab, () => string> = {
  ...
  const sectionHead = (title: string, description: string, actions = ''): string => `<div class="section-title-v17 tenant-settings-section-head-v1437"><div><h2>${esc(title)}</h2><p class="muted">${esc(description)}</p></div>${actions ? `<div class="tenant-settings-head-actions-v1437">${actions}</div>` : ''}</div>`;
      ...
      <div class="module-grid tenant-settings-two-col-v1437">
        <section class="panel glass-card embedded-panel-v32">
          ...
          <div class="tenant-settings-readiness-v1437">
            ...
            ] as Array<[string, boolean, string]>).map(([label,ready,note]) => `<button type="button" data-settings-jump="${label === 'Organization profile' ? 'profile' : label.toLowerCase().split(' ')[0]}" class="tenant-settings-readiness-row-v1437"><div><strong>${esc(label)}</strong><small>${esc(note)}</small></div>${badge(ready ? 'Configured' : 'Review required')}</button>`).join('')}
          </div>
        </section>
        <section class="panel glass-card embedded-panel-v32">
      ...
      <div class="module-grid tenant-settings-two-col-v1437">
        ...
        <section class="panel glass-card embedded-panel-v32">
          ...
          <div class="tenant-theme-grid-v1437">
            ${(['ocean','emerald','violet','graphite'] as ThemePreset[]).map(theme => `<button type="button" class="tenant-theme-card-v1437 preset-${theme} ${branding.theme === theme ? 'active' : ''}" data-theme-preview="${theme}"><span></span><strong>${theme.charAt(0).toUpperCase() + theme.slice(1)}</strong><small>${branding.theme === theme ? 'Current preset' : 'Available preset'}</small></button>`).join('')}
          </div>
  ...
  function localizationTab(): string {
    const unitRows = [
  ...
  function notificationsTab(): string {
    const preferences: Array<[string, string, boolean | null]> = [
  ...
  function securityTab(): string {
    const policies: Array<[string,string,string,string]> = [
  ...
  const applyActivityFilters = (): void => {
    ...
    if (wrap) wrap.innerHTML = activityTable(filtered);
  };

  document.addEventListener('click', event => {
    const target = event.target instanceof Element ? event.target.closest<HTMLElement>('button,[data-settings-tab],[data-settings-jump],[data-settings-edit],[data-theme-preview],[data-settings-close]') : null;
    if (!target) return;
    const tab = target.dataset.settingsTab as TenantSettingsTab | undefined;
    const jump = target.dataset.settingsJump as TenantSettingsTab | undefined;
    if (tab || jump) {
      activeTab = tab || jump || 'overview';
      document.querySelectorAll<HTMLElement>('[data-settings-tab]').forEach(button => button.classList.toggle('active', button.dataset.settingsTab === activeTab));
      drawTab();
      return;
    }
    ...
    if (edit === 'profile') editProfile();
    if (edit === 'branding') editBranding();
    if (edit === 'localization') editLocalization();
    if (edit === 'notifications') editNotifications();
    if (edit === 'data') editData();
    if (edit === 'security') editSecurity();
    const preview = target.dataset.themePreview as ThemePreset | undefined;
    if (preview) notify(unsupportedMessage);
    const action = target.dataset.settingsAction;
    if (action === 'export') exportSnapshot();
    if (action === 'export-activity') exportActivity();
  });

  document.addEventListener('input', event => {
    const target = event.target as HTMLInputElement | HTMLSelectElement | null;
    if (target?.id === 'settingsActivitySearch') applyActivityFilters();
  });
  document.addEventListener('change', event => {
    const target = event.target as HTMLInputElement | HTMLSelectElement | null;
    if (target?.id === 'settingsActivityArea') applyActivityFilters();
  });
