...
(() => {
    ...
    let events = [];
    let policies = [];
    let activeTab = localStorage.getItem('zentrid_tenant_access_tab') || 'overview';
    let selectedUserId = '';
    ...
    const firstOf = (record, keys, fallback = '') => {
    ...
    const stringList = (value) => {
        if (Array.isArray(value))
            return value.map(item => isRecord(item) ? text(firstOf(item, ['name', 'label', 'code', 'id'], '')) : text(item)).filter(Boolean);
    ...
    const tone = (value) => {
        ...
        if (/suspend|blocked|high|expired|failed|inactive|disabled/.test(valueText))
            return 'danger';
        if (/invite|pending|required|medium|due|draft|unknown/.test(valueText))
            return 'warning';
        if (/active|enabled|passed|low|approved|complete|accepted|valid/.test(valueText))
            return 'success';
        return 'info';
    };
    const badge = (value) => `<span class="badge ${tone(value)}">${esc(text(value, '—'))}</span>`;
    ...
    const unavailableAction = () => notify(unsupportedMessage);
    function explicitArray(root, keys) {
        if (!root)
            return [];
        for (const key of keys) {
    ...
    function mapUser(row, fallbackIndex = 0) {
    ...
    function mapRole(row, fallbackIndex = 0) {
    ...
    function mapInvite(row, fallbackIndex = 0) {
        return {
    ...
    function mapEvent(row, fallbackIndex = 0) {
        return {
    ...
    function mapPolicy(row, fallbackIndex = 0) {
        return {
    ...
    function stateBlock(title, detail, kind = 'empty') {
    ...
    function disabledButton(label, className = 'primary-action') {
    ...
    function render() {
      ...
      <div class="tenant-users-page-v1436" id="tenantUsersAccessRoot">
        ...
        ].map(([key, label]) => `<button type="button" class="${activeTab === key ? 'active' : ''}" data-users-tab="${key}">${label}</button>`).join('')}
          </aside>
    ...
    function userTable(list, compact = false, emptyDetail = 'No users were returned by API.') {
        if (!list.length)
            return stateBlock('No users available', emptyDetail, loadState === 'error' ? 'error' : 'empty');
    ...
    function activityList(list) {
        if (!list.length)
            return stateBlock('No access activity returned', 'The current Swagger does not expose a tenant access audit endpoint.');
    ...
    function applyUserFilters() {
        const search = document.getElementById('tenantUserSearch')?.value.trim().toLowerCase() || '';
        const role = document.getElementById('tenantUserRoleFilter')?.value || '';
        const status = document.getElementById('tenantUserStatusFilter')?.value || '';
        ...
        if (wrap)
            wrap.innerHTML = userTable(filtered, false, 'No API-backed users match the current filters.');
    }
    function exportCsv(activity = false) {
        const rows = activity
    ...
    async function loadUsersAccess(force = false) {
        loadState = 'loading';
        loadError = '';
        render();
        try {
        ...
        render();
        if (force)
            notify(loadState === 'error' ? 'Users & Access refresh failed.' : 'Users & Access refreshed from /api/Auth/me.');
    }
    document.addEventListener('click', event => {
        const target = event.target instanceof Element ? event.target.closest('button,[data-users-open],[data-role-open],[data-invite-open]') : null;
        if (!target)
            return;
        const tab = target.dataset.usersTab;
        if (tab) {
            activeTab = tab;
        ...
        const jump = target.dataset.usersJump;
        if (jump) {
            activeTab = jump;
            localStorage.setItem('zentrid_tenant_access_tab', activeTab);
            render();
        ...
        if (target.hasAttribute('data-users-close')) {
            closeDrawer();
            return;
        }
        if (target.dataset.usersOpen) {
            openUser(target.dataset.usersOpen);
            return;
        }
        if (target.dataset.roleOpen) {
            openRole(target.dataset.roleOpen);
            return;
        }
        if (target.dataset.inviteOpen) {
            reviewInvite(target.dataset.inviteOpen);
            return;
        }
        const action = target.dataset.usersAction;
        if (action === 'refresh') {
            void loadUsersAccess(true);
            return;
        }
        if (action === 'export') {
            exportCsv(false);
    ...
    document.addEventListener('input', event => {
        ...
        if (target?.id === 'tenantUserSearch')
            applyUserFilters();
    });
    document.addEventListener('change', event => {
        const target = event.target;
        if (target?.id === 'tenantUserRoleFilter' || target?.id === 'tenantUserStatusFilter')
            applyUserFilters();
    });
    render();
