...
type TelemetryQuality = 'Valid' | 'Estimated' | 'Delayed' | 'Unknown';
...
type TelemetryLoadState = 'loading' | 'live' | 'empty' | 'error';
...
(() => {
  ...
  let plants: TelemetryPlant[] = [];
  let allDevices: TelemetryDevice[] = [];
  let telemetryPoints: TelemetryPoint[] = [];
  let metricDefinitions: TelemetryMetricDefinition[] = [];
  let selectedPlantId = '';
  ...
  let loadState: TelemetryLoadState = 'loading';
  let loadError = '';
  ...
  const knownMetricFields: Array<{ keys: string[]; key: string; label: string; unit: string }> = [
  ...
  function readContext(): TelemetryContext {
    try { return JSON.parse(localStorage.getItem('zentrid_telemetry_context') || '{}') as TelemetryContext; }
    catch { return {}; }
  }
  ...
  function unitForKey(key: string, explicit: unknown): string {
  ...
  function normalizeQuality(value: unknown): TelemetryQuality {
    const normalized = text(value, '').toLowerCase();
    if (/(delay|stale|late|expired|missing)/.test(normalized)) return 'Delayed';
    if (/(estimate|interpolat|derived|calculated)/.test(normalized)) return 'Estimated';
    if (/(valid|good|ok|normal|verified|success)/.test(normalized)) return 'Valid';
    return 'Unknown';
  }

  function normalizePlantRows(payload: unknown): TelemetryPlant[] {
    const rows: TelemetryPlant[] = [];
  ...
  function normalizeDeviceRows(payload: unknown): TelemetryDevice[] {
    const rows: TelemetryDevice[] = [];
    extractRows(payload).forEach(row => {
  ...
  function dimensionContext(row: UnknownRecord, parent: UnknownRecord = {}): Omit<TelemetryPoint, 'timestamp' | 'value' | 'quality' | 'metricKey' | 'metricLabel' | 'unit'> {
    return {
  ...
  function timestampFrom(row: UnknownRecord, parent: UnknownRecord = {}): Date | null {
  ...
  function createPoint(row: UnknownRecord, parent: UnknownRecord, metricKeyValue: unknown, metricLabelValue: unknown, unitValue: unknown, value: unknown): TelemetryPoint | null {
    const timestamp = timestampFrom(row, parent);
    const numeric = numericValue(value);
    const key = normalizeKey(metricKeyValue || metricLabelValue);
    if (!timestamp || numeric === null || !key) return null;
  ...
  function expandTelemetryRow(row: UnknownRecord, parent: UnknownRecord = {}): TelemetryPoint[] {
    ...
    if (directPoint) points.push(directPoint);
  ...
  function normalizeTelemetry(payload: unknown): TelemetryPoint[] {
  ...
  function mergeTelemetryEntities(): void {
  ...
  function rebuildMetricDefinitions(): void {
    const definitions = new Map<string, TelemetryMetricDefinition>();
    telemetryPoints.forEach(point => {
      if (definitions.has(point.metricKey)) return;
  ...
  function rangeStart(now: Date): Date {
    if (selectedRange === 'Today') return new Date(now.getFullYear(), now.getMonth(), now.getDate());
    const duration = selectedRange === 'Last 30 days' ? 30 * 86400000 : selectedRange === 'Last 7 days' ? 7 * 86400000 : 24 * 3600000;
    return new Date(now.getTime() - duration);
  }

  function pointMatchesSelection(point: TelemetryPoint): boolean {
  ...
  function aggregationMs(): number {
    if (selectedGranularity === 'Daily') return 86400000;
    if (selectedGranularity === 'Hourly') return 3600000;
    if (selectedGranularity === '5 min') return 300000;
    return 900000;
  }

  function worstQuality(values: TelemetryQuality[]): TelemetryQuality {
  ...
  function selectedPoints(): TelemetryPoint[] {
  ...
  function formatValue(value: number | null, metric = selectedMetric()): string {
  ...
  function formatTime(value: Date | null | undefined): string {
    if (!value) return '—';
    if (selectedRange === 'Last 30 days') return value.toLocaleDateString(undefined, { month: 'short', day: 'numeric' });
    return value.toLocaleString(undefined, { month: 'short', day: 'numeric', hour: '2-digit', minute: '2-digit' });
  }

  function chartPath(points: TelemetryPoint[]): string {
  ...
  function chartMarkers(points: TelemetryPoint[]): string {
    if (!points.length) return '';
    const values = points.map(point => point.value);
    const min = Math.min(...values);
    const max = Math.max(...values);
  ...
  function options(values: Array<{ value: string; label: string }>, selected: string, emptyLabel: string): string {
    if (!values.length) return `<option value="">${escapeHtml(emptyLabel)}</option>`;
    return values.map(item => `<option value="${escapeHtml(item.value)}" ${item.value === selected ? 'selected' : ''}>${escapeHtml(item.label)}</option>`).join('');
  }

  function statValues(points: TelemetryPoint[]): { latest: number | null; peak: number | null; average: number | null; minimum: number | null; completeness: number | null } {
    const values = points.map(point => point.value);
  ...
  function freshnessLabel(points: TelemetryPoint[]): string {
    if (!points.length) return loadState === 'error' ? 'Unavailable' : 'No data';
  ...
  function renderPage(): void {
    const plant = selectedPlant();
    const devices = devicesForPlant();
    if (selectedDeviceId !== 'all' && !devices.some(device => device.id === selectedDeviceId)) selectedDeviceId = 'all';
    const device = selectedDevice();
  ...
  function wirePage(points: TelemetryPoint[]): void {
    const plantSelect = document.getElementById('telemetryPlant') as HTMLSelectElement | null;
    const deviceSelect = document.getElementById('telemetryDevice') as HTMLSelectElement | null;
    const metricSelect = document.getElementById('telemetryMetric') as HTMLSelectElement | null;
    const rangeSelect = document.getElementById('telemetryRange') as HTMLSelectElement | null;
    const granularitySelect = document.getElementById('telemetryGranularity') as HTMLSelectElement | null;
    ...
    document.querySelector('.telemetry-samples-table-v143')?.addEventListener('click', event => {
      const target = event.target instanceof Element ? event.target.closest<HTMLElement>('[data-telemetry-sample]') : null;
      if (!target) return;
      FleetLayout.toast(`Telemetry sample ${target.dataset.telemetrySample || ''} selected.`);
    });
  }

  function exportCsv(points: TelemetryPoint[]): void {
    if (!points.length) {
  ...
  async function loadTelemetry(forceRefresh = false): Promise<void> {
    loadState = 'loading';
    loadError = '';
    renderPage();
    const requestOptions: ZentridRequestOptions = forceRefresh ? { cache: 'no-store', retry: true } : {};
    ...
    persistContext();
    renderPage();
    if (forceRefresh) FleetLayout.toast(telemetryPoints.length ? 'Telemetry data refreshed from API.' : 'The telemetry API returned no records.');
  }
