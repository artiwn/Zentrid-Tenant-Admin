...
(() => {
    ...
    let telemetryCount = null;
    let billingProfileData = null;
    const invoices = [];
    const payments = [];
    try {
    ...
    function mergeEntityCount(primaryPayload, secondaryPayload) {
        ...
        const explicit = [totalCount(primaryPayload), totalCount(secondaryPayload)].filter((value) => value !== null);
    ...
    function nestedRecord(row, keys) {
        for (const key of keys) {
            const value = row[key];
            if (isRecord(value))
                return value;
    ...
    function explicitBillingProfile(payload) {
    ...
    function tone(value) {
        ...
        if (status.includes('paid') || status.includes('settled') || status.includes('included') || status.includes('active'))
            return 'success';
        if (status.includes('pending') || status.includes('processing') || status.includes('draft') || status.includes('billable') || status.includes('review'))
            return 'warning';
        if (status.includes('overdue') || status.includes('failed') || status.includes('blocked'))
            return 'danger';
        return 'info';
    }
    ...
    function freshnessText() {
        if (loadState === 'loading')
            return 'Loading API usage context…';
        if (loadState === 'error')
            return 'API usage context unavailable';
        if (!loadedAt)
            return 'No finance endpoint in active API';
    ...
    function invoiceRegistry(compact = false) {
    ...
    function tabContent() {
        ...
        if (activeTab === 'invoices')
            return `${invoiceRegistry()}${invoiceDetail()}`;
    ...
    function wire() {
        document.querySelectorAll('[data-finance-tab]').forEach(button => button.addEventListener('click', () => switchTab((button.dataset.financeTab || 'overview'))));
        document.querySelectorAll('[data-finance-export]').forEach(button => button.addEventListener('click', () => exportCsv(button.dataset.financeExport || 'summary')));
        document.querySelectorAll('[data-finance-download]').forEach(button => button.addEventListener('click', event => { event.stopPropagation(); downloadInvoice(button.dataset.financeDownload || ''); }));
        document.querySelector('[data-finance-refresh]')?.addEventListener('click', () => { void loadFinanceData(true); });
        ...
        searchInput?.addEventListener('keydown', event => { if (event.key === 'Enter') {
            invoiceSearch = searchInput.value;
            render();
        } });
        const statusSelect = document.getElementById('financeInvoiceStatus');
    ...
    async function loadFinanceData(force = false) {
        ...
        render();
        if (force)
            FleetLayout.toast(loadState === 'error' ? 'Finance usage context refresh failed.' : 'Finance usage context refreshed from API data.');
    }
    render();
