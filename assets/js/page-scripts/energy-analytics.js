...
(() => {
    ...
    function unitFrom(value, key = '') {
        const explicit = text(value, '').toLowerCase().replace(/\s+/g, '');
        if (explicit)
            return explicit;
        const normalizedKey = normalize(key);
        if (normalizedKey.endsWith('_gwh'))
            return 'gwh';
        if (normalizedKey.endsWith('_mwh'))
            return 'mwh';
        if (normalizedKey.endsWith('_kwh'))
            return 'kwh';
        if (normalizedKey.endsWith('_wh'))
            return 'wh';
        if (normalizedKey.endsWith('_mw'))
            return 'mw';
        if (normalizedKey.endsWith('_kw'))
            return 'kw';
        if (normalizedKey.endsWith('_w'))
            return 'w';
        if (normalizedKey.endsWith('_pct') || normalizedKey.endsWith('_percent'))
            return '%';
        return '';
    }
    function energyToMwh(value, unitValue, key = '') {
        ...
        if (unit.includes('kwh'))
            return numeric / 1000;
        if (/^wh$|watt.?hour/.test(unit))
            return numeric / 1_000_000;
        return null;
    }
    function powerToMw(value, unitValue, key = '') {
        const numeric = numericValue(value);
        if (numeric === null)
            return null;
        const unit = unitFrom(unitValue, key);
        if (unit === 'mw' || unit.includes('megawatt'))
            return numeric;
        if (unit === 'kw' || unit.includes('kilowatt'))
            return numeric / 1000;
        if (unit === 'w' || unit.includes('watt'))
            return numeric / 1_000_000;
        const raw = text(value, '').toLowerCase();
        if (raw.includes('mwp') || raw.includes('mw'))
            return numeric;
        if (raw.includes('kwp') || raw.includes('kw'))
            return numeric / 1000;
        return null;
    }
    ...
    function normalizeQuality(value) {
        const raw = text(value, '').toLowerCase();
        if (/(delay|stale|late|expired|missing)/.test(raw))
            return 'Delayed';
        if (/(partial|estimate|interpolat|derived|calculated)/.test(raw))
            return 'Partial';
        if (/(valid|fresh|good|ok|normal|verified|success)/.test(raw))
            return 'Fresh';
    ...
    function classifyMetric(keyValue, labelValue) {
        const key = normalize(`${text(keyValue, '')} ${text(labelValue, '')}`);
        if (!key)
            return null;
        if (/(performance_ratio|\bpr\b)/.test(key))
            return 'performanceRatio';
        if (/(availability|uptime)/.test(key))
            return 'availability';
        if (/(revenue|income|estimated_value|commercial_value)/.test(key))
            return 'revenue';
        if (/(current_power|active_power|ac_power|power_now|instant_power|^power$)/.test(key))
            return 'power';
        if (/(battery.*discharg|discharg.*battery)/.test(key))
            return 'batteryDischarge';
        if (/(battery.*charg|charg.*battery)/.test(key))
            return 'batteryCharge';
        if (/(grid.*export|export.*grid|feed_in|feedin)/.test(key))
            return 'gridExport';
        if (/(grid.*import|import.*grid|purchased_energy)/.test(key))
            return 'gridImport';
        if (/(load_consum|consum.*energy|energy.*consum|used_energy|self_consum|load_energy)/.test(key))
            return 'used';
        if (/(generat.*energy|energy.*generat|production_energy|energy_production|produced_energy|daily_yield|energy_today|interval_energy|^energy$)/.test(key))
            return 'produced';
        return null;
    }
    function metricMode(keyValue, kind) {
        if (kind === 'power' || kind === 'performanceRatio' || kind === 'availability' || kind === 'revenue')
            return 'snapshot';
        const key = normalize(keyValue);
        if (/(total|lifetime|counter|cumulative)/.test(key))
            return 'counter';
        if (/(today|daily|day_yield)/.test(key))
            return 'daily';
        return 'interval';
    }
    function metricNumericValue(kind, value, unit, key) {
        if (kind === 'power')
            return powerToMw(value, unit, key);
        if (kind === 'performanceRatio' || kind === 'availability')
    ...
    function normalizeTelemetry(payload) {
        ...
        function addPoint(row, keyValue, labelValue, rawValue, unitValue) {
        ...
        extractRows(payload).forEach(row => {
            ...
            if (text(metricKey, '') && numericValue(value) !== null)
                addPoint(row, metricKey, metricLabel, value, firstOf(row, ['unit', 'uom', 'measurementUnit'], ''));
            const metrics = row.metrics;
            if (isRecord(metrics)) {
                Object.entries(metrics).forEach(([key, metricValue]) => {
                    if (isRecord(metricValue))
                        addPoint({ ...row, ...metricValue }, key, firstOf(metricValue, ['label', 'name'], key), firstOf(metricValue, ['value', 'reading'], ''), firstOf(metricValue, ['unit', 'uom'], ''));
                    else
                        addPoint(row, key, key, metricValue, '');
                });
            }
    ...
    function matchPointToPlant(point, plant) {
        if (point.plantId && identity(point.plantId) === identity(plant.id))
            return true;
        if (point.plantName && identity(point.plantName) === identity(plant.name))
            return true;
        return false;
    }
    function aggregateMetric(points, kind) {
        const kindPoints = points.filter(point => point.kind === kind);
        if (!kindPoints.length)
            return null;
        if (kind === 'power' || kind === 'performanceRatio' || kind === 'availability' || kind === 'revenue')
            return kindPoints[kindPoints.length - 1]?.value ?? null;
    ...
    function actualNumberField(row, keys) {
        for (const key of keys) {
    ...
    function rebuildEnergyModels() {
        ...
        energyClients = apiClients.map(client => {
            ...
            const latestTimestamp = points.filter(point => rows.some(plant => matchPointToPlant(point, apiPlants.find(item => item.id === plant.id) || { id: plant.id, name: plant.name }))).sort((a, b) => b.timestamp.getTime() - a.timestamp.getTime())[0]?.timestamp || null;
            return {
                id: client.id,
                name: client.name,
                type: client.type,
    ...
    function currentPlants() {
        ...
        if (selectedClientId !== 'all')
            rows = rows.filter(plant => identity(plant.clientId) === identity(selectedClientId));
        if (selectedPlantId !== 'all')
            rows = rows.filter(plant => identity(plant.id) === identity(selectedPlantId));
        if (search.trim()) {
    ...
    function currentClients() {
        ...
        if (selectedClientId !== 'all')
            rows = rows.filter(client => identity(client.id) === identity(selectedClientId));
        if (search.trim()) {
            const query = search.trim().toLowerCase();
    ...
    function statusClass(value) {
        ...
        if (normalized.includes('fault') || normalized.includes('offline') || normalized.includes('stale') || normalized.includes('error'))
            return 'danger';
        if (normalized.includes('warning') || normalized.includes('review') || normalized.includes('maintenance') || normalized.includes('partial') || normalized.includes('delayed'))
            return 'warning';
        if (normalized.includes('unknown') || normalized.includes('no '))
            return 'neutral';
        return 'success';
    }
    ...
    function toolbar() {
      ...
      <div class="toolbar production-toolbar energy-toolbar-v1431">
        ...
        <select id="energyPeriod">${['Today', 'Last 7 days', 'Last 30 days', 'Year to date'].map(value => option(value, value, period)).join('')}</select>
    ...
    function selectedTelemetryPoints(plants) {
    ...
    function chartBucketKey(point, mode) {
    ...
    function chartBucketLabel(point, mode) {
        if (mode === 'day')
            return point.timestamp.toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' });
        if (mode === 'weekly')
            return point.timestamp.toLocaleDateString([], { month: 'short', day: 'numeric' });
        return point.timestamp.toLocaleDateString([], { month: 'short', year: '2-digit' });
    }
    function chartSeries(mode, plants) {
        const energyKinds = ['produced', 'used', 'gridExport', 'batteryCharge'];
        ...
        return [...buckets.values()]
            ...
            .map(bucket => ({
            label: chartBucketLabel(bucket[0], mode),
    ...
    function pointsFor(values, max, width = 720, height = 250, padding = 32) {
    ...
    function svgChart(mode, plants) {
        ...
        const numeric = [...produced, ...used, ...exported, ...battery].filter((value) => value !== null && Number.isFinite(value));
    ...
    function comparisonCards(rows) {
        ...
        if (!available.length)
            return `<div class="empty-state"><strong>No production values available</strong><small>The API returned no produced-energy metric for the current filters.</small></div>`;
    ...
    function flowBreakdown(flow) {
        ...
        const available = values.map(([, value]) => value).filter((value) => value !== null);
    ...
    function clientsTable(rows) {
        if (!rows.length)
            return `<div class="empty-state"><strong>No clients match the current filters</strong><small>${loadState === 'error' ? escapeHtml(loadError) : 'The API returned no client records for this scope.'}</small></div>`;
    ...
    function plantsTable(rows) {
        if (!rows.length)
            return `<div class="empty-state"><strong>No plants match the current filters</strong><small>${loadState === 'error' ? escapeHtml(loadError) : 'The API returned no plant records for this scope.'}</small></div>`;
    ...
    function chartTable(mode, plants) {
        const rows = chartSeries(mode, plants);
        if (!rows.length)
            return `${svgChart(mode, plants)}<div class="data-table production-table-v130 energy-chart-table-v1431"><div class="data-head"><span>${mode === 'day' ? 'Time' : mode === 'weekly' ? 'Day' : 'Month'}</span><span>Produced</span><span>Used</span><span>Battery Charge</span><span>Grid Export</span><span>Estimated Value</span></div><div class="empty-state"><strong>No API chart intervals</strong><small>No timestamped energy metrics are available for this scope.</small></div></div>`;
    ...
    function performanceTable(rows) {
        if (!rows.length)
            return `<div class="empty-state"><strong>No plant performance records</strong><small>The API returned no plants for this scope.</small></div>`;
    ...
    function sourceTable(rows) {
        ...
        if (!unique.length)
            return `<div class="section-title-v17"><div><h2>Source Mapping</h2><p class="muted">Read-only lineage from vendor fields to Zentrid canonical energy metrics.</p></div><span class="badge info">Read-only</span></div><div class="empty-state"><strong>No energy mapping records</strong><small>The telemetry API returned no recognized energy fields for this scope.</small></div>`;
    ...
    function tabContent(flow, plants, clients) {
        if (loadState === 'loading' && !apiPlants.length)
            return `<div class="empty-state"><strong>Loading energy data</strong><small>Reading plants, clients, devices and telemetry from the API.</small></div>`;
        if (loadState === 'error' && !apiPlants.length && !telemetryPoints.length)
            return `<div class="empty-state"><strong>Energy API unavailable</strong><small>${escapeHtml(loadError)}</small></div>`;
        if (activeTab === 'clients')
            return `<div class="section-title-v17"><div><h2>Client Energy Portfolio</h2><p class="muted">Energy allocation grouped by API client and plant relations.</p></div></div>${clientsTable(clients)}`;
        if (activeTab === 'plants')
            return `<div class="section-title-v17"><div><h2>Plant Energy Portfolio</h2><p class="muted">Plant-level production, source, grid, storage and performance.</p></div></div>${plantsTable(plants)}`;
        if (activeTab === 'flow')
            return `<div class="section-title-v17"><div><h2>Energy Flow</h2><p class="muted">Bidirectional production, load, battery and grid movement reported by API.</p></div></div>${flowBreakdown(flow)}<div class="info-grid production-detail-grid"><div><span>Self-consumption Ratio</span><strong>${formatPercent(ratio(flow.used, flow.produced))}</strong><small>Reported used energy divided by produced energy</small></div><div><span>Export Ratio</span><strong>${formatPercent(ratio(flow.gridExport, flow.produced))}</strong><small>Reported export divided by produced energy</small></div><div><span>Storage Capture</span><strong>${formatPercent(ratio(flow.batteryCharge, flow.produced))}</strong><small>Reported battery charge divided by produced energy</small></div><div><span>Net Grid Position</span><strong>${formatEnergy(subtract(flow.gridExport, flow.gridImport))}</strong><small>Export minus import when both values exist</small></div></div>`;
        if (activeTab === 'day')
            return chartTable('day', plants);
        if (activeTab === 'weekly')
            return chartTable('weekly', plants);
        if (activeTab === 'monthly')
            return chartTable('monthly', plants);
        if (activeTab === 'performance')
            return performanceTable(plants);
        if (activeTab === 'source')
            return sourceTable(plants);
    ...
    function wirePage(plants) {
        document.querySelectorAll('[data-energy-tab]').forEach(button => button.addEventListener('click', () => {
            activeTab = (button.dataset.energyTab || 'overview');
            renderPage();
        }));
        const searchInput = document.getElementById('energySearch');
        ...
        const clientSelect = document.getElementById('energyClient');
        clientSelect?.addEventListener('change', () => { selectedClientId = clientSelect.value; selectedPlantId = 'all'; renderPage(); });
        const plantSelect = document.getElementById('energyPlant');
        plantSelect?.addEventListener('change', () => { selectedPlantId = plantSelect.value; renderPage(); });
        const periodSelect = document.getElementById('energyPeriod');
        periodSelect?.addEventListener('change', () => { period = periodSelect.value; renderPage(); });
        ...
        document.querySelector('.energy-analytics-page-v1431')?.addEventListener('click', event => {
            const target = event.target instanceof Element ? event.target.closest('[data-energy-open-plant],[data-energy-open-client],[data-energy-open-telemetry],[data-energy-focus-plant],[data-energy-plant],[data-energy-client]') : null;
            if (!target)
                return;
            ...
            if (openPlant) {
                ...
                navigatePlant(openPlant);
                return;
            }
            if (openClient) {
                event.stopPropagation();
                navigateClient(openClient);
                return;
            }
            if (openTelemetry) {
                event.stopPropagation();
                navigateTelemetry(openTelemetry);
                return;
            }
            if (focusPlant) {
                event.stopPropagation();
                selectedPlantId = focusPlant;
                activeTab = 'overview';
                renderPage();
                return;
            }
            if (target.dataset.energyPlant && !target.closest('.kebab-menu')) {
                selectedPlantId = target.dataset.energyPlant;
                activeTab = 'overview';
                renderPage();
                return;
            }
            if (target.dataset.energyClient) {
                selectedClientId = target.dataset.energyClient;
                selectedPlantId = 'all';
                activeTab = 'overview';
                renderPage();
            }
    ...
    async function loadEnergyData(force = false) {
        loadState = 'loading';
        loadError = '';
        if (!lastUpdatedAt)
            renderPage();
        ...
        renderPage();
        if (force)
            FleetLayout.toast(loadState === 'error' ? 'Energy Analytics API refresh failed.' : 'Energy Analytics refreshed from API data.');
    }
    renderPage();
    void loadEnergyData();
})();
