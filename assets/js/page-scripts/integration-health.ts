...
type IntegrationTab = 'overview' | 'connectors' | 'sync' | 'coverage' | 'failures' | 'mapping' | 'activity';
type IntegrationTone = 'success' | 'warning' | 'danger' | 'info' | 'neutral';
type LiveState = 'loading' | 'live' | 'partial' | 'empty' | 'error';
...
(() => {
  ...
  let integrations: TenantIntegration[] = [];
  let failures: IntegrationFailure[] = [];
  let activities: IntegrationActivity[] = [];
  let mappings: IntegrationMapping[] = [];
  let activeTab: IntegrationTab = 'overview';
  let selectedId = '';
  let search = '';
  let statusFilter = 'All Statuses';
  let vendorFilter = 'All Vendors';
  let liveState: LiveState = 'loading';
  let liveMessage = 'Loading connector registry and synchronization summary…';
  let escapeBound = false;

  try {
    const saved = JSON.parse(localStorage.getItem(uiKey) || '{}') as Partial<{ activeTab: IntegrationTab; selectedId: string }>;
  ...
  function textValue(record: Record<string, unknown>, keys: string[], fallback: string): string {
  ...
  function numericValue(record: Record<string, unknown>, keys: string[], fallback: number): number {
    for (const key of keys) {
  ...
  function displayId(item: TenantIntegration): string {
  ...
  function tone(value: string): IntegrationTone {
    const normalized = normalize(value);
    if (normalized.includes('healthy') || normalized.includes('active') || normalized.includes('valid') || normalized.includes('completed') || normalized.includes('resolved') || normalized.includes('mapped')) return 'success';
    if (normalized.includes('delay') || normalized.includes('warning') || normalized.includes('monitor') || normalized.includes('partial')) return 'warning';
    if (normalized.includes('fail') || normalized.includes('critical') || normalized.includes('expired') || normalized.includes('offline')) return 'danger';
    if (normalized.includes('info') || normalized.includes('loading')) return 'info';
    return 'neutral';
  }

  function healthLabel(record: TenantIntegration): string {
    if (record.health) return record.health;
    if (record.status === 'Active' && record.syncLagMinutes <= 10) return 'Healthy';
    if (record.syncLagMinutes > 10) return 'Delayed';
  ...
  function sideTabs(): string {
    const tabs: Array<[IntegrationTab, string]> = [
  ...
  function filteredIntegrations(): TenantIntegration[] {
  ...
  function integrationRows(rows: TenantIntegration[], compact = false): string {
    if (!rows.length) return '<div class="empty-state zentrid-ux-state zentrid-ux-state-empty"><strong>No connectors match current filters.</strong><span>Clear filters or refresh tenant connector data.</span></div>';
  ...
  function coverageBars(rows: TenantIntegration[]): string {
    if (!rows.length) return '<div class="empty-state zentrid-ux-state zentrid-ux-state-empty"><strong>No connector coverage data available.</strong><span>Coverage will appear when the backend returns integration metrics.</span></div>';
  ...
  function activityList(rows: IntegrationActivity[]): string {
    if (!rows.length) return '<div class="empty-state zentrid-ux-state zentrid-ux-state-empty"><strong>No integration activity returned by the API.</strong><span>Synchronization and validation events will appear when provided by the backend.</span></div>';
  ...
  function operationalSummary(): string {
  ...
  function tabContent(): string {
    if (activeTab === 'connectors') return connectors();
    if (activeTab === 'sync') return syncHealth();
    if (activeTab === 'coverage') return coverage();
    if (activeTab === 'failures') return failuresView();
    if (activeTab === 'mapping') return mapping();
  ...
  function switchTab(tab: IntegrationTab): void {
    activeTab = tab;
    saveUi();
    render();
  }
  ...
  function openDrawer(id: string): void {
    if (!integrations.some(item => item.id === id)) return;
    selectedId = id;
    saveUi();
    ...
    requestAnimationFrame(() => {
      ...
      next.querySelector<HTMLElement>('.drawer-close')?.focus();
    });
    wireControls();
    wireOpenRows(next);
  }
  ...
  function route(routeName: string): void {
  ...
  function download(filename: string, content: string, type = 'text/csv;charset=utf-8'): void {
  ...
  function exportDiagnostics(selected?: TenantIntegration): void {
  ...
  function wireOpenRows(root: Document | Element): void {
    root.querySelectorAll<HTMLElement>('[data-integration-open]').forEach(element => {
      if (element.dataset.integrationWired === 'true') return;
  ...
  function wireControls(): void {
    document.querySelectorAll<HTMLElement>('[data-integration-tab]').forEach(button => {
      if (button.dataset.integrationControlWired === 'true') return;
      button.dataset.integrationControlWired = 'true';
      button.addEventListener('click', () => switchTab((button.dataset.integrationTab || 'overview') as IntegrationTab));
    });
    document.querySelectorAll<HTMLElement>('[data-integration-action]').forEach(button => {
      if (button.dataset.integrationControlWired === 'true') return;
      button.dataset.integrationControlWired = 'true';
      button.addEventListener('click', event => {
        event.stopPropagation();
        const action = button.dataset.integrationAction || '';
        if (action === 'refresh') void loadLive(true);
        if (action === 'export') exportDiagnostics();
        if (action === 'export-selected') exportDiagnostics(integrations.find(item => item.id === selectedId));
        if (action === 'close-drawer') closeDrawer();
        if (action === 'clear-filters') { search = ''; statusFilter = 'All Statuses'; vendorFilter = 'All Vendors'; render(); }
      });
    });
    document.querySelectorAll<HTMLElement>('[data-integration-route]').forEach(button => {
      if (button.dataset.integrationControlWired === 'true') return;
      button.dataset.integrationControlWired = 'true';
  ...
  function wireFilters(): void {
    const searchInput = document.getElementById('integrationSearch') as HTMLInputElement | null;
    searchInput?.addEventListener('input', () => {
      search = searchInput.value;
      const table = document.getElementById('integrationConnectorTable');
      if (table) table.innerHTML = integrationRows(filteredIntegrations());
      if (table) wireOpenRows(table);
    });
    const status = document.getElementById('integrationStatusFilter') as HTMLSelectElement | null;
    status?.addEventListener('change', () => { statusFilter = status.value; render(); });
    const vendor = document.getElementById('integrationVendorFilter') as HTMLSelectElement | null;
  ...
  function integrationKey(row: Record<string, unknown>, index: number): string {
  ...
  function matchSummary(row: Record<string, unknown>, summaries: Array<Record<string, unknown>>): Record<string, unknown> {
    ...
    return summaries.find(summary => {
      ...
      if (id && summaryId && id === summaryId) return true;
  ...
  function mergeLiveRows(registryRows: Array<Record<string, unknown>>, summaryRows: Array<Record<string, unknown>>): TenantIntegration[] {
  ...
  function collectFailures(rows: Array<Record<string, unknown>>, integrationsRows: TenantIntegration[]): IntegrationFailure[] {
    const result: IntegrationFailure[] = [];
    rows.forEach((row, index) => {
      ...
      const severity: IntegrationFailure['severity'] = normalize(severityText).includes('critical') ? 'Critical' : normalize(severityText).includes('info') ? 'Info' : 'Warning';
      result.push({
  ...
  function collectActivities(rows: Array<Record<string, unknown>>, integrationsRows: TenantIntegration[]): IntegrationActivity[] {
    const result: IntegrationActivity[] = [];
    rows.forEach((row, index) => {
  ...
  function collectMappings(rows: Array<Record<string, unknown>>, integrationsRows: TenantIntegration[]): IntegrationMapping[] {
  ...
  async function loadLive(forceRefresh = false): Promise<void> {
    liveState = 'loading';
    liveMessage = 'Loading connector registry and synchronization summary…';
    render();
    try {
      if (!window.FleetAPIRepositories?.integrations) throw new Error('Integration repository is unavailable.');
      ...
      if (!integrations.some(item => item.id === selectedId)) selectedId = integrations[0]?.id || '';
