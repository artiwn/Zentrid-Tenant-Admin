(() => {
  ...
  function roleAllowed(): boolean {
    const roles = window.ZentridAuth?.getRoles?.() || [];
    return roles.some((role: string) => REQUIRED_ROLES.includes(normalizeRole(role)));
  }
  ...
  async function ensureAllowedSession(): Promise<boolean> {
    const valid = await window.ZentridAuth.ensureSession('');
    if (!valid) return false;
    if (!window.ZentridAuth.getRoles().length) {
      try { await window.ZentridAuth.me(); } catch { return false; }
    }
    return roleAllowed();
  }
  ...
  function validateSynchronizedSession(): void {
    ...
    const roles = window.ZentridAuth.getRoles();
    if (roles.length && !roleAllowed()) redirectToLogin('role');
  }
  ...
  void ensureAllowedSession().then(valid => {
    if (!valid) redirectToLogin(window.ZentridAuth.getRoles().length ? 'role' : 'session');
