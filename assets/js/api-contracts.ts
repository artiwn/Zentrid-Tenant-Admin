...
(function () {
  ...
  const clients = createContract<ZentridClientDto>(CONTRACT_DEFINITIONS.clients, (row, _index, context) => {
    ...
    return {
      ...
      documents: explicitDocuments !== undefined
        ...
        : (Array.isArray(row.documentRecords) ? row.documentRecords.length : (documentFlags.length ? documentFlags.reduce<number>((count, value) => count + Number(Boolean(value)), 0) : (nestedDocumentation && typeof nestedDocumentation === 'object' ? Object.values(nestedDocumentation as Record<string, unknown>).filter(Boolean).length : null))),
      ...
      documentRecords: Array.isArray(row.documentRecords) ? row.documentRecords.map((document: any) => ({
  ...
  const tenants = createContract<ZentridTenantDto>(CONTRACT_DEFINITIONS.tenants, (row, _index, context) => {
    ...
    return {
      ...
      contacts: (Array.isArray(context.firstOf(row, ['contactPersons.contacts', 'contacts'], [])) ? context.firstOf(row, ['contactPersons.contacts', 'contacts'], []) as Record<string, unknown>[] : []).map(contact => ({
        ...contact,
      ...
      documents: (Array.isArray(context.firstOf(row, ['legalCompliance.documents', 'documents'], [])) ? context.firstOf(row, ['legalCompliance.documents', 'documents'], []) as Record<string, unknown>[] : []).map(document => ({
        ...document,
